...
class DropEdgeIndexProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static DropEdgeIndexProcessor* instance(kvstore::KVStore* kvstore) { return new DropEdgeIndexProcessor(kvstore); }

  void process(const cpp2::DropEdgeIndexReq& req);

 private:
  explicit DropEdgeIndexProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
