...
void ListListenerProcessor::process(const cpp2::ListListenerReq& req) {
  ...
  auto activeHosts = ActiveHostsMan::getActiveHosts(kvstore_, FLAGS_heartbeat_interval_secs * FLAGS_expired_time_factor,
  ...
  while (iter->valid()) {
    ...
    if (std::find(activeHosts.begin(), activeHosts.end(), *listener.host_ref()) != activeHosts.end()) {
