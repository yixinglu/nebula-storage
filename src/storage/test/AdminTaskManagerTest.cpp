...
struct HookableTask : public AdminTask {
  ...
  void addSubTask(std::function<cpp2::ErrorCode()> subTask) { subTasks.emplace_back(subTask); }
...
TEST(TaskManagerTest, run_a_medium_task_before_a_huge_task) {
  ...
  {  // 256 sub tasks
    ...
    auto taskCallback = [&](cpp2::ErrorCode ret, nebula::meta::cpp2::StatisItem&) { pro.setValue(ret); };
    mockTask->setCallback(taskCallback);
...
TEST(TaskManagerTest, gen_sub_task_failed) {
...

  {
    std::shared_ptr<AdminTask> task = std::make_shared<HookableTask>();
    HookableTask* mockTask = dynamic_cast<HookableTask*>(task.get());
    mockTask->fGenSubTasks = [&]() { return cpp2::ErrorCode::E_INVALID_TASK_PARA; };

    folly::Promise<ResultCode> pro;
    folly::Future<ResultCode> fut = pro.getFuture();

    auto cb = [&](cpp2::ErrorCode ret, nebula::meta::cpp2::StatisItem&) { pro.setValue(ret); };
...
TEST(TaskManagerTest, some_subtask_failed) {
  auto taskMgr = AdminTaskManager::instance();
  taskMgr->init();
  std::vector<cpp2::ErrorCode> errorCode{cpp2::ErrorCode::SUCCEEDED, cpp2::ErrorCode::E_PART_NOT_FOUND,
                                         cpp2::ErrorCode::E_LEADER_CHANGED, cpp2::ErrorCode::E_USER_CANCEL,
  ...
  for (auto t = ++gJobId; t < 5; ++t) {
    ...
    mockTask->setCallback([&](cpp2::ErrorCode ret, nebula::meta::cpp2::StatisItem&) { pro.setValue(ret); });
    taskMgr->addAsyncTask(task);
    fut.wait();
...
TEST(TaskManagerTest, cancel_a_task_before_all_sub_task_running) {
  ...
  task0->setCallback([&](cpp2::ErrorCode ret, nebula::meta::cpp2::StatisItem&) { pFiniTask0.setValue(ret); });
...
TEST(TaskManagerTest, cancel_a_task_while_some_sub_task_running) {
  ...
  task1->setCallback([&](cpp2::ErrorCode ret, nebula::meta::cpp2::StatisItem&) { task1_p.setValue(ret); });
