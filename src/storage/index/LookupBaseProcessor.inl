...
cpp2::ErrorCode LookupBaseProcessor<REQ, RESP>::requestCheck(const cpp2::LookupIndexRequest& req) {
  ...
  planContext_ = std::make_unique<PlanContext>(this->env_, spaceId_, this->spaceVidLen_, this->isIntId_);
  ...
  if (indices.get_contexts().empty() || !req.return_columns_ref().has_value() || (*req.return_columns_ref()).empty()) {
...
template <typename REQ, typename RESP>
bool LookupBaseProcessor<REQ, RESP>::isOutsideIndex(Expression* filter, const meta::cpp2::IndexItem* index) {
  ...
  switch (filter->kind()) {
    ...
    case Expression::Kind::kEdgeProperty: {
      ...
      auto propName = *(sExpr->prop());
      auto it =
          std::find_if(fields.begin(), fields.end(), [&propName](const auto& f) { return f.get_name() == propName; });
...
StatusOr<StoragePlan<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlan() {
  ...
  for (const auto& ctx : contexts_) {
    ...
    bool needData = false;
    auto index = planContext_->isEdge_ ? this->env_->indexMan_->getEdgeIndex(spaceId_, indexId)
    ...
    for (const auto& yieldCol : yieldCols_) {
      ...
      auto it = std::find_if(fields.begin(), fields.end(),
                             [&yieldCol](const auto& columnDef) { return yieldCol == columnDef.get_name(); });
      if (it == fields.end()) {
        needData = true;
        break;
      }
    ...
    if (!needData && !needFilter) {
      ...
      auto exprCtx = std::make_unique<StorageExpressionContext>(planContext_->vIdLen_, planContext_->isIntId_,
                                                                hasNullableCol, fields);
      filterItems_.emplace(filterId, std::make_pair(std::move(exprCtx), std::move(expr)));
      out = buildPlanWithFilter(ctx, plan, filterItems_[filterId].first.get(), filterItems_[filterId].second.get());
      filterId++;
    } else {
      auto expr = Expression::decode(ctx.get_filter());
      // Need to get columns in data, expr ctx need to be aware of schema
      const auto& schemaName = planContext_->isEdge_ ? planContext_->edgeName_ : planContext_->tagName_;
      if (schemas_.empty()) {
        return Status::Error("Schema not found");
      }
      auto exprCtx = std::make_unique<StorageExpressionContext>(
          planContext_->vIdLen_, planContext_->isIntId_, schemaName, schemas_.back().get(), planContext_->isEdge_);
      filterItems_.emplace(filterId, std::make_pair(std::move(exprCtx), std::move(expr)));
      out = buildPlanWithDataAndFilter(ctx, plan, filterItems_[filterId].first.get(),
...
std::unique_ptr<IndexOutputNode<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlanBasic(
    const cpp2::IndexQueryContext& ctx, StoragePlan<IndexID>& plan, bool hasNullableCol,
    const std::vector<meta::cpp2::ColumnDef>& fields) {
  auto indexId = ctx.get_index_id();
  auto colHints = ctx.get_column_hints();
  auto indexScan = std::make_unique<IndexScanNode<IndexID>>(planContext_.get(), indexId, std::move(colHints));

  auto output = std::make_unique<IndexOutputNode<IndexID>>(&resultDataSet_, planContext_.get(), indexScan.get(),
                                                           hasNullableCol, fields);
...
std::unique_ptr<IndexOutputNode<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlanWithData(
    const cpp2::IndexQueryContext& ctx, StoragePlan<IndexID>& plan) {
  auto indexId = ctx.get_index_id();
  auto colHints = ctx.get_column_hints();

  auto indexScan = std::make_unique<IndexScanNode<IndexID>>(planContext_.get(), indexId, std::move(colHints));
  if (planContext_->isEdge_) {
    auto edge = std::make_unique<IndexEdgeNode<IndexID>>(planContext_.get(), indexScan.get(), schemas_,
                                                         planContext_->edgeName_);
    edge->addDependency(indexScan.get());
    auto output = std::make_unique<IndexOutputNode<IndexID>>(&resultDataSet_, planContext_.get(), edge.get());
    output->addDependency(edge.get());
    plan.addNode(std::move(indexScan));
    plan.addNode(std::move(edge));
    return output;
  } else {
    auto vertex = std::make_unique<IndexVertexNode<IndexID>>(planContext_.get(), this->vertexCache_, indexScan.get(),
                                                             schemas_, planContext_->tagName_);
    vertex->addDependency(indexScan.get());
    auto output = std::make_unique<IndexOutputNode<IndexID>>(&resultDataSet_, planContext_.get(), vertex.get());
...
std::unique_ptr<IndexOutputNode<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlanWithFilter(
    const cpp2::IndexQueryContext& ctx, StoragePlan<IndexID>& plan, StorageExpressionContext* exprCtx,
    Expression* exp) {
  auto indexId = ctx.get_index_id();
  auto colHints = ctx.get_column_hints();
  ...
  auto filter = std::make_unique<IndexFilterNode<IndexID>>(indexScan.get(), exprCtx, exp, planContext_->isEdge_);
  filter->addDependency(indexScan.get());
  auto output = std::make_unique<IndexOutputNode<IndexID>>(&resultDataSet_, planContext_.get(), filter.get(), true);
 ...
 **/
template <typename REQ, typename RESP>
std::unique_ptr<IndexOutputNode<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlanWithDataAndFilter(
    const cpp2::IndexQueryContext& ctx, StoragePlan<IndexID>& plan, StorageExpressionContext* exprCtx,
    Expression* exp) {
  auto indexId = ctx.get_index_id();
  auto colHints = ctx.get_column_hints();
  ...
  if (planContext_->isEdge_) {
    ...
    auto output = std::make_unique<IndexOutputNode<IndexID>>(&resultDataSet_, planContext_.get(), filter.get());
    output->addDependency(filter.get());
    plan.addNode(std::move(indexScan));
    plan.addNode(std::move(edge));
    plan.addNode(std::move(filter));
