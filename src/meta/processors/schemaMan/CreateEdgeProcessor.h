...
class CreateEdgeProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static CreateEdgeProcessor* instance(kvstore::KVStore* kvstore) { return new CreateEdgeProcessor(kvstore); }

  void process(const cpp2::CreateEdgeReq& req);

 private:
  explicit CreateEdgeProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
