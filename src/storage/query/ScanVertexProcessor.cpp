...
void ScanVertexProcessor::doProcess(const cpp2::ScanVertexRequest& req) {
  ...
  auto kvRet =
      env_->kvstore_->rangeWithPrefix(spaceId_, partId_, start, prefix, &iter, req.get_enable_read_from_follower());
  ...
  for (int64_t rowCount = 0; iter->valid() && rowCount < rowLimit; iter->next()) {
    ...
    if (!QueryUtils::collectVertexProps(key, spaceVidLen_, isIntId_, reader.get(), props, list).ok()) {
      continue;
    }
