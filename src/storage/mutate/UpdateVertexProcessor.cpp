...
cpp2::ErrorCode UpdateVertexProcessor::checkAndBuildContexts(const cpp2::UpdateVertexRequest& req) {
...
StoragePlan<VertexID> UpdateVertexProcessor::buildPlan(nebula::DataSet* result) {
  ...
  auto tagUpdate = std::make_unique<TagNode>(planContext_.get(), &tagContext_, tagContext_.propContexts_[0].first,
                                             &(tagContext_.propContexts_[0].second));

  auto filterNode =
      std::make_unique<FilterNode<VertexID>>(planContext_.get(), tagUpdate.get(), expCtx_.get(), filterExp_.get());
  filterNode->addDependency(tagUpdate.get());

  auto updateNode = std::make_unique<UpdateTagNode>(planContext_.get(), indexes_, updatedProps_, filterNode.get(),
                                                    insertable_, depPropMap_, expCtx_.get(), &tagContext_);
  updateNode->addDependency(filterNode.get());

  auto resultNode = std::make_unique<UpdateResNode<VertexID>>(planContext_.get(), updateNode.get(), getReturnPropsExp(),
                                                              expCtx_.get(), result);
...
cpp2::ErrorCode UpdateVertexProcessor::buildTagContext(const cpp2::UpdateVertexRequest& req) {
  ...
  for (auto& prop : updatedProps_) {
    SourcePropertyExpression sourcePropExp(new std::string(tagName), new std::string(prop.get_name()));
  ...
  if (expCtx_ == nullptr) {
    expCtx_ = std::make_unique<StorageExpressionContext>(spaceVidLen_, isIntId_, planContext_->tagName_,
                                                         planContext_->tagSchema_, false);
  }
  return cpp2::ErrorCode::SUCCEEDED;
}
