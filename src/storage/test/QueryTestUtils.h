...
class QueryTestUtils {
 public:
  static bool mockVertexData(storage::StorageEnv* env, int32_t totalParts, bool enableIndex = false,
                             bool indexWithProp = true, bool schemaWithProp = true) {
    ...
    for (const auto& vertex : vertices) {
      ...
      env->kvstore_->asyncMultiPut(spaceId, partId, std::move(data), [&](kvstore::ResultCode code) {
  ...
  static bool mockEdgeData(storage::StorageEnv* env, int32_t totalParts, bool enableIndex = false,
                           bool indexWithProp = true, bool schemaWithProp = true, int vers = 1) {
    GraphSpaceID spaceId = 1;
    auto status = env->schemaMan_->getSpaceVidLen(spaceId);
    if (!status.ok()) {
      LOG(ERROR) << "Get space vid length failed";
    ...
    for (const auto& edge : edges) {
      ...
      for (int i = 0; i < vers; i++) {
        auto key = NebulaKeyUtils::edgeKey(spaceVidLen, partId, edge.srcId_, edge.type_, edge.rank_, edge.dstId_);
      ...
      if (enableIndex) {
        if (edge.type_ == 102 || edge.type_ == 101) {
          ...
          encodeEdgeIndex(spaceVidLen, partId, edge.srcId_, edge.dstId_, edge.rank_, indexID, values, colNum, data);
        }
      }
      env->kvstore_->asyncMultiPut(spaceId, partId, std::move(data), [&](kvstore::ResultCode code) {
        EXPECT_EQ(code, kvstore::ResultCode::SUCCEEDED);
  ...
  static bool mockBenchEdgeData(storage::StorageEnv* env, int32_t totalParts, SchemaVer schemaVerCount,
    ...
    for (const auto& entry : edges) {
      ...
      for (const auto& edge : entry.second) {
        auto key = NebulaKeyUtils::edgeKey(spaceVidLen, partId, edge.srcId_, edge.type_, edge.rank_, edge.dstId_, 0L);
  ...
  static bool encode(const meta::NebulaSchemaProvider* schema, const std::string& key, const std::vector<Value>& props,
                     std::vector<kvstore::KV>& data) {
  ...
  static void encodeTagIndex(size_t spaceVidLen, PartitionID partId, VertexID vId, IndexID indexId,
                             const std::vector<Value>& values, int32_t count, std::vector<kvstore::KV>& data) {
    std::string row;
  ...
  static void encodeEdgeIndex(size_t spaceVidLen, PartitionID partId, VertexID srcId, VertexID dstId, EdgeRanking rank,
                              IndexID indexId, const std::vector<Value>& values, int32_t count,
                              std::vector<kvstore::KV>& data) {
    std::string row;
    for (auto i = 0; i < count; i++) {
      auto v = values[i];
    ...
    auto index = IndexKeyUtils::edgeIndexKey(spaceVidLen, partId, indexId, srcId, rank, dstId, std::move(row));
    data.emplace_back(std::move(index), "");
  }

  static cpp2::GetNeighborsRequest buildRequest(int32_t totalParts, const std::vector<VertexID> vertices,
  ...
  static void checkResponse(const nebula::DataSet& dataSet, const std::vector<VertexID>& vertices,
                            const std::vector<EdgeType>& over,
                            const std::vector<std::pair<TagID, std::vector<std::string>>>& tags,
                            const std::vector<std::pair<EdgeType, std::vector<std::string>>>& edges,
                            size_t expectRowCount, size_t expectColumnCount,
  ...
  static void checkResponse(const cpp2::LookupIndexResp& resp, const std::vector<std::string>& expectCols,
  ...
  static void checkColNames(const nebula::DataSet& dataSet,
                            const std::vector<std::pair<TagID, std::vector<std::string>>>& tags,
                            const std::vector<std::pair<EdgeType, std::vector<std::string>>>& edges) {
    ...
    for (size_t i = 0; i < edges.size(); i++) {
      ...
      expected.append(edges[i].first > 0 ? "+" : "-").append(folly::to<std::string>(std::abs(edges[i].first)));
  ...
  static void checkPlayer(const std::vector<std::string>& props, const mock::Player& player,
  ...
  static void checkTeam(const std::vector<std::string>& props, const std::string& team,
                        const std::vector<Value>& values) {
    // if no property specified, would return all props
    if (props.empty()) {
  ...
  static void checkOutServe(EdgeType edgeType, const std::vector<std::string>& props,
                            const std::vector<mock::Serve>& serves, const std::vector<Value>& values,
                            size_t teamNameIndex = 0, size_t startYearIndex = 1) {
    if (props.empty()) {
    ...
    auto iter = std::find_if(serves.begin(), serves.end(), [&](const auto& serve) {
      // find corresponding record by team name and start year
      return serve.teamName_ == values[teamNameIndex].getStr() && serve.startYear_ == values[startYearIndex].getInt();
    });
    ASSERT_TRUE(iter != serves.end());
    checkSomePropertyOfServe(edgeType, props, *iter, values);
  }

  static void checkInServe(EdgeType edgeType, const std::vector<std::string>& props,
                           const std::vector<mock::Serve>& serves, const std::vector<Value>& values,
                           size_t playerNameIndex = 0, size_t startYearIndex = 1) {
    if (props.empty()) {
      auto iter = std::find_if(serves.begin(), serves.end(), [&](const auto& serve) {
  ...
  static void checkAllPropertyOfServe(EdgeType edgeType, const mock::Serve& serve, const std::vector<Value>& values) {
    UNUSED(edgeType);
    // property in value
  ...
  static void checkSomePropertyOfServe(EdgeType edgeType, const std::vector<std::string>& props,
                                       const mock::Serve& serve, const std::vector<Value>& values) {
    for (size_t i = 0; i < props.size(); i++) {
      if (props[i] == kSrc) {
        if (edgeType > 0) {
  ...
  static void checkTeammate(EdgeType edgeType, const std::vector<std::string>& props,
                            const std::vector<Value>& values) {
    if (props.empty()) {
  ...
  static void checkRowProps(const nebula::Row& row, const std::vector<std::string> colNames,
                            const std::vector<std::pair<TagID, std::vector<std::string>>>& tags,
                            const std::vector<std::pair<EdgeType, std::vector<std::string>>>& edges) {
    auto vId = row.values[0].getStr();
    ...
    for (size_t i = 2; i < colNames.size() - 1; i++) {
      ...
      switch (entryId) {
        case 1: {
          // tag player
          auto iter = std::find_if(mock::MockData::players_.begin(), mock::MockData::players_.end(),
        ...
        case -102: {
          auto iter =
              std::find_if(mock::MockData::teammates_.begin(), mock::MockData::teammates_.end(),
                           [&](const auto& teammate) { return teammate.player1_ == vId || teammate.player2_ == vId; });
  ...
  static const mock::Teammate& findTeammate(const std::string& player1, const std::string& player2) {
    auto iter = std::find_if(
        mock::MockData::teammates_.begin(), mock::MockData::teammates_.end(),
        [&](const auto& teammate) { return teammate.player1_ == player1 && teammate.player2_ == player2; });
    if (iter != mock::MockData::teammates_.end()) {
      return *iter;
    }
    iter = std::find_if(
        mock::MockData::teammates_.begin(), mock::MockData::teammates_.end(),
        [&](const auto& teammate) { return teammate.player1_ == player2 && teammate.player2_ == player1; });
    if (iter == mock::MockData::teammates_.end()) {
  ...
  static std::vector<std::string> findExpectProps(
      int32_t entryId, const std::vector<std::pair<TagID, std::vector<std::string>>>& tags,
      const std::vector<std::pair<EdgeType, std::vector<std::string>>>& edges) {
    auto propIter = std::find_if(tags.begin(), tags.end(), [&](const auto& entry) { return entry.first == entryId; });
    if (propIter != tags.end()) {
      return propIter->second;
    }
    propIter = std::find_if(edges.begin(), edges.end(), [&](const auto& entry) { return entry.first == entryId; });
