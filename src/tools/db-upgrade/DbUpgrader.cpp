...
Status UpgraderSpace::init(meta::MetaClient* mclient, meta::ServerBasedSchemaManager* sMan, meta::IndexManager* iMan,
                           const std::string& srcPath, const std::string& dstPath, const std::string& entry) {
  metaClient_ = mclient;
  schemaMan_ = sMan;
  indexMan_ = iMan;
  srcPath_ = srcPath;
...
Status UpgraderSpace::initSpace(const std::string& sId) {
  ...
  readEngine_.reset(new nebula::kvstore::RocksEngine(spaceId_, spaceVidLen_, srcPath_, nullptr, nullptr, true));
  writeEngine_.reset(new nebula::kvstore::RocksEngine(spaceId_, spaceVidLen_, dstPath_));

  parts_.clear();
  parts_ = readEngine_->allParts();
  LOG(INFO) << "Src data path: " << srcPath_ << " space id " << spaceId_ << " has " << parts_.size() << " parts";
...
Status UpgraderSpace::buildSchemaAndIndex() {
  ...
  for (auto& edge : edgeSchemas_) {
    ...
    LOG(INFO) << "Edgetype " << edgetype << " has " << edgeFieldName_[edgetype].size() << " fields!";
  }
  ...
  for (auto& edgeindexes : edgeIndexes_) {
    LOG(INFO) << "EdgeType " << edgeindexes.first << " has " << edgeindexes.second.size() << " indexes";
  }
  return Status::OK();
}
...
bool UpgraderSpace::isValidVidLen(VertexID srcVId, VertexID dstVId) {
  if (!NebulaKeyUtils::isValidVidLen(spaceVidLen_, srcVId, dstVId)) {
    LOG(ERROR) << "Vertex id length is illegal, expect: " << spaceVidLen_ << " result: " << srcVId << " " << dstVId;
    return false;
  }
  return true;
}
...
void UpgraderSpace::runPartV1() {
  ...
  if (auto pId = partQueue_.try_take_for(take_dura)) {
    ...
    LOG(INFO) << "Start to handle vertex/edge/index data in space id " << spaceId_ << " part id " << partId;
    ...
    if (retCode != kvstore::ResultCode::SUCCEEDED) {
      ...
      LOG(ERROR) << "Handle vertex/edge/index data in space id " << spaceId_ << " part id " << partId << " failed";

      auto unFinishedPart = --unFinishedPart_;
      if (unFinishedPart == 0) {
        // all parts has finished
        LOG(INFO) << "Handle last part: " << partId << " vertex/edge/index data in space id " << spaceId_
                  << " finished";
      } else {
    ...
    while (iter && iter->valid()) {
      ...
      if (NebulaKeyUtilsV1::isVertex(key)) {
        ...
        if (svId == lastSrcVertexId && edgetype == lastEdgeType && ranking == lastRank && dvId == lastDstVertexId) {
          // Multi version
          iter->next();
          continue;
        }
        ...
        // Generate 2.0 key
        auto newKey = NebulaKeyUtils::edgeKey(spaceVidLen_, partId, strsvId, edgetype, ranking, strdvId);
        auto val = iter->val();
        auto reader = RowReaderWrapper::getEdgePropReader(schemaMan_, spaceId_, std::abs(edgetype), val);
        if (!reader) {
        ...
        // Generate 2.0 value and index records
        encodeEdgeValue(partId, reader.get(), newEdgeSchema, newKey, strsvId, edgetype, ranking, strdvId, data);
    ...
    data.clear();
    LOG(INFO) << "Handle vertex/edge/index data in space id " << spaceId_ << " part id " << partId << " finished";

    auto unFinishedPart = --unFinishedPart_;
    if (unFinishedPart == 0) {
      // all parts has finished
      LOG(INFO) << "Handle last part: " << partId << " vertex/edge/index data in space id " << spaceId_ << " finished";
    } else {
      pool_->add(std::bind(&UpgraderSpace::runPartV1, this));
    }
  } else {
  ...
  if (auto pId = partQueue_.try_take_for(take_dura)) {
    ...
    while (iter && iter->valid()) {
      ...
      if (NebulaKeyUtilsV2::isVertex(spaceVidLen_, key)) {
        ...
        // Generate 2.0 value and index records
        encodeEdgeValue(partId, reader.get(), newEdgeSchema, newKey, svId, edgetype, ranking, dvId, data);
        lastSrcVertexId = svId;
        lastEdgeType = edgetype;
        lastRank = ranking;
        lastDstVertexId = dvId;
    ...
    data.clear();
    LOG(INFO) << "Handle vertex/edge/index data in space id " << spaceId_ << " part id " << partId << " succeed";
...
void UpgraderSpace::encodeVertexValue(PartitionID partId, RowReader* reader, const meta::NebulaSchemaProvider* schema,
                                      std::string& newkey, VertexID& strVid, TagID tagId,
                                      std::vector<kvstore::KV>& data) {
  ...
  if (it != tagIndexes_.end()) {
    ...
    if (nReader == nullptr) {
      LOG(ERROR) << "Bad format row: space id " << spaceId_ << " tag id " << tagId << " vertex id " << strVid;
      return;
    }
...
WriteResult UpgraderSpace::convertValue(const meta::NebulaSchemaProvider* nSchema,
                                        const meta::SchemaProviderIf* oSchema, std::string& name, Value& val) {
...
std::string UpgraderSpace::encodeRowVal(const RowReader* reader, const meta::NebulaSchemaProvider* schema,
...
std::string UpgraderSpace::indexVertexKey(PartitionID partId, VertexID& vId, RowReader* reader,
                                          std::shared_ptr<nebula::meta::cpp2::IndexItem> index) {
  auto values = IndexKeyUtils::collectIndexValues(reader, index->get_fields());
  if (!values.ok()) {
    return "";
  }
  return IndexKeyUtils::vertexIndexKey(spaceVidLen_, partId, index->get_index_id(), vId, std::move(values).value());
}

void UpgraderSpace::encodeEdgeValue(PartitionID partId, RowReader* reader, const meta::NebulaSchemaProvider* schema,
                                    std::string& newkey, VertexID& svId, EdgeType type, EdgeRanking rank,
                                    VertexID& dstId, std::vector<kvstore::KV>& data) {
  // Get all returned field name
  ...
  if (it != edgeIndexes_.end()) {
    ...
    if (nReader == nullptr) {
      LOG(ERROR) << "Bad format row: space id " << spaceId_ << " edgetype " << type << " srcVertexId " << svId
                 << " rank " << rank << " dstVertexId " << dstId;
      return;
    }
    for (auto& index : it->second) {
...
std::string UpgraderSpace::indexEdgeKey(PartitionID partId, RowReader* reader, VertexID& svId, EdgeRanking rank,
                                        VertexID& dstId, std::shared_ptr<nebula::meta::cpp2::IndexItem> index) {
  auto values = IndexKeyUtils::collectIndexValues(reader, index->get_fields());
  if (!values.ok()) {
    return "";
  }
  return IndexKeyUtils::edgeIndexKey(spaceVidLen_, partId, index->get_index_id(), svId, rank, dstId,
...
bool UpgraderSpace::copyWal() {
  ...
  auto srcPath = folly::stringPrintf("%s/%s/%s/%s", srcPath_.c_str(), "nebula", entry_.c_str(), "wal");
  ...
  for (size_t i = 0; i < partDirs.size(); i++) {
    ...
    auto files =
        fs::FileUtils::listAllFilesInDir(folly::stringPrintf("%s/%s", srcPath.c_str(), partDirs[i].c_str()).c_str());
    ...
    for (const auto& file : files) {
      std::fstream srcF(folly::stringPrintf("%s/%s/%s", srcPath.c_str(), partDirs[i].c_str(), file.c_str()),
...
Status DbUpgrader::init(meta::MetaClient* mclient, meta::ServerBasedSchemaManager* sMan, meta::IndexManager* iMan,
...
void DbUpgrader::run() {
  LOG(INFO) << "Upgrade from path " << srcPath_ << " to path " << dstPath_ << " in DbUpgrader run begin";
  ...
  for (auto& entry : subDirs_) {
    ...
    if (!ret.ok()) {
      LOG(WARNING) << "Upgrade from path " << srcPath_ << " space id " << entry << " to path " << dstPath_
                   << " init failed";
  ...
  auto spaceConcurrency = std::min(static_cast<size_t>(FLAGS_max_concurrent_spaces), upgraderSpaces.size());
  ...
  LOG(INFO) << "Upgrade from path " << srcPath_ << " to path " << dstPath_ << " in DbUpgrader run end";
}
...
void DbUpgrader::doSpace() {
  ...
  if (auto UpSpace = spaceQueue_.try_take_for(take_dura)) {
    ...
    LOG(INFO) << "Upgrade from path " << upgraderSpaceIter->srcPath_ << " space id " << upgraderSpaceIter->entry_
              << " to path " << upgraderSpaceIter->dstPath_ << " begin";
    ...
    if (unFinishedSpace == 0) {
      ...
      LOG(INFO) << "Upgrade last space: " << upgraderSpaceIter->entry_ << " from " << upgraderSpaceIter->srcPath_
                << " to path " << upgraderSpaceIter->dstPath_ << " end";
    } else {
      pool_->add(std::bind(&DbUpgrader::doSpace, this));
    }
  } else {
