...
void GetEdgeProcessor::process(const cpp2::GetEdgeReq& req) {
  ...
  if (req.get_version() < 0) {
    ...
    if (!ret.ok()) {
      LOG(ERROR) << "Get Edge SpaceID: " << req.get_space_id() << ", edgeName: " << req.get_edge_name() << ", version "
                 << req.get_version() << " not found";
    ...
    auto edgeKey = MetaServiceUtils::schemaEdgeKey(req.get_space_id(), edgeType, req.get_version());
  ...
  VLOG(3) << "Get Edge SpaceID: " << req.get_space_id() << ", edgeName: " << req.get_edge_name() << ", version "
          << req.get_version();
