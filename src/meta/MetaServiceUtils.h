...
class MetaServiceUtils final {
 public:
  ...
  static std::string rebuildIndexStatus(GraphSpaceID space, char type, const std::string& indexName);
  ...
  static cpp2::ErrorCode alterColumnDefs(std::vector<cpp2::ColumnDef>& cols, cpp2::SchemaProp& prop,
                                         const cpp2::ColumnDef col, const cpp2::AlterSchemaOp op);

  static cpp2::ErrorCode alterSchemaProp(std::vector<cpp2::ColumnDef>& cols, cpp2::SchemaProp& schemaProp,
  ...
  static std::string balanceTaskKey(BalanceID balanceId, GraphSpaceID spaceId, PartitionID partId, HostAddr src,
                                    HostAddr dst);

  static std::string balanceTaskVal(BalanceTaskStatus status, BalanceTaskResult retult, int64_t startTime,
                                    int64_t endTime);
  ...
  static std::string fulltextServiceVal(cpp2::FTServiceType type, const std::vector<cpp2::FTClient>& clients);
  ...
  static bool replaceHostInPartition(kvstore::KVStore* kvstore, const HostAddr& ipv4From, const HostAddr& ipv4To,
                                     bool direct = false);
  static bool replaceHostInZone(kvstore::KVStore* kvstore, const HostAddr& ipv4From, const HostAddr& ipv4To,
                                bool direct = false);
  // backup
  ...
  static folly::Optional<std::vector<std::string>> backup(kvstore::KVStore* kvstore,
                                                          const std::unordered_set<GraphSpaceID>& spaces,
