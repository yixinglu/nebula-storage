...
class IndexEdgeNode final : public RelNode<T> {
 public:
  ...
  IndexEdgeNode(PlanContext* planCtx, IndexScanNode<T>* indexScanNode,
                const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>& schemas,
                const std::string& schemaName)
      : planContext_(planCtx), indexScanNode_(indexScanNode), schemas_(schemas), schemaName_(schemaName) {}
  ...
  kvstore::ResultCode execute(PartitionID partId) override {
    ...
    for (const auto& edge : edges) {
      auto prefix = NebulaKeyUtils::edgePrefix(planContext_->vIdLen_, partId, (*edge.src_ref()).getStr(),
                                               planContext_->edgeType_, edge.get_ranking(), (*edge.dst_ref()).getStr());
      std::unique_ptr<kvstore::KVIterator> eIter;
      ret = planContext_->env_->kvstore_->prefix(planContext_->spaceId_, partId, prefix, &eIter);
  ...
  const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>& getSchemas() { return schemas_; }
