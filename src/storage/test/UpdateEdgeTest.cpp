...
static bool mockEdgeData(storage::StorageEnv* env, int32_t totalParts, int32_t spaceVidLen) {
  ...
  for (const auto& part : edgesPart) {
    ...
    for (const auto& edge : part.second) {
      ...
      auto key =
          NebulaKeyUtils::edgeKey(spaceVidLen, part.first, edge.srcId_, edge.type_, edge.rank_, edge.dstId_, version);
    ...
    env->kvstore_->asyncMultiPut(spaceId, part.first, std::move(data), [&](kvstore::ResultCode code) {
...
TEST(UpdateEdgeTest, No_Filter_Test) {
  ...
  EXPECT_TRUE(mockEdgeData(env, parts, spaceVidLen));
  {
    ...
    auto edgeReader = RowReaderWrapper::getEdgePropReader(env->schemaMan_, spaceId, std::abs(edgeType), iter->val());
...
TEST(UpdateEdgeTest, Filter_Yield_Test) {
  ...
  auto* left = new RelationalExpression(Expression::Kind::kRelEQ, srcExp1, priExp1);
  ...
  auto* right = new RelationalExpression(Expression::Kind::kRelEQ, srcExp2, priExp2);
  // left AND right is ture
  auto logExp = std::make_unique<LogicalExpression>(Expression::Kind::kLogicalAnd, left, right);
  ...
  auto reader = RowReaderWrapper::getEdgePropReader(env->schemaMan_, spaceId, std::abs(edgeType), iter->val());
