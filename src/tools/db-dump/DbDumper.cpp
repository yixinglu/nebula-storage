...
Status DbDumper::openDb() {
  ...
  if (spaceFound == subDirs.end()) {
    return Status::Error("Space '%s' not found in directory '%s'.", FLAGS_space_name.c_str(), FLAGS_db_path.c_str());
  }
  ...
  if (!status.ok()) {
    return Status::Error("Unable to open database '%s' for reading: '%s'", path.c_str(), status.ToString().c_str());
  }
  db_.reset(dbPtr);
  return Status::OK();
}
...
void DbDumper::run() {
  ...
  uint32_t bitmap = (parts_.empty() ? 0 : 1 << 3) | (vids_.empty() ? 0 : 1 << 2) | (tagIds_.empty() ? 0 : 1 << 1) |
...
void DbDumper::iterates(kvstore::RocksPrefixIter* it) {
  for (; it->valid(); it->next()) {
    ...
    if (NebulaKeyUtils::isVertex(spaceVidLen_, key)) {
      ...
      if (FLAGS_mode == "scan") {
        printEdgeKey(key);
        auto reader = RowReaderWrapper::getEdgePropReader(schemaMng_.get(), spaceId_, edgeType, value);
        if (!reader) {
...
inline void DbDumper::printEdgeKey(const folly::StringPiece& key) {
  ...
  std::cout << "[edge] key: " << part << ", " << src << ", " << getEdgeName(edgeType) << ", " << rank << ", " << dst;
}
