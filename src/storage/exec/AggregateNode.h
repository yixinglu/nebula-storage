...
class AggregateNode : public IterateNode<T> {
 public:
  ...
  explicit AggregateNode(nebula::DataSet* resultSet) : resultSet_(resultSet) {}

  AggregateNode(PlanContext* planCtx, IterateNode<T>* upstream, EdgeContext* edgeContext)
      : IterateNode<T>(upstream), planContext_(planCtx), edgeContext_(edgeContext) {}
 ...
 private:
  VertexIDSlice srcId() const { return NebulaKeyUtils::getSrcId(planContext_->vIdLen_, this->key()); }

  EdgeType edgeType() const { return NebulaKeyUtils::getEdgeType(planContext_->vIdLen_, this->key()); }

  EdgeRanking edgeRank() const { return NebulaKeyUtils::getRank(planContext_->vIdLen_, this->key()); }

  VertexIDSlice dstId() const { return NebulaKeyUtils::getDstId(planContext_->vIdLen_, this->key()); }
  ...
  kvstore::ResultCode collectEdgeStats(folly::StringPiece key, RowReader* reader,
                                       const std::vector<PropContext>* props) {
    for (const auto& prop : *props) {
      if (prop.hasStat_) {
        for (const auto statIndex : prop.statIndex_) {
          VLOG(2) << "Collect stat prop " << prop.name_;
          auto value = QueryUtils::readEdgeProp(key, planContext_->vIdLen_, planContext_->isIntId_, reader, prop);
          if (!value.ok()) {
