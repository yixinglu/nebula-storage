...
class IndexVertexNode final : public RelNode<T> {
 public:
  ...
  IndexVertexNode(PlanContext* planCtx, VertexCache* vertexCache, IndexScanNode<T>* indexScanNode,
  ...
  kvstore::ResultCode execute(PartitionID partId) override {
    ...
    for (const auto& vId : vids) {
      ...
      if (FLAGS_enable_vertex_cache && vertexCache_ != nullptr) {
        ...
        if (result.ok()) {
          auto vertexKey = NebulaKeyUtils::vertexKey(planContext_->vIdLen_, partId, vId, planContext_->tagId_);
      ...
      auto prefix = NebulaKeyUtils::vertexPrefix(planContext_->vIdLen_, partId, vId, planContext_->tagId_);
      ret = planContext_->env_->kvstore_->prefix(planContext_->spaceId_, partId, prefix, &vIter);
