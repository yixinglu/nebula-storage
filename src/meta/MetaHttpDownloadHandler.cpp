...
void MetaHttpDownloadHandler::init(nebula::kvstore::KVStore *kvstore, nebula::hdfs::HdfsHelper *helper,
...
void MetaHttpDownloadHandler::onError(ProxygenError error) noexcept {
  LOG(ERROR) << "Web Service MetaHttpDownloadHandler got error : " << proxygen::getErrorString(error);
}

bool MetaHttpDownloadHandler::dispatchSSTFiles(const std::string &hdfsHost, int hdfsPort, const std::string &hdfsPath) {
  ...
  for (auto &pair : hostPartition) {
    ...
    auto dispatcher = [hostaddr, hdfsHost, hdfsPort, hdfsPath, partsStr, this]() {
      ...
      std::string url = folly::stringPrintf(tmp, hostaddr.c_str(), FLAGS_ws_storage_http_port, "download",
                                            hdfsHost.c_str(), hdfsPort, hdfsPath.c_str(), partsStr.c_str(), spaceID_);
