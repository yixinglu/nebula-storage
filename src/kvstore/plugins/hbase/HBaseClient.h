...
class HBaseClient final {
 public:
  ...
  std::pair<ResultCode, std::vector<Status>> multiGet(const std::string& tableName,
  ...
  ResultCode multiPut(const std::string& tableName, std::vector<std::pair<std::string, std::vector<KV>>>& dataList);

  ResultCode range(const std::string& tableName, const std::string& start, const std::string& end,
                   std::vector<std::pair<std::string, KVMap>>& dataList);
