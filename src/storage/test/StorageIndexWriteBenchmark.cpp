...
bool processVertices(kvstore::KVStore* kv, meta::SchemaManager* schemaMan, meta::IndexManager* indexMan,
                     VertexID& vId) {
...
void insertVertices(bool withoutIndex) {
  ...
  BENCHMARK_SUSPEND {
    std::string rootPath;
    if (withoutIndex) {
      rootPath = folly::stringPrintf("%s/%s", FLAGS_root_data_path.c_str(), "withoutIndex");
    } else {
      rootPath = folly::stringPrintf("%s/%s", FLAGS_root_data_path.c_str(), "attachIndex");
    }
  ...
  BENCHMARK_SUSPEND {
    if (withoutIndex) {
      ...
      if (cnt != FLAGS_total_vertices_size) {
        LOG(ERROR) << "Vertices insert error , expected : " << FLAGS_total_vertices_size << "actual : " << cnt;
      }
    } else {
      {
        auto prefix = NebulaKeyUtils::prefix(0);
      ...
      }
      {
        ...
        if (cnt != FLAGS_total_vertices_size) {
          LOG(ERROR) << "Indexes number error , expected : " << FLAGS_total_vertices_size << "actual : " << cnt;
          return;
        }
      }
    }
...
void insertUnmatchIndex() {
  ...
  BENCHMARK_SUSPEND{{auto prefix = NebulaKeyUtils::prefix(0);
  std::unique_ptr<kvstore::KVIterator> iter;
  auto status = kv->prefix(0, 0, prefix, &iter);
  if (status != kvstore::ResultCode::SUCCEEDED) {
    LOG(ERROR) << "Index scan error";
...
void insertDupVertices() {
  ...
  VertexID vId = 0;
  BENCHMARK_SUSPEND {
    auto rootPath = folly::stringPrintf("%s/%s", FLAGS_root_data_path.c_str(), "duplicateIndex");
    kv = TestUtils::initKV(std::move(rootPath).c_str());
    schemaMan = TestUtils::mockSchemaMan();
    indexMan = TestUtils::mockIndexMan(0, 3001, 3002);
  };
...
void insertVerticesMultIndex() {
  ...
  BENCHMARK_SUSPEND {
    std::string rootPath;
    rootPath = folly::stringPrintf("%s/%s", FLAGS_root_data_path.c_str(), "multIndex");
  ...
  BENCHMARK_SUSPEND {
    ...
    }
    {
      ...
      if (cnt != FLAGS_total_vertices_size * 3) {
        LOG(ERROR) << "Indexes number error , expected : " << FLAGS_total_vertices_size * 3 << "actual : " << cnt;
        return;
      }
    }
  }
