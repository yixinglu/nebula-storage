...
class CreateEdgeIndexProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static CreateEdgeIndexProcessor* instance(kvstore::KVStore* kvstore) { return new CreateEdgeIndexProcessor(kvstore); }

  void process(const cpp2::CreateEdgeIndexReq& req);

 private:
  explicit CreateEdgeIndexProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
