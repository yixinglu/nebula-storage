...
DEFINE_string(method, "getNeighbors",
...
DEFINE_string(tag_name, "test_tag",
              "Specify the tag name, "
              "all property of the tag must be string");
DEFINE_string(edge_name, "test_edge",
...
class Perf {
 public:
  ...
  void runInternal() {
    while (finishedRequests_ < FLAGS_totalReqs) {
      ...
      PLOG_EVERY_N(INFO, 2000) << "Progress " << finishedRequests_ / static_cast<double>(FLAGS_totalReqs) * 100 << "%"
                               << ", qps=" << qps_.rate(0)
 ...
 private:
  std::vector<VertexID> randomVertices() {
    return {std::to_string(FLAGS_min_vertex_id + folly::Random::rand32(FLAGS_max_vertex_id - FLAGS_min_vertex_id))};
  }

  std::vector<Value> randomEdges() {
    std::vector<Value> values;
    auto src = folly::Random::rand32(FLAGS_max_vertex_id - FLAGS_min_vertex_id) + FLAGS_min_vertex_id;
  ...
  void getNeighborsTask() {
    ...
    for (auto i = 0; i < tokens; i++) {
      ...
      graphStorageClient_
          ->getNeighbors(spaceId_, colNames, vertices, {edgeType_}, edgeDire, &statProps, &vProps, &eProps, nullptr)
          .via(evb)
