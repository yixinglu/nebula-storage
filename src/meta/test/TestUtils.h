...
class TestUtils {
 public:
  static cpp2::ColumnDef columnDef(int32_t index, cpp2::PropertyType type, Value defaultValue = Value(),
                                   bool isNull = true, int16_t typeLen = 0) {
  ...
  static void setupHB(kvstore::KVStore* kv, const std::vector<HostAddr>& hosts, cpp2::HostRole role,
  ...
  static void assembleGroupAndZone(kvstore::KVStore* kv, const ZoneInfo& zoneInfo, const GroupInfo& groupInfo) {
    ...
    for (auto iter = zoneInfo.begin(); iter != zoneInfo.end(); iter++) {
      ...
      data.emplace_back(MetaServiceUtils::zoneKey(iter->first), MetaServiceUtils::zoneVal(iter->second));
      id += 1;
    }
    ...
    for (auto iter = groupInfo.begin(); iter != groupInfo.end(); iter++) {
      ...
      data.emplace_back(MetaServiceUtils::groupKey(iter->first), MetaServiceUtils::groupVal(iter->second));
      id += 1;
    }
  ...
  static bool modifyHostAboutZone(kvstore::KVStore* kv, const std::string& zoneName, HostAddr host, bool isAdd) {
    ...
    bool ret = false;
    folly::Baton<true, std::atomic> baton;
    kv->asyncMultiPut(0, 0, std::move(data), [&](kvstore::ResultCode resultCode) {
  ...
  static bool modifyZoneAboutGroup(kvstore::KVStore* kv, const std::string& groupName, const std::string& zoneName,
                                   bool isAdd) {
  ...
  static void assembleSpace(kvstore::KVStore* kv, GraphSpaceID id, int32_t partitionNum, int32_t replica = 1,
                            int32_t totalHost = 1) {
  ...
  static void mockTag(kvstore::KVStore* kv, int32_t tagNum, SchemaVer version = 0, bool nullable = false) {
    ...
    for (auto t = 0; t < tagNum; t++) {
      ...
      for (auto i = 0; i < 2; i++) {
        ...
        (*column.type_ref()).set_type(i < 1 ? PropertyType::INT64 : PropertyType::FIXED_STRING);
        if (nullable) {
      ...
      tags.emplace_back(MetaServiceUtils::schemaTagKey(1, tagId, ver++), MetaServiceUtils::schemaVal(tagName, srcsch));
    }
    folly::Baton<true, std::atomic> baton;
    kv->asyncMultiPut(0, 0, std::move(tags), [&](kvstore::ResultCode code) {
      ASSERT_EQ(kvstore::ResultCode::SUCCEEDED, code);
      baton.post();
    });
    baton.wait();
  ...
  static void mockTagIndex(kvstore::KVStore* kv, TagID tagID, std::string tagName, IndexID indexID,
                           std::string indexName, std::vector<cpp2::ColumnDef> columns) {
  ...
  static void mockEdgeIndex(kvstore::KVStore* kv, EdgeType edgeType, std::string edgeName, IndexID indexID,
                            std::string indexName, std::vector<cpp2::ColumnDef> columns) {
    GraphSpaceID space = 1;
    cpp2::IndexItem item;
    item.set_index_id(indexID);
  ...
  static void mockEdge(kvstore::KVStore* kv, int32_t edgeNum, SchemaVer version = 0, bool nullable = false) {
  ...
  static cpp2::Schema mockSchemaWithAllType() {
    ...
    cols.emplace_back(TestUtils::columnDef(8, PropertyType::FIXED_STRING, Value("longlongstring"), false, 10));
  ...
  static bool verifySchema(cpp2::Schema& result, cpp2::Schema& expected) {
    ...
    std::sort(resultColumns.begin(), resultColumns.end(),
              [](cpp2::ColumnDef col0, cpp2::ColumnDef col1) { return col0.get_name() < col1.get_name(); });
  ...
  static bool verifyMap(std::map<std::string, std::vector<std::string>>& result,
    ...
    for (auto resultIter = result.begin(), expectedIter = expected.begin();
         resultIter != result.end() && expectedIter != expected.end(); resultIter++, expectedIter++) {
      if (resultIter->first != expectedIter->first && !verifyResult(resultIter->second, expectedIter->second)) {
        return false;
      }
    }
  ...
  static bool verifyResult(std::vector<cpp2::ColumnDef>& result, std::vector<cpp2::ColumnDef>& expected) {
    if (result.size() != expected.size()) {
      return false;
    }

    std::sort(result.begin(), result.end(),
              [](cpp2::ColumnDef& x, cpp2::ColumnDef& y) { return x.get_name().compare(y.get_name()); });
    std::sort(expected.begin(), expected.end(),
              [](cpp2::ColumnDef& x, cpp2::ColumnDef& y) { return x.get_name().compare(y.get_name()); });
    int32_t size = result.size();
    for (int32_t i = 0; i < size; i++) {
      if (result[i].get_name() != expected[i].get_name() || result[i].get_type() != expected[i].get_type()) {
        return false;
      }
      if (result[i].nullable_ref().has_value() != expected[i].nullable_ref().has_value() ||
          (result[i].nullable_ref().has_value() && *result[i].nullable_ref() != *expected[i].nullable_ref())) {
        return false;
      }
    }
    return true;
