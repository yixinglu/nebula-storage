...
class ListTagIndexesProcessor : public BaseProcessor<cpp2::ListTagIndexesResp> {
 public:
  static ListTagIndexesProcessor* instance(kvstore::KVStore* kvstore) { return new ListTagIndexesProcessor(kvstore); }

  void process(const cpp2::ListTagIndexesReq& req);

 private:
  explicit ListTagIndexesProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListTagIndexesResp>(kvstore) {}
};
