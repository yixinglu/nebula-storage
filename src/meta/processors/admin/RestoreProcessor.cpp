...
void RestoreProcessor::process(const cpp2::RestoreMetaReq& req) {
  ...
  if (!replaceHosts.empty()) {
    for (auto h : replaceHosts) {
      auto result = MetaServiceUtils::replaceHostInPartition(kvstore_, h.get_from_host(), h.get_to_host(), true);
      if (!result) {
      ...
      result = MetaServiceUtils::replaceHostInZone(kvstore_, h.get_from_host(), h.get_to_host(), true);
      if (!result) {
