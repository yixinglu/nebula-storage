...
void copyData(kvstore::KVStore* kv, int32_t space, int32_t part, const std::string& keySrc, const std::string& keyDst) {
  std::string val;
...
struct JobCallBack {
  JobCallBack(JobManager* jobMgr, int job, int task, int n) : jobMgr_(jobMgr), jobId_(job), taskId_(task), n_(n) {}
...
TEST_F(GetStatisTest, StatisJob) {
  ...
  // Remove statis data.
  {
    ...
    kv_->asyncRemove(kDefaultSpaceId, kDefaultPartId, key, [&](nebula::kvstore::ResultCode code) {
...
TEST_F(GetStatisTest, MockSingleMachineTest) {
  ...
  for (const auto& entry : leaders) {
    ...
    auto ret = ActiveHostsMan::updateHostInfo(kv_.get(), entry.first,
                                              HostInfo(now, cpp2::HostRole::STORAGE, gitInfoSha()), &entry.second);
