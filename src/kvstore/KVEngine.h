...
class WriteBatch {
 public:
  ...
  virtual ResultCode removeRange(folly::StringPiece start, folly::StringPiece end) = 0;
};
...
class KVEngine {
 public:
  ...
  virtual ResultCode commitBatchWrite(std::unique_ptr<WriteBatch> batch, bool disableWAL = true, bool sync = false) = 0;
  ...
  virtual ResultCode range(const std::string& start, const std::string& end, std::unique_ptr<KVIterator>* iter) = 0;

  // Get all results with 'prefix' str as prefix.
  virtual ResultCode prefix(const std::string& prefix, std::unique_ptr<KVIterator>* iter) = 0;

  // Get all results with 'prefix' str as prefix starting form 'start'
  virtual ResultCode rangeWithPrefix(const std::string& start, const std::string& prefix,
  ...
  virtual ResultCode removeRange(const std::string& start, const std::string& end) = 0;
  ...
  // Set Config Option
  virtual ResultCode setOption(const std::string& configKey, const std::string& configValue) = 0;

  // Set DB Config Option
  virtual ResultCode setDBOption(const std::string& configKey, const std::string& configValue) = 0;
  ...
  // fo meta
  virtual ErrorOr<ResultCode, std::string> backupTable(const std::string& path, const std::string& tablePrefix,
