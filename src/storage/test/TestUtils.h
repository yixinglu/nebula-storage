...
void checkAddVerticesData(cpp2::AddVerticesRequest req, StorageEnv* env, int expectNum,
  ...
  for (auto& part : *req.parts_ref()) {
    ...
    for (auto& newVertex : newVertexVec) {
      ...
      for (auto& newTag : newTagVec) {
        ...
        EXPECT_EQ(kvstore::ResultCode::SUCCEEDED, env->kvstore_->prefix(spaceId, partId, prefix, &iter));
...
void checkVerticesData(int32_t spaceVidLen, GraphSpaceID spaceId,
                       std::unordered_map<PartitionID, std::vector<Value>> parts, StorageEnv* env, int expectNum) {
  int totalCount = 0;
  for (auto& part : parts) {
    auto partId = part.first;
...
void checkAddEdgesData(cpp2::AddEdgesRequest req, StorageEnv* env, int expectNum,
                       /* 0 not specify prop_names, 1 specify prop_names, 2 mix */
                       int mode = 0) {
  EXPECT_TRUE(mode == 0 || mode == 1 || mode == 2);
  auto spaceId = req.get_space_id();
  ...
  for (auto& part : *req.parts_ref()) {
    ...
    for (auto& newEdge : newEdgeVec) {
      ...
      auto prefix = NebulaKeyUtils::edgePrefix(spaceVidLen, partId, edgekey.get_src().getStr(), edgekey.get_edge_type(),
...
void checkEdgesData(int32_t spaceVidLen, GraphSpaceID spaceId,
                    std::unordered_map<PartitionID, std::vector<nebula::storage::cpp2::EdgeKey>> parts, StorageEnv* env,
                    int expectNum) {
  int totalCount = 0;
  for (auto& part : parts) {
    auto partId = part.first;
