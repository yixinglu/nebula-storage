...
TEST(ProcessorTest, ListSpecficHostsTest) {
  ...
  std::vector<cpp2::HostRole> roleVec{cpp2::HostRole::GRAPH, cpp2::HostRole::META, cpp2::HostRole::STORAGE};
  std::vector<std::string> gitInfoShaVec{"fakeGraphInfoSHA", "fakeMetaInfoSHA", "fakeStorageInfoSHA"};
  FLAGS_heartbeat_interval_secs = 1;
  std::unique_ptr<kvstore::KVStore> kv(MockCluster::initMetaKV(rootPath.path()));
...
TEST(ProcessorTest, ListPartsTest) {
  ...
  TestUtils::assembleSpace(kv.get(), 1, 9, 3, 3);
  {
    ...
    std::sort(parts.begin(), parts.end(),
              [](const auto& a, const auto& b) { return a.get_part_id() < b.get_part_id(); });
    ...
    for (auto& part : parts) {
      ...
      for (auto& peer : *part.peers_ref()) {
        auto it = std::find_if(hosts.begin(), hosts.end(),
                               [&](const auto& host) { return host.host == peer.host && host.port == peer.port; });
...
TEST(ProcessorTest, SpaceTest) {
  ...
  }
  {
    ...
    ASSERT_EQ(cpp2::PropertyType::FIXED_STRING, resp.get_item().get_properties().get_vid_type().get_type());
...
TEST(ProcessorTest, KVOperationTest) {
  ...
  }
  {
    ...
    for (auto i = 0; i < 10; i++) {
      pairs.emplace_back(std::make_pair(folly::stringPrintf("key_%d", i), folly::stringPrintf("value_%d", i)));
    }
...
TEST(ProcessorTest, ListOrGetTagsTest) {
  ...
  // Test GetTagProcessor with version
  {
    ...
    for (auto i = 0; i < 2; i++) {
      ...
      ASSERT_EQ((i < 1 ? PropertyType::INT64 : PropertyType::FIXED_STRING), cols[i].get_type().get_type());
    }
  }
...
TEST(ProcessorTest, DropTagTest) {
  ...
  // Check tag data has been deleted.
  {
    ...
    ret = kv.get()->get(kDefaultSpaceId, kDefaultPartId, std::move(MetaServiceUtils::indexTagKey(1, "tag_0")), &tagVal);
...
TEST(ProcessorTest, DropEdgeTest) {
  ...
  // Check edge data has been deleted.
  {
    ...
    kvstore::ResultCode ret;
    std::unique_ptr<kvstore::KVIterator> iter;
    ret = kv.get()->get(kDefaultSpaceId, kDefaultPartId, std::move(MetaServiceUtils::indexEdgeKey(1, "edge_0")),
                        &edgeVal);
    ASSERT_EQ(kvstore::ResultCode::ERR_KEY_NOT_FOUND, ret);
...
TEST(ProcessorTest, AlterTagTest) {
  ...
  // Verify alter result.
  {
    ...
    EXPECT_EQ(*schema.get_schema_prop().get_ttl_duration(), *tag.get_schema().get_schema_prop().get_ttl_duration());
    EXPECT_EQ(*schema.get_schema_prop().get_ttl_col(), *tag.get_schema().get_schema_prop().get_ttl_col());
  }
...
TEST(ProcessorTest, AlterEdgeTest) {
  ...
  // Verify alter result.
  {
    ...
    EXPECT_EQ(*schema.get_schema_prop().get_ttl_duration(), *edge.get_schema().get_schema_prop().get_ttl_duration());
    EXPECT_EQ(*schema.get_schema_prop().get_ttl_col(), *edge.get_schema().get_schema_prop().get_ttl_col());
  }
