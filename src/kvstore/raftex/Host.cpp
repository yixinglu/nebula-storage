...
DEFINE_uint32(max_appendlog_batch_size, 128, "The max number of logs in each appendLog request batch");
...
Host::Host(const HostAddr& addr, std::shared_ptr<RaftPart> part, bool isLearner)
    : part_(std::move(part)),
      ...
      idStr_(folly::stringPrintf("%s[Host: %s:%d] ", part_->idStr_.c_str(), addr_.host.c_str(), addr_.port)),
...
folly::Future<cpp2::AskForVoteResponse> Host::askForVote(const cpp2::AskForVoteRequest& req, folly::EventBase* eb) {
  {
  ...
  auto client = part_->clientMan_->client(addr_, eb, false, FLAGS_raft_heartbeat_interval_secs * 1000);
  return client->future_askForVote(req);
}

folly::Future<cpp2::AppendLogResponse> Host::appendLogs(folly::EventBase* eb, TermID term, LogID logId,
                                                        LogID committedLogId, TermID prevLogTerm, LogID prevLogId) {
  VLOG(3) << idStr_ << "Entering Host::appendLogs()";

  if (FLAGS_trace_raft) {
    LOG(INFO) << idStr_ << "Append logs to the host [term = " << term << ", logId = " << logId
              << ", committedLogId = " << committedLogId << ", lastLogTermSent = " << prevLogTerm
  ...
  std::shared_ptr<cpp2::AppendLogRequest> req;
  {
    ...
    if (UNLIKELY(lastLogIdSent_ == 0 && lastLogTermSent_ == 0)) {
      ...
      LOG(INFO) << idStr_ << "This is the first time to send the logs to this host"
                << ", lastLogIdSent = " << lastLogIdSent_ << ", lastLogTermSent = " << lastLogTermSent_;
    }
    if (prevLogTerm < lastLogTermSent_ || prevLogId < lastLogIdSent_) {
      LOG(INFO) << idStr_ << "We have sended this log, so go on from id " << lastLogIdSent_ << ", term "
                << lastLogTermSent_ << "; current prev log id " << prevLogId << ", current prev log term "
                << prevLogTerm;
    }
    logTermToSend_ = term;
    logIdToSend_ = logId;
...
void Host::appendLogsInternal(folly::EventBase* eb, std::shared_ptr<cpp2::AppendLogRequest> req) {
  sendAppendLogRequest(eb, std::move(req))
      ...
      .then([eb, self = shared_from_this()](folly::Try<cpp2::AppendLogResponse>&& t) {
        ...
        if (FLAGS_trace_raft) {
          LOG(INFO) << self->idStr_ << "AppendLogResponse "
                    << "code " << static_cast<int32_t>(resp.get_error_code()) << ", currTerm "
                    << resp.get_current_term() << ", lastLogId " << resp.get_last_log_id() << ", lastLogTerm "
                    << resp.get_last_log_term() << ", commitLogId " << resp.get_committed_log_id()
                    << ", lastLogIdSent_ " << self->lastLogIdSent_ << ", lastLogTermSent_ " << self->lastLogTermSent_;
        }
        switch (resp.get_error_code()) {
          case cpp2::ErrorCode::SUCCEEDED: {
            ...
            std::shared_ptr<cpp2::AppendLogRequest> newReq;
            {
              ...
              if (res != cpp2::ErrorCode::SUCCEEDED) {
                ...
                VLOG(1) << self->idStr_ << "We send nothing in the last request"
                ...
                if (self->lastLogIdSent_ < self->logIdToSend_) {
                  ...
                  VLOG(2) << self->idStr_ << "Fulfill the promise, size = " << self->promise_.size();
                  // Fulfill the promise
          ...
          case cpp2::ErrorCode::E_LOG_STALE: {
            ...
            std::shared_ptr<cpp2::AppendLogRequest> newReq;
            {
              ...
              if (res != cpp2::ErrorCode::SUCCEEDED) {
                ...
                VLOG(1) << self->idStr_ << "It means the request has been received by follower";
                self->lastLogIdSent_ = self->logIdToSend_ - 1;
                self->lastLogTermSent_ = resp.get_last_log_term();
                self->followerCommittedLogId_ = resp.get_committed_log_id();
                cpp2::AppendLogResponse r;
          ...
          default: {
            LOG_EVERY_N(ERROR, 100) << self->idStr_ << "Failed to append logs to the host (Err: "
...
std::shared_ptr<cpp2::AppendLogRequest> Host::prepareAppendLogRequest() {
  ...
  VLOG(2) << idStr_ << "Prepare AppendLogs request from Log " << lastLogIdSent_ + 1 << " to " << logIdToSend_;
  if (lastLogIdSent_ + 1 > part_->wal()->lastLogId()) {
    LOG(INFO) << idStr_ << "My lastLogId in wal is " << part_->wal()->lastLogId() << ", but you are seeking "
              << lastLogIdSent_ + 1 << ", so i have nothing to send.";
    return req;
  }
  ...
  if (it->valid()) {
    ...
    for (size_t cnt = 0; it->valid() && it->logTerm() == term && cnt < FLAGS_max_appendlog_batch_size; ++(*it), ++cnt) {
      cpp2::LogEntry le;
    ...
    if (!sendingSnapshot_) {
      LOG(INFO) << idStr_ << "Can't find log " << lastLogIdSent_ + 1 << " in wal, send the snapshot"
                << ", logIdToSend = " << logIdToSend_ << ", firstLogId in wal = " << part_->wal()->firstLogId()
                << ", lastLogId in wal = " << part_->wal()->lastLogId();
      sendingSnapshot_ = true;
      part_->snapshot_->sendSnapshot(part_, addr_).thenValue([self = shared_from_this()](Status&& status) {
        if (status.ok()) {
...
folly::Future<cpp2::AppendLogResponse> Host::sendAppendLogRequest(folly::EventBase* eb,
                                                                  std::shared_ptr<cpp2::AppendLogRequest> req) {
  ...
  VLOG(1) << idStr_ << "Sending request space " << req->get_space() << ", part " << req->get_part() << ", current term "
          << req->get_current_term() << ", last_log_id " << req->get_last_log_id() << ", committed_id "
          << req->get_committed_log_id() << ", last_log_term_sent" << req->get_last_log_term_sent()
