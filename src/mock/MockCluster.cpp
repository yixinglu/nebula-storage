...
// static
void MockCluster::waitUntilAllElected(kvstore::NebulaStore* kvstore, GraphSpaceID spaceId,
...
// static
std::unique_ptr<kvstore::MemPartManager> MockCluster::memPartMan(GraphSpaceID spaceId,
...
// static
std::unique_ptr<kvstore::NebulaStore> MockCluster::initKV(kvstore::KVOptions options, HostAddr localHost) {
  auto ioPool = std::make_shared<folly::IOThreadPoolExecutor>(4);
  auto workers = apache::thrift::concurrency::PriorityThreadManager::newPriorityThreadManager(1, true /*stats*/);
  ...
  auto store = std::make_unique<kvstore::NebulaStore>(std::move(options), ioPool, localHost, workers);
  store->init();
  return store;
}
...
void MockCluster::initListener(const char* dataPath, const HostAddr& addr) {
  ...
  lMetaClient_ = std::make_unique<meta::MetaClient>(threadPool, localhosts, meta::MetaClientOptions());
  ...
  esListener_ = std::make_unique<kvstore::NebulaStore>(std::move(KVOpt), ioThreadPool, addr, getWorkers());
  esListener_->init();
  sleep(FLAGS_heartbeat_interval_secs + 1);
}

void MockCluster::initStorageKV(const char* dataPath, HostAddr addr, SchemaVer schemaVerCount, bool hasProp,
                                bool hasListener, const std::vector<meta::cpp2::FTClient>& clients,
                                bool needCffBuilder) {
  ...
  if (metaClient_ != nullptr) {
    ...
    if (hasListener) {
      ...
      ret = metaClient_->addListener(spaceId, meta::cpp2::ListenerType::ELASTICSEARCH, {listenerHost}).get();
      if (!ret.ok()) {
    ...
    options.partMan_ = std::make_unique<kvstore::MetaServerBasedPartManager>(addr, metaClient_.get());
...
void MockCluster::startStorage(HostAddr addr, const std::string& rootPath, bool isGeneralService,
...
std::unique_ptr<meta::SchemaManager> MockCluster::memSchemaMan(SchemaVer schemaVerCount, GraphSpaceID spaceId,
                                                               bool hasProp) {
