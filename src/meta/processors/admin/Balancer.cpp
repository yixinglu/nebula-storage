...
DEFINE_double(leader_balance_deviation, 0.05,
...
ErrorOr<cpp2::ErrorCode, BalanceID> Balancer::cleanLastInValidPlan() {
  ...
  if (iter->valid()) {
    ...
    if (status == BalanceStatus::FAILED) {
      ...
      kv_->asyncMultiRemove(kDefaultSpaceId, kDefaultPartId, {iter->key().str()},
...
cpp2::ErrorCode Balancer::recovery() {
  ...
  if (kv_) {
    ...
    plan_->onFinished_ = [this]() {
      ...
      {
        std::lock_guard<std::mutex> lg(lock_);
        if (LastUpdateTimeMan::update(kv_, time::WallClock::fastNowInMilliSec()) != kvstore::ResultCode::SUCCEEDED) {
...
ErrorOr<cpp2::ErrorCode, std::vector<BalanceTask>> Balancer::genTasks(GraphSpaceID spaceId, int32_t spaceReplica,
  ...
  for (auto& lostHost : lostHosts) {
    ...
    for (auto& partId : lostParts) {
      ...
      if (!transferLostHost(tasks, confirmedHostParts, lostHost, spaceId, partId, dependentOnGroup)) {
...
bool Balancer::transferLostHost(std::vector<BalanceTask>& tasks, HostParts& confirmedHostParts, const HostAddr& source,
                                GraphSpaceID spaceId, PartitionID partId, bool dependentOnGroup) {
...
std::pair<HostParts, std::vector<HostAddr>> Balancer::fetchHostParts(GraphSpaceID spaceId, bool dependentOnGroup,
...
bool Balancer::balanceParts(BalanceID balanceId, GraphSpaceID spaceId, HostParts& confirmedHostParts,
                            int32_t totalParts, std::vector<BalanceTask>& tasks) {
  ...
  while (maxPartsHost.second > maxLoad || minPartsHost.second < minLoad) {
    ...
    LOG(INFO) << maxPartsHost.first << ":" << partsFrom.size() << " -> " << minPartsHost.first << ":" << partsTo.size();
    ...
    for (auto& partId : diff) {
      LOG(INFO) << "partsFrom size " << partsFrom.size() << " partsTo size " << partsTo.size() << " minLoad " << minLoad
                << " maxLoad " << maxLoad;
      if (partsFrom.size() == partsTo.size() + 1 || partsFrom.size() == static_cast<size_t>(minLoad) ||
          partsTo.size() == static_cast<size_t>(maxLoad)) {
        VLOG(3) << "No need to move any parts from " << maxPartsHost.first << " to " << minPartsHost.first;
        break;
      }
      ...
      tasks.emplace_back(balanceId, spaceId, partId, maxPartsHost.first, minPartsHost.first, kv_, client_);
      noAction = false;
    }
    if (noAction) {
...
bool Balancer::getHostParts(GraphSpaceID spaceId, bool dependentOnGroup, HostParts& hostParts, int32_t& totalParts) {
  folly::SharedMutex::ReadHolder rHolder(LockUtils::spaceLock());
...
bool Balancer::assembleZoneParts(const std::string& groupName, HostParts& hostParts) {
  ...
  for (auto it = hostParts.begin(); it != hostParts.end(); it++) {
    ...
    auto zoneIter = std::find_if(zoneHosts.begin(), zoneHosts.end(),
                                 [host](const auto& pair) -> bool { return host == pair.first.first; });
...
void Balancer::calDiff(const HostParts& hostParts, const std::vector<HostAddr>& activeHosts,
                       std::vector<HostAddr>& expand, std::vector<HostAddr>& lost) {
  for (auto it = hostParts.begin(); it != hostParts.end(); it++) {
...
std::vector<std::pair<HostAddr, int32_t>> Balancer::sortedHostsByParts(const HostParts& hostParts) {
  ...
  std::sort(hosts.begin(), hosts.end(), [](const auto& l, const auto& r) { return l.second < r.second; });
  return hosts;
}

Status Balancer::checkReplica(const HostParts& hostParts, const std::vector<HostAddr>& activeHosts, int32_t replica,
                              PartitionID partId) {
...
StatusOr<HostAddr> Balancer::hostWithMinimalParts(const HostParts& hostParts, PartitionID partId) {
...
StatusOr<HostAddr> Balancer::hostWithMinimalPartsForZone(const HostAddr& source, const HostParts& hostParts,
                                                         PartitionID partId) {
  auto hosts = sortedHostsByParts(hostParts);
  for (auto& h : hosts) {
    auto it = hostParts.find(h.first);
...
cpp2::ErrorCode Balancer::leaderBalance() {
  ...
  if (inLeaderBalance_.compare_exchange_strong(expected, true)) {
    ...
    for (const auto& spaceInfo : spaces) {
      ...
      auto balanceResult = buildLeaderBalancePlan(hostLeaderMap_.get(), spaceId, replicaFactor, dependentOnGroup, plan);
      if (!balanceResult) {
      ...
      for (const auto& task : plan) {
        futures.emplace_back(client_->transLeader(std::get<0>(task), std::get<1>(task), std::move(std::get<2>(task)),
...
bool Balancer::buildLeaderBalancePlan(HostLeaderMap* hostLeaderMap, GraphSpaceID spaceId, int32_t replicaFactor,
                                      bool dependentOnGroup, LeaderBalancePlan& plan, bool useDeviation) {
  ...
  if (dependentOnGroup) {
    ...
    if (useDeviation) {
      globalMin = std::ceil(static_cast<double>(leaderParts) / activeSize * (1 - FLAGS_leader_balance_deviation));
      globalMax = std::floor(static_cast<double>(leaderParts) / activeSize * (1 + FLAGS_leader_balance_deviation));
    }
    VLOG(3) << "Build leader balance plan, expected min load: " << globalMin << ", max load: " << globalMax
            << " avg: " << globalAvg;
  ...
  while (true) {
    ...
    for (const auto& hostEntry : leaderHostParts) {
      ...
      if (hostMinLoad <= partSize && partSize <= hostMaxLoad) {
        VLOG(3) << partSize << " is between min load " << hostMinLoad << " and max load " << hostMaxLoad;
        continue;
      }
      ...
      if (partSize < hostMinLoad) {
        ...
        LOG(INFO) << "Acquire leaders to host: " << host << " loading: " << partSize << " min loading " << hostMinLoad;
        taskCount += acquireLeaders(allHostParts, leaderHostParts, peersMap, activeHosts, host, plan, spaceId);
      } else {
        // need to transfer leader to other hosts
        LOG(INFO) << "Giveup leaders from host: " << host << " loading: " << partSize << " max loading " << hostMaxLoad;
...
int32_t Balancer::acquireLeaders(HostParts& allHostParts, HostParts& leaderHostParts, PartAllocation& peersMap,
                                 std::unordered_set<HostAddr>& activeHosts, const HostAddr& target,
                                 LeaderBalancePlan& plan, GraphSpaceID spaceId) {
  // host will loop for the partition which is not leader, and try to acuire the
  // leader
  int32_t taskCount = 0;
  std::vector<PartitionID> diff;
  std::set_difference(allHostParts[target].begin(), allHostParts[target].end(), leaderHostParts[target].begin(),
                      leaderHostParts[target].end(), std::back_inserter(diff));
  ...
  for (const auto& partId : diff) {
    ...
    for (const auto& source : sources) {
      ...
      VLOG(3) << "Check peer: " << source << " min load: " << minLoad << " peerLeaders size: " << sourceLeaders.size();
      ...
      if (it != sourceLeaders.end() && minLoad < sourceLeaders.size()) {
        ...
        LOG(INFO) << "acquire plan trans leader space: " << spaceId << " part: " << partId << " from " << source.host
                  << ":" << source.port << " to " << target.host << ":" << target.port;
        ++taskCount;
        break;
      }
    }
...
int32_t Balancer::giveupLeaders(HostParts& leaderParts, PartAllocation& peersMap,
                                std::unordered_set<HostAddr>& activeHosts, const HostAddr& source,
                                LeaderBalancePlan& plan, GraphSpaceID spaceId) {
  int32_t taskCount = 0;
  ...
  for (auto it = sourceLeaders.begin(); it != sourceLeaders.end();) {
    ...
    auto target = std::min_element(targets.begin(), targets.end(), [&](const auto& l, const auto& r) -> bool {
    ...
    if (target != targets.end()) {
      ...
      if (targetLeaderSize < hostBounds_[*target].second) {
        ...
        targetLeaders.emplace_back(partId);
        plan.emplace_back(spaceId, partId, source, *target);
        LOG(INFO) << "giveup plan trans leader space: " << spaceId << " part: " << partId << " from " << source.host
                  << ":" << source.port << " to " << target->host << ":" << target->port;
        ++taskCount;
        isErase = true;
      }
...
void Balancer::simplifyLeaderBalnacePlan(GraphSpaceID spaceId, LeaderBalancePlan& plan) {
  ...
  for (const auto& partEntry : buckets) {
    plan.emplace_back(spaceId, partEntry.first, std::get<2>(partEntry.second.front()),
                      std::get<3>(partEntry.second.back()));
  }
}

bool Balancer::collectZoneParts(const std::string& groupName, HostParts& hostParts) {
  auto groupKey = MetaServiceUtils::groupKey(groupName);
  std::string groupValue;
  auto code = kv_->get(kDefaultSpaceId, kDefaultPartId, groupKey, &groupValue);
  if (code != kvstore::ResultCode::SUCCEEDED) {
...
bool Balancer::checkZoneLegal(const HostAddr& source, const HostAddr& target, PartitionID part) {
  ...
  auto sourceIter =
      std::find_if(zoneParts_.begin(), zoneParts_.end(), [&source](const auto& pair) { return source == pair.first; });
  ...
  auto targetIter =
      std::find_if(zoneParts_.begin(), zoneParts_.end(), [&target](const auto& pair) { return target == pair.first; });
