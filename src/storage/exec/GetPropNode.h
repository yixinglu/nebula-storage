...
class GetTagPropNode : public QueryNode<VertexID> {
 public:
  ...
  explicit GetTagPropNode(PlanContext* planCtx, std::vector<TagNode*> tagNodes, nebula::DataSet* resultDataSet,
  ...
  kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
    ...
    if (!std::any_of(tagNodes_.begin(), tagNodes_.end(), [](const auto& tagNode) { return tagNode->valid(); })) {
    ...
    for (auto* tagNode : tagNodes_) {
      ret = tagNode->collectTagPropsIfValid(
          ...
          [this, &row, vIdLen, isIntId, &vId, tagNode](folly::StringPiece key, RowReader* reader,
...
class GetEdgePropNode : public QueryNode<cpp2::EdgeKey> {
 public:
  ...
  GetEdgePropNode(PlanContext* planCtx, std::vector<EdgeNode<cpp2::EdgeKey>*> edgeNodes, nebula::DataSet* resultDataSet)
      : planContext_(planCtx), edgeNodes_(std::move(edgeNodes)), resultDataSet_(resultDataSet) {}
  ...
  kvstore::ResultCode execute(PartitionID partId, const cpp2::EdgeKey& edgeKey) override {
    ...
    for (auto* edgeNode : edgeNodes_) {
      ret = edgeNode->collectEdgePropsIfValid(
          ...
          [&row, vIdLen, isIntId](folly::StringPiece key, RowReader* reader,
                                  const std::vector<PropContext>* props) -> kvstore::ResultCode {
