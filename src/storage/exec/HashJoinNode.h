...
class HashJoinNode : public IterateNode<VertexID> {
 public:
  ...
  HashJoinNode(PlanContext* planCtx, const std::vector<TagNode*>& tagNodes,
               const std::vector<EdgeNode<VertexID>*>& edgeNodes, TagContext* tagContext, EdgeContext* edgeContext,
  ...
  kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
    ...
    for (auto* tagNode : tagNodes_) {
      ret = tagNode->collectTagPropsIfValid(
          ...
          [this, &vId, &result, tagNode](folly::StringPiece key, RowReader* reader,
            ...
            for (const auto& prop : *props) {
              ...
              auto value = QueryUtils::readVertexProp(key, planContext_->vIdLen_, planContext_->isIntId_, reader, prop);
              if (!value.ok()) {
            ...
            if (FLAGS_enable_vertex_cache && tagContext_->vertexCache_ != nullptr) {
              tagContext_->vertexCache_->insert(std::make_pair(vId, tagId), reader->getData());
            }
