...
void HBProcessor::process(const cpp2::HBReq& req) {
  ...
  if (FLAGS_hosts_whitelist_enabled &&
      hostExist(MetaServiceUtils::hostKey(host.host, host.port)) == Status::HostNotFound()) {
  ...
  VLOG(3) << "Receive heartbeat from " << host << ", role = " << apache::thrift::util::enumNameSafe(req.get_role());
