...
std::vector<std::string> MockData::teams_ = {
    "Warriors",  "Nuggets",      "Rockets", "Trail Blazers", "Spurs",     "Thunders",  "Jazz",  "Clippers",
    "Kings",     "Timberwolves", "Lakers",  "Pelicans",      "Grizzlies", "Mavericks", "Suns",  "Hornets",
    "Cavaliers", "Celtics",      "Raptors", "76ers",         "Pacers",    "Bulls",     "Hawks", "Knicks",
...
std::vector<Player> MockData::players_ = {{"Tim Duncan", 44, false, 19, 1997, 2016, 1392, 19.0, 1, "America", 5},
...
// Mock schema
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockPlayerTagSchema(SchemaVer ver, bool hasProp) {
  std::shared_ptr<meta::NebulaSchemaProvider> schema(new meta::NebulaSchemaProvider(ver));
  if (!hasProp) {
    return schema;
  }
  schema->addField("name", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression(""));
  // only age filed has no default value and nullable is false
  schema->addField("age", meta::cpp2::PropertyType::INT64, 0, false);
  schema->addField("playing", meta::cpp2::PropertyType::BOOL, 0, false, new ConstantExpression(true));
  schema->addField("career", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(10L));
  schema->addField("startYear", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(0L));
  schema->addField("endYear", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(0L));
  schema->addField("games", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(0L));
  schema->addField("avgScore", meta::cpp2::PropertyType::DOUBLE, 0, false, new ConstantExpression(0.0));
  schema->addField("serveTeams", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(0L));

  // Set ttl property
  if (FLAGS_mock_ttl_col) {
    schema->addField("insertTime", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(0L));
  ...
  // Use default value
  schema->addField("country", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression("America"));
  // Use nullable
...
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockTeamTagSchema(SchemaVer ver, bool hasProp) {
  std::shared_ptr<meta::NebulaSchemaProvider> schema(new meta::NebulaSchemaProvider(ver));
  if (!hasProp) {
    return schema;
  }
...
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockServeEdgeSchema(SchemaVer ver, bool hasProp) {
  std::shared_ptr<meta::NebulaSchemaProvider> schema(new meta::NebulaSchemaProvider(ver));
  if (!hasProp) {
    return schema;
  }
  schema->addField("playerName", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression(""));
  schema->addField("teamName", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression(""));
  schema->addField("startYear", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(2020L));
  schema->addField("endYear", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(2020L));
  ...
  schema->addField("teamGames", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(1L));
  schema->addField("teamAvgScore", meta::cpp2::PropertyType::DOUBLE, 0, false, new ConstantExpression(0.0));
  ...
  // Use default value
  schema->addField("type", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression("trade"));
  // Use nullable
  schema->addField("champions", meta::cpp2::PropertyType::INT64, 0, true);
...
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockTeammateEdgeSchema(SchemaVer ver, bool hasProp) {
  std::shared_ptr<meta::NebulaSchemaProvider> schema(new meta::NebulaSchemaProvider(ver));
  if (!hasProp) {
    return schema;
  }
...
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockTypicaSchemaV2() {
  ...
  schema->addField("col_bool_default", meta::cpp2::PropertyType::BOOL, 0, false, new ConstantExpression(true));
  schema->addField("col_int", meta::cpp2::PropertyType::INT64);
  schema->addField("col_int_null", meta::cpp2::PropertyType::INT64, 0, true);
  schema->addField("col_int_default", meta::cpp2::PropertyType::INT64, 0, false, new ConstantExpression(20L));
  schema->addField("col_float", meta::cpp2::PropertyType::FLOAT);
  schema->addField("col_float_null", meta::cpp2::PropertyType::FLOAT, 0, true);
  schema->addField("col_float_default", meta::cpp2::PropertyType::FLOAT, 0, false, new ConstantExpression(2.2F));
  schema->addField("col_str", meta::cpp2::PropertyType::STRING);
  schema->addField("col_str_null", meta::cpp2::PropertyType::STRING, 0, true);
  schema->addField("col_str_default", meta::cpp2::PropertyType::STRING, 0, false, new ConstantExpression("sky"));
  schema->addField("col_date", meta::cpp2::PropertyType::DATE);
  schema->addField("col_date_null", meta::cpp2::PropertyType::DATE, 0, true);

  const Date date = {2020, 2, 20};
  schema->addField("col_date_default", meta::cpp2::PropertyType::DATE, 0, false, new ConstantExpression(date));
...
std::unordered_map<PartitionID, std::vector<VertexData>> MockData::mockVerticesofPart(int32_t parts) {
...
std::vector<EdgeData> MockData::mockEdges(bool upper) {
  ...
  for (auto& serve : serves_) {
    ...
    if (upper) {
      ...
      auto toupper = [](auto c) { return ::toupper(c); };
      std::transform(serve.playerName_.begin(), serve.playerName_.end(), upperName.begin(), toupper);
...
std::vector<std::pair<PartitionID, std::string>> MockData::mockServeIndexKeys() {
  ...
  for (auto& serve : serves_) {
    ...
    auto key = IndexKeyUtils::edgeIndexKey(32, part, 101, serve.playerName_, serve.startYear_, serve.teamName_,
                                           std::move(values));
    auto pair = std::make_pair(part, std::move(key));
    keys.emplace_back(std::move(pair));
  }
...
std::unordered_map<VertexID, std::vector<EdgeData>> MockData::mockmMultiRankServes(EdgeRanking rankCount) {
...
nebula::storage::cpp2::AddVerticesRequest MockData::mockAddVerticesSpecifiedOrderReq(int32_t parts) {
  nebula::storage::cpp2::AddVerticesRequest req;
  req.set_space_id(1);
  req.set_overwritable(true);
  ...
  for (auto& rec : retRecs) {
    ...
    if (rec.tId_ == 1) {
      std::vector<std::string> colNames{"serveTeams", "avgScore", "games", "endYear", "startYear",
                                        "career",     "playing",  "age",   "name"};
...
nebula::storage::cpp2::AddEdgesRequest MockData::mockAddEdgesSpecifiedOrderReq(int32_t parts) {
  ...
  for (auto& rec : retRecs) {
    ...
    std::vector<std::string> colNames{"teamAvgScore", "teamGames", "teamCareer", "endYear",
                                      "startYear",    "teamName",  "playerName"};
