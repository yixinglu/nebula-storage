...
Value RowReaderV2::getValueByIndex(const int64_t index) const noexcept {
  ...
  switch (field->type()) {
    ...
    case meta::cpp2::PropertyType::DATE: {
      ...
      memcpy(reinterpret_cast<void*>(&dt.day), &data_[offset + sizeof(int16_t) + sizeof(int8_t)], sizeof(int8_t));
      return dt;
    }
    case meta::cpp2::PropertyType::TIME: {
      ...
      memcpy(reinterpret_cast<void*>(&t.microsec), &data_[offset + 3 * sizeof(int8_t)], sizeof(int32_t));
      return t;
    }
    case meta::cpp2::PropertyType::DATETIME: {
      ...
      memcpy(reinterpret_cast<void*>(&dt.month), &data_[offset + sizeof(int16_t)], sizeof(int8_t));
      memcpy(reinterpret_cast<void*>(&dt.day), &data_[offset + sizeof(int16_t) + sizeof(int8_t)], sizeof(int8_t));
      memcpy(reinterpret_cast<void*>(&dt.hour), &data_[offset + sizeof(int16_t) + 2 * sizeof(int8_t)], sizeof(int8_t));
      memcpy(reinterpret_cast<void*>(&dt.minute), &data_[offset + sizeof(int16_t) + 3 * sizeof(int8_t)],
             sizeof(int8_t));
      memcpy(reinterpret_cast<void*>(&dt.sec), &data_[offset + sizeof(int16_t) + 4 * sizeof(int8_t)], sizeof(int8_t));
      memcpy(reinterpret_cast<void*>(&dt.microsec), &data_[offset + sizeof(int16_t) + 5 * sizeof(int8_t)],
             sizeof(int32_t));
      return dt;
    }
