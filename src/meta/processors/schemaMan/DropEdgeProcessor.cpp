...
void DropEdgeProcessor::process(const cpp2::DropEdgeReq& req) {
  ...
  if (iRet.ok()) {
    ...
    handleErrorCode(req.get_if_exists() == true ? cpp2::ErrorCode::SUCCEEDED : cpp2::ErrorCode::E_NOT_FOUND);
    onFinished();
    return;
  }
...
StatusOr<std::vector<std::string>> DropEdgeProcessor::getEdgeKeys(GraphSpaceID id, EdgeType edgeType) {
