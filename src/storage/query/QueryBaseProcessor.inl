...
cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::handleVertexProps(std::vector<cpp2::VertexProp>& vertexProps) {
...
template <typename REQ, typename RESP>
cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::handleEdgeProps(std::vector<cpp2::EdgeProp>& edgeProps) {
...
template <typename REQ, typename RESP>
void QueryBaseProcessor<REQ, RESP>::addReturnPropContext(std::vector<PropContext>& ctxs, const char* propName,
...
std::vector<cpp2::VertexProp> QueryBaseProcessor<REQ, RESP>::buildAllTagProps() {
  ...
  std::sort(result.begin(), result.end(), [&](const auto& a, const auto& b) { return a.get_tag() < b.get_tag(); });
  return result;
}

template <typename REQ, typename RESP>
std::vector<cpp2::EdgeProp> QueryBaseProcessor<REQ, RESP>::buildAllEdgeProps(const cpp2::EdgeDirection& direction) {
  ...
  std::sort(result.begin(), result.end(), [&](const auto& a, const auto& b) { return a.get_type() < b.get_type(); });
  return result;
}
...
template <typename REQ, typename RESP>
cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::checkExp(const Expression* exp, bool returned, bool filtered,
                                                        bool updated) {
  switch (exp->kind()) {
    ...
    case Expression::Kind::kVar: {
      ...
      return varExp->isInner() ? cpp2::ErrorCode::SUCCEEDED : cpp2::ErrorCode::E_INVALID_FILTER;
    }
    ...
    case Expression::Kind::kSrcProperty: {
      ...
      addPropContextIfNotExists(tagContext_.propContexts_, tagContext_.indexMap_, tagContext_.tagNames_, tagId, tagName,
                                propName, field, returned, filtered);
      if (updated) {
    ...
    case Expression::Kind::kEdgeProperty: {
      ...
      */
      if (edgeContext_.indexMap_.count(edgeType)) {
        addPropContextIfNotExists(edgeContext_.propContexts_, edgeContext_.indexMap_, edgeContext_.edgeNames_, edgeType,
                                  edgeName, propName, field, returned, filtered);
      }
      if (edgeContext_.indexMap_.count(-edgeType)) {
        addPropContextIfNotExists(edgeContext_.propContexts_, edgeContext_.indexMap_, edgeContext_.edgeNames_,
                                  -edgeType, edgeName, propName, field, returned, filtered);
      }
      if (updated) {
        valueProps_.emplace(*propName);
      }
...
void QueryBaseProcessor<REQ, RESP>::addPropContextIfNotExists(
    ...
    std::unordered_map<SchemaID, size_t>& indexMap, std::unordered_map<SchemaID, std::string>& names, int32_t entryId,
    const std::string* entryName, const std::string* propName, const meta::SchemaProviderIf::Field* field,
    bool returned, bool filtered, const std::pair<size_t, cpp2::StatType>* statInfo) {
  ...
  if (idxIter == indexMap.end()) {
    ...
    auto iter =
        std::find_if(props.begin(), props.end(), [propName](const auto& prop) { return prop.name_ == *propName; });
    if (iter == props.end()) {
