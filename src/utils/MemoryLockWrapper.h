...
class MemoryLockGuard {
 public:
  MemoryLockGuard(MemoryLockCore<Key>* lock, const Key& key) : MemoryLockGuard(lock, std::vector<Key>{key}) {}
  ...
  MemoryLockGuard(MemoryLockGuard&& lg) noexcept : lock_(lg.lock_), keys_(std::move(lg.keys_)), locked_(lg.locked_) {}
