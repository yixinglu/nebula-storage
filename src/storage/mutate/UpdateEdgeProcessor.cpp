...
void UpdateEdgeProcessor::doProcess(const cpp2::UpdateEdgeRequest& req) {
  ...
  if (!NebulaKeyUtils::isValidVidLen(spaceVidLen_, edgeKey_.get_src().getStr(), edgeKey_.get_dst().getStr())) {
  ...
  VLOG(3) << "Update edge, spaceId: " << spaceId_ << ", partId:  " << partId << ", src: " << edgeKey_.get_src()
          << ", edge_type: " << edgeKey_.get_edge_type() << ", dst: " << edgeKey_.get_dst()
...
cpp2::ErrorCode UpdateEdgeProcessor::checkAndBuildContexts(const cpp2::UpdateEdgeRequest& req) {
...
StoragePlan<cpp2::EdgeKey> UpdateEdgeProcessor::buildPlan(nebula::DataSet* result) {
  ...
  auto edgeUpdate = std::make_unique<FetchEdgeNode>(
      planContext_.get(), &edgeContext_, edgeContext_.propContexts_[0].first, &(edgeContext_.propContexts_[0].second));

  auto filterNode = std::make_unique<FilterNode<cpp2::EdgeKey>>(planContext_.get(), edgeUpdate.get(), expCtx_.get(),
                                                                filterExp_.get());
  filterNode->addDependency(edgeUpdate.get());

  auto updateNode = std::make_unique<UpdateEdgeNode>(planContext_.get(), indexes_, updatedProps_, filterNode.get(),
                                                     insertable_, depPropMap_, expCtx_.get(), &edgeContext_);
  updateNode->addDependency(filterNode.get());

  auto resultNode = std::make_unique<UpdateResNode<cpp2::EdgeKey>>(planContext_.get(), updateNode.get(),
                                                                   getReturnPropsExp(), expCtx_.get(), result);
...
cpp2::ErrorCode UpdateEdgeProcessor::buildEdgeContext(const cpp2::UpdateEdgeRequest& req) {
  // Build default edge context
  auto edgeNameRet = env_->schemaMan_->toEdgeName(spaceId_, std::abs(edgeKey_.get_edge_type()));
  if (!edgeNameRet.ok()) {
    VLOG(1) << "Can't find spaceId " << spaceId_ << " edgeType " << std::abs(edgeKey_.get_edge_type());
  ...
  for (auto& edgeProp : updatedProps_) {
    EdgePropertyExpression edgePropExp(new std::string(edgeName), new std::string(edgeProp.get_name()));
  ...
  if (expCtx_ == nullptr) {
    expCtx_ = std::make_unique<StorageExpressionContext>(spaceVidLen_, isIntId_, planContext_->edgeName_,
                                                         planContext_->edgeSchema_, true);
  }
