...
DEFINE_int32(removed_threshold_sec, 24 * 60 * 60, "Hosts will be removed in this time if no heartbeat received");
...
Status ListHostsProcessor::allHostsWithStatus(cpp2::HostRole role) {
  ...
  while (iter->valid()) {
    ...
    if (now - info.lastHBTimeInMilliSec_ < FLAGS_removed_threshold_sec * 1000) {
      ...
      if (now - info.lastHBTimeInMilliSec_ < FLAGS_heartbeat_interval_secs * FLAGS_expired_time_factor * 1000) {
        item.set_status(cpp2::HostStatus::ONLINE);
      } else {
...
Status ListHostsProcessor::fillLeaderAndPartInfoPerHost() {
  ...
  while (iter->valid()) {
    ...
    if (std::find(activeHosts.begin(), activeHosts.end(), host) != activeHosts.end()) {
      auto hostIt = std::find_if(hostItems_.begin(), hostItems_.end(),
                                 [&](const auto& item) { return item.get_hostAddr() == host; });
  ...
  std::unordered_map<HostAddr, std::unordered_map<std::string, std::vector<PartitionID>>> allParts;
  ...
  for (const auto& hostEntry : allParts) {
    ...
    auto it = std::find_if(hostItems_.begin(), hostItems_.end(),
                           [&](const auto& item) { return item.get_hostAddr() == hostAddr; });
...
void ListHostsProcessor::removeExpiredHosts(std::vector<std::string>&& removeHostsKey) {
  ...
  kvstore_->asyncMultiRemove(kDefaultSpaceId, kDefaultPartId, std::move(removeHostsKey), [](kvstore::ResultCode code) {
...
std::unordered_map<std::string, std::vector<PartitionID>> ListHostsProcessor::getLeaderPartsWithSpaceName(
