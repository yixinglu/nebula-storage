      ...
      for (auto& newTag : newTags) {
        ...
        if (nebula::ok(obsIdx)) {
          if (!nebula::value(obsIdx).empty()) {
            oReader = RowReaderWrapper::getTagPropReader(env_->schemaMan_, spaceId_, tagId, nebula::value(obsIdx));
          }
        } else {
        ...
        if (!retEnc.value().empty()) {
          nReader = RowReaderWrapper::getTagPropReader(env_->schemaMan_, spaceId_, tagId, retEnc.value());
        }
...
ErrorOr<kvstore::ResultCode, std::string> AddVerticesProcessor::findOldValue(PartitionID partId, const VertexID& vId,
                                                                             TagID tagId) {
...
std::string AddVerticesProcessor::indexKey(PartitionID partId, const VertexID& vId, RowReader* reader,
