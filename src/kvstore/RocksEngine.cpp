 ...
 *
 **************************************/
RocksEngine::RocksEngine(GraphSpaceID spaceId, int32_t vIdLen, const std::string& dataPath,
                         std::shared_ptr<rocksdb::MergeOperator> mergeOp,
                         std::shared_ptr<rocksdb::CompactionFilterFactory> cfFactory, bool readonly)
...
std::unique_ptr<WriteBatch> RocksEngine::startBatchWrite() { return std::make_unique<RocksWriteBatch>(); }

ResultCode RocksEngine::commitBatchWrite(std::unique_ptr<WriteBatch> batch, bool disableWAL, bool sync) {
...
std::vector<Status> RocksEngine::multiGet(const std::vector<std::string>& keys, std::vector<std::string>* values) {
  rocksdb::ReadOptions options;
...
ResultCode RocksEngine::range(const std::string& start, const std::string& end,
...
ResultCode RocksEngine::prefix(const std::string& prefix, std::unique_ptr<KVIterator>* storageIter) {
  rocksdb::ReadOptions options;
...
ResultCode RocksEngine::rangeWithPrefix(const std::string& start, const std::string& prefix,
                                        std::unique_ptr<KVIterator>* storageIter) {
  rocksdb::ReadOptions options;
  options.prefix_same_as_start = true;
  rocksdb::Iterator* iter = db_->NewIterator(options);
...
std::string RocksEngine::partKey(PartitionID partId) { return NebulaKeyUtils::systemPartKey(partId); }
...
ResultCode RocksEngine::createCheckpoint(const std::string& name) {
  ...
  auto checkpointPath = folly::stringPrintf("%s/checkpoints/%s/data", dataPath_.c_str(), name.c_str());
...
ErrorOr<ResultCode, std::string> RocksEngine::backupTable(const std::string& name, const std::string& tablePrefix,
                                                          std::function<bool(const folly::StringPiece& key)> filter) {
  auto backupPath =
      folly::stringPrintf("%s/checkpoints/%s/%s.sst", dataPath_.c_str(), name.c_str(), tablePrefix.c_str());
  VLOG(3) << "Start writing the sst file with table (" << tablePrefix << ") to file: " << backupPath;
  ...
  if (!s.ok()) {
    LOG(WARNING) << "Failure to insert data when backupTable,  " << backupPath << ", error: " << s.ToString();
    return ResultCode::ERR_BACKUP_EMPTY_TABLE;
  }
