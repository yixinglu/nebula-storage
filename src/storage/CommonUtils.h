...
struct ProcessorCounters {
  ...
  virtual void init(const std::string& counterName) {
    if (!numCalls_.valid()) {
      ...
      numErrors_ = stats::StatsManager::registerStats("num_" + counterName + "_errors", "rate, sum");
      latency_ = stats::StatsManager::registerHisto(counterName + "_latency_us", 1000, 0, 20000, "avg, p75, p95, p99");
...
class CommonUtils final {
 public:
  static bool checkDataExpiredForTTL(const meta::SchemaProviderIf* schema, RowReader* reader, const std::string& ttlCol,
                                     int64_t ttlDuration);
