...
GraphStorageServiceHandler::GraphStorageServiceHandler(StorageEnv* env)
    : env_(env), vertexCache_(FLAGS_vertex_cache_num, FLAGS_vertex_cache_bucket_exp) {
  if (FLAGS_reader_handlers_type == "io") {
    auto tf = std::make_shared<folly::NamedThreadFactory>("reader-pool");
    readerPool_ = std::make_shared<folly::IOThreadPoolExecutor>(FLAGS_reader_handlers, std::move(tf));
  } else {
...
// Vertice section
folly::Future<cpp2::ExecResponse> GraphStorageServiceHandler::future_addVertices(const cpp2::AddVerticesRequest& req) {
...
folly::Future<cpp2::UpdateResponse> GraphStorageServiceHandler::future_updateVertex(
  ...
  auto* processor = UpdateVertexProcessor::instance(env_, &kUpdateVertexCounters, readerPool_.get(), &vertexCache_);
  RETURN_FUTURE(processor);
}

// Edge section
folly::Future<cpp2::ExecResponse> GraphStorageServiceHandler::future_addEdges(const cpp2::AddEdgesRequest& req) {
  auto* processor = AddEdgesProcessor::instance(env_, &kAddEdgesCounters);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::ExecResponse> GraphStorageServiceHandler::future_deleteEdges(const cpp2::DeleteEdgesRequest& req) {
  auto* processor = DeleteEdgesProcessor::instance(env_, &kDelEdgesCounters);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::UpdateResponse> GraphStorageServiceHandler::future_updateEdge(const cpp2::UpdateEdgeRequest& req) {
...
folly::Future<cpp2::GetNeighborsResponse> GraphStorageServiceHandler::future_getNeighbors(
  ...
  auto* processor = GetNeighborsProcessor::instance(env_, &kGetNeighborsCounters, readerPool_.get(), &vertexCache_);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::GetPropResponse> GraphStorageServiceHandler::future_getProps(const cpp2::GetPropRequest& req) {
  auto* processor = GetPropProcessor::instance(env_, &kGetPropCounters, readerPool_.get(), &vertexCache_);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::LookupIndexResp> GraphStorageServiceHandler::future_lookupIndex(
    const cpp2::LookupIndexRequest& req) {
  auto* processor = LookupProcessor::instance(env_, &kLookupCounters, readerPool_.get(), &vertexCache_);
  RETURN_FUTURE(processor);
}
...
folly::Future<cpp2::ScanEdgeResponse> GraphStorageServiceHandler::future_scanEdge(const cpp2::ScanEdgeRequest& req) {
  auto* processor = ScanEdgeProcessor::instance(env_, &kScanEdgeCounters, readerPool_.get());
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::GetUUIDResp> GraphStorageServiceHandler::future_getUUID(const cpp2::GetUUIDReq&) {
  LOG(FATAL) << "Unsupported in version 2.0";
}

folly::Future<cpp2::ExecResponse> GraphStorageServiceHandler::future_addEdgesAtomic(const cpp2::AddEdgesRequest& req) {
