...
void BalancePlan::invoke() {
  ...
  std::sort(tasks_.begin(), tasks_.end(), [](auto& l, auto& r) { return l.taskIdStr() < r.taskIdStr(); });
  dispatchTasks();
    ...
    for (size_t j = 0; j < buckets_[i].size(); j++) {
      ...
      tasks_[taskIndex].onError_ = [this, i, j, taskIndex]() {
        ...
        if (finished) {
          ...
          auto& task = this->tasks_[this->buckets_[i][j + 1]];
          if (tasks_[taskIndex].spaceId_ == task.spaceId_ && tasks_[taskIndex].partId_ == task.partId_) {
...
cpp2::ErrorCode BalancePlan::saveInStore(bool onlyPlan) {
  ...
  data.emplace_back(MetaServiceUtils::balancePlanKey(id_), MetaServiceUtils::balancePlanVal(status_));
  if (!onlyPlan) {
    for (auto& task : tasks_) {
      data.emplace_back(
          MetaServiceUtils::balanceTaskKey(task.balanceId_, task.spaceId_, task.partId_, task.src_, task.dst_),
          MetaServiceUtils::balanceTaskVal(task.status_, task.ret_, task.startTimeMs_, task.endTimeMs_));
    }
  }
  folly::Baton<true, std::atomic> baton;
  auto ret = kvstore::ResultCode::SUCCEEDED;
  kv_->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&baton, &ret](kvstore::ResultCode code) {
