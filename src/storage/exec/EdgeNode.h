...
class EdgeNode : public IterateNode<T> {
 public:
  ...
  kvstore::ResultCode collectEdgePropsIfValid(NullHandler nullHandler, PropHandler valueHandler) {
 ...
 protected:
  EdgeNode(PlanContext* planCtx, EdgeContext* ctx, EdgeType edgeType, const std::vector<PropContext>* props,
           StorageExpressionContext* expCtx, Expression* exp)
      : planContext_(planCtx), edgeContext_(ctx), edgeType_(edgeType), props_(props), expCtx_(expCtx), exp_(exp) {
    UNUSED(expCtx_);
    UNUSED(exp_);
  ...
  EdgeNode(PlanContext* planCtx, EdgeContext* ctx) : planContext_(planCtx), edgeContext_(ctx) {}
...
class FetchEdgeNode final : public EdgeNode<cpp2::EdgeKey> {
 public:
  ...
  FetchEdgeNode(PlanContext* planCtx, EdgeContext* ctx, EdgeType edgeType, const std::vector<PropContext>* props,
                StorageExpressionContext* expCtx = nullptr, Expression* exp = nullptr)
  ...
  kvstore::ResultCode execute(PartitionID partId, const cpp2::EdgeKey& edgeKey) override {
    ...
    VLOG(1) << "partId " << partId << ", edgeType " << edgeType_ << ", prop size " << props_->size();
    if (edgeType_ != *edgeKey.edge_type_ref()) {
      iter_.reset();
      return kvstore::ResultCode::SUCCEEDED;
    }
    prefix_ =
        NebulaKeyUtils::edgePrefix(planContext_->vIdLen_, partId, (*edgeKey.src_ref()).getStr(),
                                   *edgeKey.edge_type_ref(), *edgeKey.ranking_ref(), (*edgeKey.dst_ref()).getStr());
    ...
    if (ret == kvstore::ResultCode::SUCCEEDED && iter && iter->valid()) {
      if (planContext_->env_->txnMan_ && planContext_->env_->txnMan_->enableToss(planContext_->spaceId_)) {
        ...
        iter_.reset(new TossEdgeIterator(planContext_, std::move(iter), edgeType_, schemas_, &ttl_, stopAtFirstEdge));
      } else {
        iter_.reset(new SingleEdgeIterator(planContext_, std::move(iter), edgeType_, schemas_, &ttl_, false));
      }
    } else {
      iter_.reset();
    }
...
class SingleEdgeNode final : public EdgeNode<VertexID> {
 public:
  using RelNode::execute;
  SingleEdgeNode(PlanContext* planCtx, EdgeContext* ctx, EdgeType edgeType, const std::vector<PropContext>* props,
                 StorageExpressionContext* expCtx = nullptr, Expression* exp = nullptr)
      : EdgeNode(planCtx, ctx, edgeType, props, expCtx, exp) {}
  ...
  kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
    ...
    VLOG(1) << "partId " << partId << ", vId " << vId << ", edgeType " << edgeType_ << ", prop size " << props_->size();
    std::unique_ptr<kvstore::KVIterator> iter;
    ...
    if (ret == kvstore::ResultCode::SUCCEEDED && iter && iter->valid()) {
      if (planContext_->env_->txnMan_ && planContext_->env_->txnMan_->enableToss(planContext_->spaceId_)) {
        ...
        iter_.reset(new SingleEdgeIterator(planContext_, std::move(iter), edgeType_, schemas_, &ttl_));
      }
    } else {
      iter_.reset();
    }
