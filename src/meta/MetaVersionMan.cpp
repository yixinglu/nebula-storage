...
bool MetaVersionMan::setMetaVersionToKV(kvstore::KVStore* kv) {
  ...
  data.emplace_back(kMetaVersionKey, std::string(reinterpret_cast<const char*>(&v2), sizeof(MetaVersion)));
  bool ret = true;
...
Status MetaVersionMan::updateMetaV1ToV2(kvstore::KVStore* kv) {
  CHECK_NOTNULL(kv);
  auto snapshot = folly::format("META_UPGRADE_SNAPSHOT_{}", MetaServiceUtils::genTimestampStr()).str();
...
Status MetaVersionMan::doUpgrade(kvstore::KVStore* kv) {
  ...
  // delete
  {
    std::vector<std::string> prefixes({nebula::oldmeta::kIndexStatusTable, nebula::oldmeta::kDefaultTable,
                                       nebula::oldmeta::kCurrJob, nebula::oldmeta::kJobArchive});
    std::unique_ptr<kvstore::KVIterator> iter;
      ...
      if (ret == kvstore::ResultCode::SUCCEEDED) {
        ...
        while (iter->valid()) {
          if (prefix == nebula::oldmeta::kIndexesTable) {
            ...
            status = upgrader.deleteKeyVal(MetaServiceUtils::indexIndexKey(spaceId, oldItem.get_index_name()));
            if (!status.ok()) {
              LOG(ERROR) << status;
              return status;
            }
