...
// static
std::string NebulaKeyUtils::vertexKey(size_t vIdLen, PartitionID partId, const VertexID& vId, TagID tagId) {
...
// static
std::string NebulaKeyUtils::edgeKey(size_t vIdLen, PartitionID partId, const VertexID& srcId, EdgeType type,
                                    EdgeRanking rank, const VertexID& dstId, EdgeVerPlaceHolder ev) {
...
std::string NebulaKeyUtils::kvKey(PartitionID partId, const folly::StringPiece& name) {
  ...
  key.append(reinterpret_cast<const char*>(&item), sizeof(int32_t)).append(name.data(), name.size());
  return key;
}

// static
std::string NebulaKeyUtils::vertexPrefix(size_t vIdLen, PartitionID partId, const VertexID& vId, TagID tagId) {
  CHECK_GE(vIdLen, vId.size());
...
// static
std::string NebulaKeyUtils::edgePrefix(size_t vIdLen, PartitionID partId, const VertexID& srcId, EdgeType type) {
  CHECK_GE(vIdLen, srcId.size());
