...
struct HostInfo {
  ...
  HostInfo(int64_t lastHBTimeInMilliSec, cpp2::HostRole role, std::string gitInfoSha)
      : lastHBTimeInMilliSec_(lastHBTimeInMilliSec), role_(role), gitInfoSha_(std::move(gitInfoSha)) {}

  bool operator==(const HostInfo& that) const { return this->lastHBTimeInMilliSec_ == that.lastHBTimeInMilliSec_; }
...
class ActiveHostsMan final {
 public:
  ...
  static kvstore::ResultCode updateHostInfo(kvstore::KVStore* kv, const HostAddr& hostAddr, const HostInfo& info,
  ...
  static std::vector<HostAddr> getActiveHostsInZone(kvstore::KVStore* kv, const std::string& zoneName,
                                                    int32_t expiredTTL = 0);

  static std::vector<HostAddr> getActiveHostsWithGroup(kvstore::KVStore* kv, GraphSpaceID spaceId,
                                                       int32_t expiredTTL = 0);

  static std::vector<HostAddr> getActiveAdminHosts(kvstore::KVStore* kv, int32_t expiredTTL = 0,
                                                   cpp2::HostRole role = cpp2::HostRole::STORAGE);
