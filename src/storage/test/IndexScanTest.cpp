...
std::string indexStr(RowReader* reader, const nebula::cpp2::ColumnDef& col) {
  ...
  switch (col.get_type().get_type()) {
    ...
    default:
      LOG(ERROR) << "Unknown type: " << static_cast<int32_t>(col.get_type().get_type());
  }
  return "";
}

IndexValues collectIndexValues(RowReader* reader, const std::vector<nebula::cpp2::ColumnDef>& cols) {
  IndexValues values;
  if (reader == nullptr) {
    return values;
  }
...
static std::string genEdgeIndexKey(meta::SchemaManager* schemaMan, const std::string& prop, GraphSpaceID spaceId,
                                   PartitionID partId, EdgeType type, std::shared_ptr<nebula::cpp2::IndexItem>& index,
...
static std::string genVertexIndexKey(meta::SchemaManager* schemaMan, const std::string& prop, GraphSpaceID spaceId,
                                     PartitionID partId, TagID tagId, std::shared_ptr<nebula::cpp2::IndexItem>& index,
                                     VertexID vId) {
...
static std::shared_ptr<meta::SchemaProviderIf> genEdgeSchemaProvider(int32_t intFieldsNum, int32_t stringFieldsNum) {
 ...
 * */
static std::shared_ptr<meta::SchemaProviderIf> genTagSchemaProvider(TagID tagId, int32_t intFieldsNum,
...
static std::unique_ptr<meta::SchemaManager> mockSchemaMan(GraphSpaceID spaceId, EdgeType edgeType, TagID tagId) {
  auto* schemaMan = new AdHocSchemaManager();
  schemaMan->addEdgeSchema(spaceId /*space id*/, edgeType /*edge type*/, genEdgeSchemaProvider(10, 10));
...
void mockData(kvstore::KVStore* kv, meta::SchemaManager* schemaMan, meta::IndexManager* indexMan, EdgeType edgeType,
              TagID tagId, GraphSpaceID spaceId) {
  ...
  for (auto partId = 0; partId < 3; partId++) {
    ...
    for (auto vertexId = partId * 10; vertexId < (partId + 1) * 10; vertexId++) {
      ...
      for (auto dstId = 10001; dstId <= 10007; dstId++) {
        ...
        auto ekey = NebulaKeyUtils::edgeKey(partId, vertexId, edgeType, 0, dstId, std::numeric_limits<int>::max() - 1);
        ...
        auto edgeIndex = genEdgeIndexKey(schemaMan, eval, spaceId, partId, edgeType, eindex, vertexId, dstId);
...
static std::unique_ptr<meta::IndexManager> mockIndexMan(GraphSpaceID spaceId = 0, TagID tagId = 3001,
...
static cpp2::LookUpVertexIndexResp execLookupVertices(const std::string& filter, bool hasReturnCols = true) {
  ...
  auto* processor = LookUpVertexIndexProcessor::instance(kv.get(), schemaMan.get(), indexMan.get(), nullptr);
...
static cpp2::LookUpEdgeIndexResp execLookupEdges(const std::string& filter, bool hasReturnCols = true) {
  ...
  auto indexMan = mockIndexMan(spaceId, tagId, type);
  mockData(kv.get(), schemaMan.get(), indexMan.get(), type, tagId, spaceId);
  auto* processor = LookUpEdgeIndexProcessor::instance(kv.get(), schemaMan.get(), indexMan.get(), nullptr);
  cpp2::LookUpIndexRequest req;
  decltype(req.parts) parts;
  parts.emplace_back(0);
  parts.emplace_back(1);
    ...
    for (auto partId = 0; partId < 3; partId++) {
      ...
      auto version = std::numeric_limits<int>::max() - 1;
      {
        ...
        ewriter << "AB"
                ...
                << "CABC";
        auto eval = ewriter.encode();
        auto edgeIndex = genEdgeIndexKey(schemaMan.get(), eval, spaceId, partId, type, eindex, srcId, dstId);
        data.emplace_back(std::move(edgeIndex), "");
        data.emplace_back(std::move(eKey), std::move(eval));
      }
      {
  ...
  sleep(FLAGS_raft_heartbeat_interval_secs);
  {
    for (auto partId = 0; partId < 3; partId++) {
      ...
      auto version = std::numeric_limits<int>::max() - 1;
      {
        ...
        auto vIndex = genVertexIndexKey(schemaMan.get(), tval, spaceId, partId, tagId, vindex, vertexId);
...
TEST(IndexScanTest, SimpleScanTest) {
  {
    ...
    auto relExp = std::make_unique<RelationalExpression>(aliaExp, RelationalExpression::Operator::EQ, priExp);
