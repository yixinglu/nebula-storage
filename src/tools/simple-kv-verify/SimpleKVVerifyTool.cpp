...
class SimpleKVVerifyTool {
 public:
  int32_t init() {
    ...
    auto ioExecutor = std::make_shared<folly::IOThreadPoolExecutor>(FLAGS_io_threads, std::move(threadFactory));
    meta::MetaClientOptions options;
    options.skipConfig_ = true;
    metaClient_ = std::make_unique<nebula::meta::MetaClient>(ioExecutor, std::move(addrs.value()), options);
    ...
    generalStorageClient_ = std::make_unique<storage::GeneralStorageClient>(ioExecutor, metaClient_.get());
    return EXIT_SUCCESS;
  }
  ...
  void runInsert(std::unordered_map<std::string, std::string>& data) {
    ...
    if (!resp.failedParts().empty()) {
      for (const auto& partEntry : resp.failedParts()) {
        LOG(ERROR) << "Put Failed in " << partEntry.first << ", Code: " << static_cast<int32_t>(partEntry.second);
      }
      return;
    }
      ...
      for (const auto& result : resp.responses()) {
        ...
        if (iter != result.key_values.end()) {
          if (iter->second != pairs[key]) {
            LOG(ERROR) << "Check Fail: key = " << key << ", values: " << iter->second << " != " << pairs[key];
            return;
          }
          found = true;
          break;
