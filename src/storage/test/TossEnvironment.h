...
struct TossEnvironment {
  ...
  std::unique_ptr<meta::MetaClient> setupMetaClient(const std::string& metaName, uint32_t metaPort) {
   ...
   * */
  void init(const std::string& spaceName, size_t part, int replica, std::vector<meta::cpp2::ColumnDef>& colDefs) {
    if (spaceId_ != 0) {
      return;
    }
    ...
    while (!leaderLoaded) {
      ...
      for (auto& leader : leaderMap) {
        LOG(INFO) << "spaceId=" << leader.first.first << ", part=" << leader.first.second << ", host=" << leader.second;
  ...
  EdgeType setupEdgeSchema(const std::string& edgeName, std::vector<meta::cpp2::ColumnDef> columns) {
  ...
  cpp2::NewEdge generateEdge(int srcId, int rank, std::vector<nebula::Value> values, int dstId = 0) {
    ...
    do {
      ...
      if (!f.value().succeeded()) {
        ...
        for (auto& part : f.value().failedParts()) {
          LOG(INFO) << "partId=" << part.first << ", ec=" << apache::thrift::util::enumNameSafe(part.second);
  ...
  folly::SemiFuture<StorageRpcResponse<cpp2::ExecResponse>> addEdgesAsync(const std::vector<cpp2::NewEdge>& edges,
                                                                          bool useToss = true) {
    ...
    do {
      ...
      if (!rpcResp.failedParts().empty()) {
        ...
        for (auto& p : rpcResp.failedParts()) {
          LOG(INFO) << "failedPart: " << p.first << ", err=" << apache::thrift::util::enumNameSafe(p.second);
      ...
      if (!fparts.empty()) {
        for (cpp2::PartitionResult& res : fparts) {
          LOG(INFO) << "part_id: " << res.part_id << ", part leader " << res.get_leader() << ", code "
  ...
  folly::SemiFuture<StorageRpcResponse<cpp2::GetNeighborsResponse>> getNeighborsWrapper(
      const std::vector<cpp2::NewEdge>& edges, int64_t limit = std::numeric_limits<int64_t>::max()) {
    // para3
    ...
    return sClient_->getNeighbors(spaceId_, colNames, vertices, edgeTypes, edgeDirection, statProps, vertexProps,
                                  &edgeProps, expressions, dedup, random, orderBy, limit);
  }
  ...
  std::vector<std::string> getNeiProps(const std::vector<cpp2::NewEdge>& _edges,
    ...
    auto last = std::unique(edges.begin(), edges.end(), [](const auto& a, const auto& b) {
      return a.get_key().get_src() == b.get_key().get_src() && a.get_key().get_dst() == b.get_key().get_dst();
    });
  ...
  std::vector<std::string> diffProps(std::vector<std::string> actual, std::vector<std::string> expect) {
    std::sort(actual.begin(), actual.end());
    std::sort(expect.begin(), expect.end());
    std::vector<std::string> diff;

    std::set_difference(actual.begin(), actual.end(), expect.begin(), expect.end(), std::inserter(diff, diff.begin()));
    return diff;
  }
  ...
  std::string encodeRowVal(const meta::NebulaSchemaProvider* schema, const std::vector<std::string>& propNames,
