...
class RaftexService : public cpp2::RaftexServiceSvIf {
 public:
  static std::shared_ptr<RaftexService> createService(std::shared_ptr<folly::IOThreadPoolExecutor> pool,
                                                      std::shared_ptr<folly::Executor> workers, uint16_t port = 0);
  ...
  void sendSnapshot(cpp2::SendSnapshotResponse& resp, const cpp2::SendSnapshotRequest& req) override;
 ...
 private:
  void initThriftServer(std::shared_ptr<folly::IOThreadPoolExecutor> pool, std::shared_ptr<folly::Executor> workers,
                        uint16_t port = 0);
  bool setup();
  void serve();
