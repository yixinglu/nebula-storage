...
class AdminSubTask {
 public:
  ...
  explicit AdminSubTask(std::function<kvstore::ResultCode()> f) {
    run_ = [f = f]() { return toStorageErr(f()); };
  }

  cpp2::ErrorCode invoke() { return run_(); }
...
class AdminTask {
  using TCallBack = std::function<void(cpp2::ErrorCode, nebula::meta::cpp2::StatisItem&)>;
 ...
 public:
  ...
  virtual cpp2::ErrorCode status() const { return rc_; }
