...
Status MetaDataUpgrade::rewriteHosts(const folly::StringPiece &key, const folly::StringPiece &val) {
  ...
  auto newKey = MetaServiceUtils::hostKeyV2(network::NetworkUtils::intToIPv4(host.get_ip()), host.get_port());
  NG_LOG_AND_RETURN_IF_ERROR(put(newKey, newVal));
  NG_LOG_AND_RETURN_IF_ERROR(remove(key));
  return Status::OK();
}

Status MetaDataUpgrade::rewriteLeaders(const folly::StringPiece &key, const folly::StringPiece &val) {
  auto host = oldmeta::MetaServiceUtilsV1::parseLeaderKey(key);
  auto newKey = MetaServiceUtils::leaderKey(network::NetworkUtils::intToIPv4(host.get_ip()), host.get_port());
  NG_LOG_AND_RETURN_IF_ERROR(put(newKey, val));
  NG_LOG_AND_RETURN_IF_ERROR(remove(key));
  return Status::OK();
}

Status MetaDataUpgrade::rewriteSpaces(const folly::StringPiece &key, const folly::StringPiece &val) {
...
Status MetaDataUpgrade::rewriteSchemas(const folly::StringPiece &key, const folly::StringPiece &val) {
  ...
  NG_LOG_AND_RETURN_IF_ERROR(convertToNewColumns((*oldSchema.columns_ref()), (*newSchema.columns_ref())));
...
Status MetaDataUpgrade::rewriteIndexes(const folly::StringPiece &key, const folly::StringPiece &val) {
  ...
  NG_LOG_AND_RETURN_IF_ERROR(convertToNewIndexColumns((*oldItem.fields_ref()), (*newItem.fields_ref())));
  NG_LOG_AND_RETURN_IF_ERROR(put(key, MetaServiceUtils::indexVal(newItem)));
  return Status::OK();
}

Status MetaDataUpgrade::rewriteConfigs(const folly::StringPiece &key, const folly::StringPiece &val) {
  ...
  auto newVal = MetaServiceUtils::configValue(static_cast<cpp2::ConfigMode>(item.get_mode()), configVal);
  NG_LOG_AND_RETURN_IF_ERROR(put(key, newVal));
  return Status::OK();
}

Status MetaDataUpgrade::rewriteJobDesc(const folly::StringPiece &key, const folly::StringPiece &val) {
  ...
  for (auto &para : paras) {
    ...
    str.append(reinterpret_cast<const char *>(&len), sizeof(len)).append(reinterpret_cast<const char *>(&para[0]), len);
  }
    ...
    if (colDef.default_value_ref().has_value()) {
      ...
      switch (colDef.get_type().get_type()) {
        case oldmeta::cpp2::SupportedType::BOOL:
          encodeStr = Expression::encode(ConstantExpression(colDef.get_default_value()->get_bool_value()));
          break;
        case oldmeta::cpp2::SupportedType::INT:
          encodeStr = Expression::encode(ConstantExpression(colDef.get_default_value()->get_int_value()));
          break;
        case oldmeta::cpp2::SupportedType::DOUBLE:
          encodeStr = Expression::encode(ConstantExpression(colDef.get_default_value()->get_double_value()));
          break;
        case oldmeta::cpp2::SupportedType::STRING:
          encodeStr = Expression::encode(ConstantExpression(colDef.get_default_value()->get_string_value()));
          break;
        case oldmeta::cpp2::SupportedType::TIMESTAMP:
          encodeStr = Expression::encode(ConstantExpression(colDef.get_default_value()->get_timestamp()));
          break;
        default:
...
Status MetaDataUpgrade::convertToNewIndexColumns(const std::vector<oldmeta::cpp2::ColumnDef> &oldCols,
                                                 std::vector<cpp2::ColumnDef> &newCols) {
  for (auto &colDef : oldCols) {
    cpp2::ColumnDef columnDef;
    columnDef.set_name(colDef.get_name());
...
void MetaDataUpgrade::printSchemas(const folly::StringPiece &val) {
  ...
  for (auto &colDef : oldSchema.get_columns()) {
    ...
    LOG(INFO) << "Schema column type: " << apache::thrift::util::enumNameSafe(colDef.get_type().get_type());
    Value defaultValue;
    if (colDef.default_value_ref().has_value()) {
      switch (colDef.get_type().get_type()) {
        case oldmeta::cpp2::SupportedType::BOOL:
        ...
        case oldmeta::cpp2::SupportedType::TIMESTAMP:
          ...
          break;
        default:
          LOG(ERROR) << "Wrong default type: " << apache::thrift::util::enumNameSafe(colDef.get_type().get_type());
      }
...
void MetaDataUpgrade::printIndexes(const folly::StringPiece &val) {
  ...
  for (auto &colDef : oldItem.get_fields()) {
    ...
    LOG(INFO) << "Index field type: " << apache::thrift::util::enumNameSafe(colDef.get_type().get_type());
  }
}
