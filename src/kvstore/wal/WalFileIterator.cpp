...
WalFileIterator::WalFileIterator(std::shared_ptr<FileBasedWal> wal, LogID startId, LogID lastId)
  ...
  if (currId_ > lastId_) {
    LOG(ERROR) << wal_->idStr_ << "The log " << currId_ << " is out of range, the lastLogId is " << lastId_;
    return;
  }

  if (startId < wal_->firstLogId()) {
    LOG(ERROR) << wal_->idStr_ << "The given log id " << startId << " is out of the range, the wal firstLogId is "
               << wal_->firstLogId();
    currId_ = lastId_ + 1;
    return;
  }
  ...
  wal_->accessAllWalInfo([this](WalFileInfoPtr info) {
    ...
    if (fd < 0) {
      LOG(ERROR) << "Failed to open wal file \"" << info->path() << "\" (" << errno << "): " << strerror(errno);
      currId_ = lastId_ + 1;
      return false;
    }
    fds_.push_front(fd);
  ...
  if (currId_ > idRanges_.front().second) {
    LOG(FATAL) << wal_->idStr_ << "currId_ " << currId_ << ", idRanges.front firstLogId " << idRanges_.front().first
               << ", idRanges.front lastLogId " << idRanges_.front().second << ", idRanges size " << idRanges_.size()
               << ", lastId_ " << lastId_ << ", nextFirstId_ " << nextFirstId_;
  }
  ...
  if (!idRanges_.empty()) {
    ...
    while (true) {
      ...
      // Read the termID
      CHECK_EQ(pread(fd, reinterpret_cast<char*>(&currTerm_), sizeof(TermID), currPos_ + sizeof(LogID)),
               static_cast<ssize_t>(sizeof(TermID)));
      // Read the log length
      CHECK_EQ(
          pread(fd, reinterpret_cast<char*>(&currMsgLen_), sizeof(int32_t), currPos_ + sizeof(LogID) + sizeof(TermID)),
          static_cast<ssize_t>(sizeof(int32_t)));
      if (logId == currId_) {
        break;
      }
      currPos_ += sizeof(LogID) + sizeof(TermID) + sizeof(int32_t) * 2 + currMsgLen_ + sizeof(ClusterID);
    }
  }
}
...
LogIterator& WalFileIterator::operator++() {
  ...
  if (currId_ >= nextFirstId_) {
    ...
    VLOG(2) << "Current ID is " << currId_ << ", and the first ID in the next file is " << nextFirstId_
  ...
  if (idRanges_.front().second <= 0) {
    ...
    int fd = fds_.front();
    // Read the logID
    CHECK_EQ(pread(fd, reinterpret_cast<char*>(&logId), sizeof(LogID), currPos_), static_cast<ssize_t>(sizeof(LogID)))
        << "currPos = " << currPos_;
    ...
    // Read the log length
    CHECK_EQ(
        pread(fd, reinterpret_cast<char*>(&currMsgLen_), sizeof(int32_t), currPos_ + sizeof(TermID) + sizeof(LogID)),
        static_cast<ssize_t>(sizeof(int32_t)));
  }
...
ClusterID WalFileIterator::logSource() const {
  ...
  ClusterID cluster = 0;
  CHECK_EQ(
      pread(fds_.front(), &(cluster), sizeof(ClusterID), currPos_ + sizeof(LogID) + sizeof(TermID) + sizeof(int32_t)),
      static_cast<ssize_t>(sizeof(ClusterID)))
      << "Failed to read. Curr position is " << currPos_ << ", expected read length is " << sizeof(ClusterID)
...
folly::StringPiece WalFileIterator::logMsg() const {
  ...
  CHECK_EQ(pread(fds_.front(), &(currLog_[0]), currMsgLen_,
                 currPos_ + sizeof(LogID) + sizeof(TermID) + sizeof(int32_t) + sizeof(ClusterID)),
           static_cast<ssize_t>(currMsgLen_))
      << "Failed to read. Curr position is " << currPos_ << ", expected read length is " << currMsgLen_
      << " (errno: " << errno << "): " << strerror(errno);
