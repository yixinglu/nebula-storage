...
class GetEdgeIndexProcessor : public BaseProcessor<cpp2::GetEdgeIndexResp> {
 public:
  static GetEdgeIndexProcessor* instance(kvstore::KVStore* kvstore) { return new GetEdgeIndexProcessor(kvstore); }

  void process(const cpp2::GetEdgeIndexReq& req);

 private:
  explicit GetEdgeIndexProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::GetEdgeIndexResp>(kvstore) {}
};
