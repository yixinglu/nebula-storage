...
class JobDescription {
 ...
 public:
  ...
  JobDescription(JobID id, cpp2::AdminCmd cmd, std::vector<std::string> paras, Status status = Status::QUEUE,
                 int64_t startTime = 0, int64_t stopTime = 0);

  /*
   * return the JobDescription if both key & val is valid
   * */
  static folly::Optional<JobDescription> makeJobDescription(folly::StringPiece key, folly::StringPiece val);
   ...
   * */
  static folly::Optional<JobDescription> loadJobDescription(JobID iJob, nebula::kvstore::KVStore* kv);
