...
class RemoveRangeProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static RemoveRangeProcessor* instance(kvstore::KVStore* kvstore) { return new RemoveRangeProcessor(kvstore); }

  void process(const cpp2::RemoveRangeReq& req);

 private:
  explicit RemoveRangeProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
