 ...
 *
 ******************************************************/
std::shared_ptr<RaftexService> RaftexService::createService(std::shared_ptr<folly::IOThreadPoolExecutor> pool,
                                                            std::shared_ptr<folly::Executor> workers, uint16_t port) {
...
void RaftexService::initThriftServer(std::shared_ptr<folly::IOThreadPoolExecutor> pool,
  ...
  if (workers != nullptr) {
    server_->setThreadManager(std::dynamic_pointer_cast<apache::thrift::concurrency::ThreadManager>(workers));
  }
...
std::shared_ptr<folly::Executor> RaftexService::getThreadManager() { return server_->getThreadManager(); }
...
void RaftexService::waitUntilStop() {
  if (serverThread_) {
    ...
    serverThread_.reset();
    server_.reset();
    LOG(INFO) << "Server thread has stopped. Service on port " << serverPort_ << " is ready to be destroyed";
  }
}
...
void RaftexService::sendSnapshot(cpp2::SendSnapshotResponse& resp, const cpp2::SendSnapshotRequest& req) {
  auto part = findPart(req.get_space(), req.get_part());
  if (!part) {
    // Not found
    resp.set_error_code(cpp2::ErrorCode::E_UNKNOWN_PART);
