  ...
  if (!isEdge_) {
    ...
    for (const auto& partEntry : req.get_parts()) {
      ...
      for (const auto& row : partEntry.second) {
        ...
        if (ret != kvstore::ResultCode::SUCCEEDED && failedParts.find(partId) == failedParts.end()) {
...
StoragePlan<VertexID> GetPropProcessor::buildTagPlan(nebula::DataSet* result) {
  ...
  auto output = std::make_unique<GetTagPropNode>(planContext_.get(), tags, result, tagContext_.vertexCache_);
  for (auto* tag : tags) {
...
StoragePlan<cpp2::EdgeKey> GetPropProcessor::buildEdgePlan(nebula::DataSet* result) {
  ...
  for (const auto& ec : edgeContext_.propContexts_) {
    auto edge = std::make_unique<FetchEdgeNode>(planContext_.get(), &edgeContext_, ec.first, &ec.second);
