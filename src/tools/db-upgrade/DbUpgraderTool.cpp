...
int main(int argc, char* argv[]) {
  ...
  std::transform(srcPaths.begin(), srcPaths.end(), srcPaths.begin(),
                 [](auto& p) { return folly::trimWhitespace(p).str(); });
  if (srcPaths.empty()) {
  ...
  std::transform(dstPaths.begin(), dstPaths.end(), dstPaths.begin(),
                 [](auto& p) { return folly::trimWhitespace(p).str(); });
  if (dstPaths.empty()) {
  ...
  auto metaClient = std::make_unique<nebula::meta::MetaClient>(ioExecutor, std::move(addrs.value()), options);
  ...
  if (FLAGS_upgrade_version != 1 && FLAGS_upgrade_version != 2) {
    LOG(ERROR) << "Flag upgrade_version : " << FLAGS_upgrade_version << " illegal, upgrade_version can only be 1 or 2";
    return EXIT_FAILURE;
  }
  ...
  for (size_t i = 0; i < srcPaths.size(); i++) {
    threads.emplace_back(std::thread([mclient = metaClient.get(), sMan = schemaMan.get(), iMan = indexMan.get(),
                                      srcPath = srcPaths[i], dstPath = dstPaths[i]] {
