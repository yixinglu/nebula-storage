...
cpp2::ErrorCode StatisTask::getSchemas(GraphSpaceID spaceId) {
...
ErrorOr<cpp2::ErrorCode, std::vector<AdminSubTask>> StatisTask::genSubTasks() {
  ...
  for (const auto& part : parts) {
    std::function<kvstore::ResultCode()> task = std::bind(&StatisTask::genSubTask, this, spaceId_, part, tags_, edges_);
...
kvstore::ResultCode StatisTask::genSubTask(GraphSpaceID spaceId, PartitionID part,
...
void StatisTask::finish(cpp2::ErrorCode rc) {
  FLOG_INFO("task(%d, %d) finished, rc=[%d]", ctx_.jobId_, ctx_.taskId_, static_cast<int>(rc));
    ...
    for (auto& elem : statistics_) {
      ...
      *result.space_edges_ref() += *item.space_edges_ref();
      ...
      (*result.positive_part_correlativity_ref())
          .insert((*item.positive_part_correlativity_ref()).begin(), (*item.positive_part_correlativity_ref()).end());
      (*result.negative_part_correlativity_ref())
          .insert((*item.negative_part_correlativity_ref()).begin(), (*item.negative_part_correlativity_ref()).end());
    }
