...
class KVCompactionFilter final : public rocksdb::CompactionFilter {
 public:
  ...
  bool Filter(int, const rocksdb::Slice& key, const rocksdb::Slice& val, std::string*, bool*) const override {
    return kvFilter_->filter(spaceId_, folly::StringPiece(key.data(), key.size()),
