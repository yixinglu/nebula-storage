...
class LeaderBalanceProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static LeaderBalanceProcessor* instance(kvstore::KVStore* kvstore) { return new LeaderBalanceProcessor(kvstore); }

  void process(const cpp2::LeaderBalanceReq& req);

 private:
  explicit LeaderBalanceProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
