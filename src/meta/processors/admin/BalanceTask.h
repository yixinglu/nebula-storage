...
class BalanceTask {
 ...
 public:
  ...
  BalanceTask(BalanceID balanceId, GraphSpaceID spaceId, PartitionID partId, const HostAddr& src, const HostAddr& dst,
              kvstore::KVStore* kv, AdminClient* client)
 ...
 private:
  std::string buildTaskId() {
    return folly::stringPrintf("[%ld, %d:%d, %s:%d->%s:%d]", balanceId_, spaceId_, partId_, src_.host.c_str(),
                               src_.port, dst_.host.c_str(), dst_.port);
  }
