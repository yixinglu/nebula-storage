...
bool ESListener::apply(const std::vector<KV>& data) {
  ...
  for (const auto& kv : data) {
    if (!nebula::NebulaKeyUtils::isVertex(vIdLen_, kv.first) && !nebula::NebulaKeyUtils::isEdge(vIdLen_, kv.first)) {
      continue;
    }
...
std::pair<LogID, TermID> ESListener::lastCommittedLogId() {
  ...
  LogID logId;
  CHECK_EQ(pread(fd, reinterpret_cast<char*>(&logId), sizeof(LogID), 0), static_cast<ssize_t>(sizeof(LogID)));
...
LogID ESListener::lastApplyLogId() {
  ...
  CHECK_EQ(pread(fd, reinterpret_cast<char*>(&logId), sizeof(LogID), offset), static_cast<ssize_t>(sizeof(LogID)));
  close(fd);
  return logId;
}
...
bool ESListener::writeAppliedId(LogID lastId, TermID lastTerm, LogID lastApplyLogId) {
  ...
  if (written != (ssize_t)raw.size()) {
    VLOG(3) << idStr_ << "bytesWritten:" << written << ", expected:" << raw.size() << ", error:" << strerror(errno);
    close(fd);
    return false;
  }
  close(fd);
...
std::string ESListener::encodeAppliedId(LogID lastId, TermID lastTerm, LogID lastApplyLogId) const noexcept {
  std::string val;
...
bool ESListener::appendDocs(std::vector<DocItem>& items, const meta::SchemaProviderIf* schema, RowReader* reader,
                            int32_t schemaId, bool isEdge) const {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    items.emplace_back(DocItem(std::move(ftIndex), std::move(name), partId_, schemaId, std::move(v).getStr()));
  }
  return true;
}
