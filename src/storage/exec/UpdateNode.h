...
class UpdateNode : public RelNode<T> {
 public:
  UpdateNode(PlanContext* planCtx, std::vector<std::shared_ptr<nebula::meta::cpp2::IndexItem>> indexes,
             std::vector<storage::cpp2::UpdatedProp>& updatedProps, FilterNode<T>* filterNode, bool insertable,
             std::vector<std::pair<std::string, std::unordered_set<std::string>>> depPropMap,
             StorageExpressionContext* expCtx, bool isEdge)
  ...
  kvstore::ResultCode getDefaultOrNullValue(const meta::SchemaProviderIf::Field* field, const std::string& name) {
    if (field->hasDefault()) {
...
class UpdateTagNode : public UpdateNode<VertexID> {
 public:
  ...
  UpdateTagNode(PlanContext* planCtx, std::vector<std::shared_ptr<nebula::meta::cpp2::IndexItem>> indexes,
                std::vector<storage::cpp2::UpdatedProp>& updatedProps, FilterNode<VertexID>* filterNode,
                bool insertable, std::vector<std::pair<std::string, std::unordered_set<std::string>>> depPropMap,
                StorageExpressionContext* expCtx, TagContext* tagContext)
      : UpdateNode<VertexID>(planCtx, indexes, updatedProps, filterNode, insertable, depPropMap, expCtx, false),
  ...
  kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
    ...
    std::vector<VMLI> dummyLock = {std::make_tuple(planContext_->spaceId_, partId, tagId_, vId)};
    nebula::MemoryLockGuard<VMLI> lg(planContext_->env_->verticesML_.get(), std::move(dummyLock));
    if (!lg) {
    ...
    planContext_->env_->kvstore_->asyncAppendBatch(planContext_->spaceId_, partId, std::move(batch).value(), callback);
    baton.wait();
    return ret;
  }
  ...
  kvstore::ResultCode getLatestTagSchemaAndName() {
    ...
    if (iter == tagContext_->tagNames_.end()) {
      VLOG(1) << "Can't find spaceId " << planContext_->spaceId_ << " tagId " << tagId_;
      return kvstore::ResultCode::ERR_TAG_NOT_FOUND;
    }
      ...
      for (auto& index : indexes_) {
        if (tagId_ == index->get_schema_id().get_tag_id()) {
          ...
          if (!val_.empty()) {
            ...
            if (!oi.empty()) {
              auto iState = planContext_->env_->getIndexState(planContext_->spaceId_, partId);
          ...
          if (!nReader) {
            nReader = RowReaderWrapper::getTagPropReader(planContext_->env_->schemaMan_, planContext_->spaceId_, tagId_,
                                                         nVal);
          }
          if (!nReader) {
            LOG(ERROR) << "Bad format row";
          ...
          if (!ni.empty()) {
            auto indexState = planContext_->env_->getIndexState(planContext_->spaceId_, partId);
  ...
  std::string indexKey(PartitionID partId, const VertexID& vId, RowReader* reader,
    ...
    return IndexKeyUtils::vertexIndexKey(planContext_->vIdLen_, partId, index->get_index_id(), vId,
...
class UpdateEdgeNode : public UpdateNode<cpp2::EdgeKey> {
 public:
  ...
  UpdateEdgeNode(PlanContext* planCtx, std::vector<std::shared_ptr<nebula::meta::cpp2::IndexItem>> indexes,
                 std::vector<storage::cpp2::UpdatedProp>& updatedProps, FilterNode<cpp2::EdgeKey>* filterNode,
                 bool insertable, std::vector<std::pair<std::string, std::unordered_set<std::string>>> depPropMap,
                 StorageExpressionContext* expCtx, EdgeContext* edgeContext)
      : UpdateNode<cpp2::EdgeKey>(planCtx, indexes, updatedProps, filterNode, insertable, depPropMap, expCtx, true),
  ...
  kvstore::ResultCode execute(PartitionID partId, const cpp2::EdgeKey& edgeKey) override {
    ...
    if (planContext_->env_->txnMan_ && planContext_->env_->txnMan_->enableToss(planContext_->spaceId_)) {
      LOG(INFO) << "before update edge atomic" << TransactionUtils::dumpKey(edgeKey);
      auto f = planContext_->env_->txnMan_->updateEdgeAtomic(planContext_->vIdLen_, planContext_->spaceId_, partId,
                                                             edgeKey, std::move(op));
      f.wait();
      ...
      std::vector<EMLI> dummyLock = {std::make_tuple(planContext_->spaceId_, partId, edgeKey.get_src().getStr(),
                                                     edgeKey.get_edge_type(), edgeKey.get_ranking(),
                                                     edgeKey.get_dst().getStr())};
      nebula::MemoryLockGuard<EMLI> lg(planContext_->env_->edgesML_.get(), std::move(dummyLock));
      if (!lg) {
        auto conflict = lg.conflictKey();
        LOG(ERROR) << "edge conflict " << std::get<0>(conflict) << ":" << std::get<1>(conflict) << ":"
                   << std::get<2>(conflict) << ":" << std::get<3>(conflict) << ":" << std::get<4>(conflict) << ":"
                   << std::get<5>(conflict);
        return kvstore::ResultCode::ERR_DATA_CONFLICT_ERROR;
      }
      folly::Baton<true, std::atomic> baton;
      ...
      planContext_->env_->kvstore_->asyncAppendBatch(planContext_->spaceId_, partId, std::move(batch).value(),
                                                     callback);
      baton.wait();
    }
    return ret;
  ...
  kvstore::ResultCode getLatestEdgeSchemaAndName() {
    ...
    if (iter == edgeContext_->edgeNames_.end()) {
      VLOG(1) << "Can't find spaceId " << planContext_->spaceId_ << " edgeType " << edgeType_;
      return kvstore::ResultCode::ERR_EDGE_NOT_FOUND;
    }
  ...
  kvstore::ResultCode insertEdgeProps(const PartitionID partId, const cpp2::EdgeKey& edgeKey) {
    ...
    key_ = NebulaKeyUtils::edgeKey(planContext_->vIdLen_, partId, edgeKey.get_src().getStr(), edgeKey.get_edge_type(),
                                   edgeKey.get_ranking(), edgeKey.get_dst().getStr());
    rowWriter_ = std::make_unique<RowWriterV2>(schema_);
  ...
  folly::Optional<std::string> updateAndWriteBack(const PartitionID partId, const cpp2::EdgeKey& edgeKey) {
    for (auto& updateProp : updatedProps_) {
      auto propName = updateProp.get_name();
      auto updateExp = Expression::decode(updateProp.get_value());
      if (!updateExp) {
    ...
    if (!indexes_.empty()) {
      ...
      for (auto& index : indexes_) {
        if (edgeType_ == index->get_schema_id().get_edge_type()) {
          ...
          if (!nReader) {
            nReader = RowReaderWrapper::getEdgePropReader(planContext_->env_->schemaMan_, planContext_->spaceId_,
  ...
  std::string indexKey(PartitionID partId, RowReader* reader, const cpp2::EdgeKey& edgeKey,
                       std::shared_ptr<nebula::meta::cpp2::IndexItem> index) {
    auto values = IndexKeyUtils::collectIndexValues(reader, index->get_fields());
    if (!values.ok()) {
      return "";
    }
    return IndexKeyUtils::edgeIndexKey(planContext_->vIdLen_, partId, index->get_index_id(), edgeKey.get_src().getStr(),
                                       edgeKey.get_ranking(), edgeKey.get_dst().getStr(), std::move(values).value());
  }
