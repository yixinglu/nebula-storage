 ...
 public:
  ...
  void OnCompactionBegin(rocksdb::DB*, const rocksdb::CompactionJobInfo& info) override {
    LOG(INFO) << "Rocksdb start compaction column family: " << info.cf_name << " because of "
              << compactionReasonString(info.compaction_reason) << ", status: " << info.status.ToString()
              << ", compacted " << info.input_files.size() << " files into " << info.output_files.size()
              << ", base level is " << info.base_input_level << ", output level is " << info.output_level;
  }
  ...
  void OnFlushBegin(rocksdb::DB*, const rocksdb::FlushJobInfo& info) override {
    VLOG(1) << "Rocksdb start flush column family: " << info.cf_name << " because of "
            ...
            << ", the smallest sequence number is " << info.smallest_seqno << ", the largest sequence number is "
            << info.largest_seqno << ", the properties of the table: " << info.table_properties.ToString();
  }
  ...
  void OnTableFileCreated(const rocksdb::TableFileCreationInfo& info) override {
    VLOG(3) << "Rocksdb SST file created: the path is " << info.file_path << " the file size is " << info.file_size;
  }
  ...
  void OnTableFileCreationStarted(const rocksdb::TableFileCreationBriefInfo& info) override {
    VLOG(3) << " database's name is " << info.db_name << ", column family's name is " << info.cf_name
            << ", the created file is " << info.file_path << ", because of "
  ...
  void OnExternalFileIngested(rocksdb::DB*, const rocksdb::ExternalFileIngestionInfo& info) override {
    LOG(INFO) << "Ingest external SST file: column family " << info.cf_name << ", the external file path "
              << info.external_file_path << ", the internal file path " << info.internal_file_path
  ...
  void OnErrorRecoveryBegin(rocksdb::BackgroundErrorReason reason, rocksdb::Status /* bg_error */,
  ...
  void OnErrorRecoveryCompleted(rocksdb::Status) override { LOG(INFO) << "Error Recovery Completed"; }
