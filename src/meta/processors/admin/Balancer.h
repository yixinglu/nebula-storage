...
class Balancer {
 ...
 private:
  ...
  ErrorOr<cpp2::ErrorCode, std::vector<BalanceTask>> genTasks(GraphSpaceID spaceId, int32_t spaceReplica,
                                                              bool dependentOnGroup, std::vector<HostAddr>&& lostHosts);

  std::pair<HostParts, std::vector<HostAddr>> fetchHostParts(GraphSpaceID spaceId, bool dependentOnGroup,
                                                             const HostParts& hostParts,
                                                             std::vector<HostAddr>& lostHosts);

  bool getHostParts(GraphSpaceID spaceId, bool dependentOnGroup, HostParts& hostParts, int32_t& totalParts);

  bool assembleZoneParts(const std::string& groupName, HostParts& hostParts);

  void calDiff(const HostParts& hostParts, const std::vector<HostAddr>& activeHosts, std::vector<HostAddr>& newlyAdded,
               std::vector<HostAddr>& lost);

  Status checkReplica(const HostParts& hostParts, const std::vector<HostAddr>& activeHosts, int32_t replica,
                      PartitionID partId);

  StatusOr<HostAddr> hostWithMinimalParts(const HostParts& hostParts, PartitionID partId);

  StatusOr<HostAddr> hostWithMinimalPartsForZone(const HostAddr& source, const HostParts& hostParts,
                                                 PartitionID partId);
  bool balanceParts(BalanceID balanceId, GraphSpaceID spaceId, HostParts& newHostParts, int32_t totalParts,
                    std::vector<BalanceTask>& tasks);

  bool transferLostHost(std::vector<BalanceTask>& tasks, HostParts& newHostParts, const HostAddr& source,
  ...
  bool buildLeaderBalancePlan(HostLeaderMap* hostLeaderMap, GraphSpaceID spaceId, int32_t replicaFactor,
                              bool dependentOnGroup, LeaderBalancePlan& plan, bool useDeviation = true);

  void simplifyLeaderBalnacePlan(GraphSpaceID spaceId, LeaderBalancePlan& plan);

  int32_t acquireLeaders(HostParts& allHostParts, HostParts& leaderHostParts, PartAllocation& peersMap,
                         std::unordered_set<HostAddr>& activeHosts, const HostAddr& target, LeaderBalancePlan& plan,
                         GraphSpaceID spaceId);

  int32_t giveupLeaders(HostParts& leaderHostParts, PartAllocation& peersMap, std::unordered_set<HostAddr>& activeHosts,
                        const HostAddr& source, LeaderBalancePlan& plan, GraphSpaceID spaceId);
