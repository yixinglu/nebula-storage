...
using TagSchemas = std::unordered_map<TagID, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>>;

using TagSchema = nebula::meta::TagSchema;
// the different version of edge schema, from oldest to newest
using EdgeSchemas = std::unordered_map<EdgeType, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>>;
...
class AdHocSchemaManager final : public nebula::meta::SchemaManager {
 public:
  ...
  void addTagSchema(GraphSpaceID space, TagID tag, std::shared_ptr<nebula::meta::NebulaSchemaProvider> schema);

  void addEdgeSchema(GraphSpaceID space, EdgeType edge, std::shared_ptr<nebula::meta::NebulaSchemaProvider> schema);

  void removeTagSchema(GraphSpaceID space, TagID tag);

  std::shared_ptr<const nebula::meta::NebulaSchemaProvider> getTagSchema(GraphSpaceID space, TagID tag,
  ...
  std::shared_ptr<const nebula::meta::NebulaSchemaProvider> getEdgeSchema(GraphSpaceID space, EdgeType edge,
                                                                          SchemaVer version = -1) override;
  ...
  // benchmark
  std::shared_ptr<const nebula::meta::NebulaSchemaProvider> getTagSchemaFromMap(GraphSpaceID space, TagID tag,
                                                                                SchemaVer ver);

  // Mock previous version of get schema from cache in MetaClient, only used of
  // benchmark
  std::shared_ptr<const nebula::meta::NebulaSchemaProvider> getEdgeSchemaFromMap(GraphSpaceID space, EdgeType edge,
                                                                                 SchemaVer ver);
 ...
 private:
  ...
  std::unordered_map<GraphSpaceID, std::unordered_map<std::pair<TagID, SchemaVer>,
  ...
  // MetaClient cache Only used for benchmark comparison
  std::unordered_map<GraphSpaceID, std::unordered_map<std::pair<EdgeType, SchemaVer>,
                                                      std::shared_ptr<const nebula::meta::NebulaSchemaProvider>>>
      edgeSchemasInMap_;
