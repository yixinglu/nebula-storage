...
class GetTagIndexProcessor : public BaseProcessor<cpp2::GetTagIndexResp> {
 public:
  static GetTagIndexProcessor* instance(kvstore::KVStore* kvstore) { return new GetTagIndexProcessor(kvstore); }

  void process(const cpp2::GetTagIndexReq& req);

 private:
  explicit GetTagIndexProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::GetTagIndexResp>(kvstore) {}
};
