...
cpp2::GetNeighborsRequest buildRequest(const std::vector<VertexID>& vertex, const std::vector<std::string>& playerProps,
...
void initContext(std::unique_ptr<nebula::storage::PlanContext>& planCtx, nebula::storage::EdgeContext& edgeContext,
                 const std::vector<std::string>& serveProps) {
...
void go(int32_t iters, const std::vector<nebula::VertexID>& vertex, const std::vector<std::string>& playerProps,
        const std::vector<std::string>& serveProps) {
...
void goFilter(int32_t iters, const std::vector<nebula::VertexID>& vertex, const std::vector<std::string>& playerProps,
              const std::vector<std::string>& serveProps, int64_t value = FLAGS_max_rank * FLAGS_filter_ratio,
              bool oneFilter = true) {
  nebula::storage::cpp2::GetNeighborsRequest req;
  BENCHMARK_SUSPEND {
    nebula::EdgeType serve = 101;
    ...
    if (oneFilter) {
      ...
      nebula::RelationalExpression exp(
          ...
          new nebula::EdgePropertyExpression(new std::string(folly::to<std::string>(serve)),
...
void goEdgeNode(int32_t iters, const std::vector<nebula::VertexID>& vertex, const std::vector<std::string>& playerProps,
                const std::vector<std::string>& serveProps) {
  UNUSED(playerProps);
  ...
  BENCHMARK_SUSPEND {
    ...
    const auto& ec = edgeContext.propContexts_.front();
    edgeNode = std::make_unique<nebula::storage::SingleEdgeNode>(planCtx.get(), &edgeContext, ec.first, &ec.second);
  }
  auto totalParts = gCluster->getTotalParts();
  for (decltype(iters) i = 0; i < iters; i++) {
...
void prefix(int32_t iters, const std::vector<nebula::VertexID>& vertex, const std::vector<std::string>& playerProps,
            const std::vector<std::string>& serveProps) {
  std::unique_ptr<nebula::storage::PlanContext> planCtx;
  nebula::storage::EdgeContext edgeContext;
  BENCHMARK_SUSPEND { initContext(planCtx, edgeContext, serveProps); }
  for (decltype(iters) i = 0; i < iters; i++) {
    nebula::storage::cpp2::GetNeighborsResponse resp;
    nebula::DataSet resultDataSet;
    ...
    for (const auto& vId : vertex) {
      ...
      }
      {
        ...
        for (; iter->valid(); iter->next()) {
          ...
          auto props = planCtx->props_;
          for (const auto& prop : *props) {
            auto value = nebula::storage::QueryUtils::readValue(reader.get(), prop.name_, prop.field_);
            CHECK(value.ok());
            list.emplace_back(std::move(value).value());
          }
          cell.values.emplace_back(std::move(list));
...
void encodeBench(int32_t iters, const std::vector<nebula::VertexID>& vertex,
                 const std::vector<std::string>& playerProps, const std::vector<std::string>& serveProps) {
  nebula::storage::cpp2::GetNeighborsResponse resp;
  BENCHMARK_SUSPEND {
    auto* env = gCluster->storageEnv_.get();
...
BENCHMARK_RELATIVE(EncodeFiveProperty, iters) {
  encodeBench(iters, {"Tim Duncan"}, {"name"}, {"teamName", "startYear", "endYear", "teamCareer", "teamGames"});
}
...
BENCHMARK_RELATIVE(OneVertexFiveProperty, iters) {
  go(iters, {"Tim Duncan"}, {"name"}, {"teamName", "startYear", "endYear", "teamCareer", "teamGames"});
}
BENCHMARK_RELATIVE(OneVertexOnePropertyOnlyEdgeNode, iters) {
  goEdgeNode(iters, {"Tim Duncan"}, {"name"}, {"teamName"});
}
BENCHMARK_RELATIVE(OneVertexOneProperyOnlyKV, iters) { prefix(iters, {"Tim Duncan"}, {"name"}, {"teamName"}); }
...
BENCHMARK(TenVertexOneProperty, iters) {
  go(iters,
     {"Tim Duncan", "Kobe Bryant", "Stephen Curry", "Manu Ginobili", "Joel Embiid", "Giannis Antetokounmpo", "Yao Ming",
      "Damian Lillard", "Dirk Nowitzki", "Klay Thompson"},
     {"name"}, {"teamName"});
}
...
BENCHMARK_RELATIVE(TenVertexThreeProperty, iters) {
  go(iters,
     {"Tim Duncan", "Kobe Bryant", "Stephen Curry", "Manu Ginobili", "Joel Embiid", "Giannis Antetokounmpo", "Yao Ming",
      "Damian Lillard", "Dirk Nowitzki", "Klay Thompson"},
     {"name"}, {"teamName", "startYear", "endYear"});
}
BENCHMARK_RELATIVE(TenVertexFiveProperty, iters) {
  go(iters,
     {"Tim Duncan", "Kobe Bryant", "Stephen Curry", "Manu Ginobili", "Joel Embiid", "Giannis Antetokounmpo", "Yao Ming",
      "Damian Lillard", "Dirk Nowitzki", "Klay Thompson"},
     {"name"}, {"teamName", "startYear", "endYear", "teamCareer", "teamGames"});
}
BENCHMARK_RELATIVE(TenVertexOnePropertyOnlyEdgeNode, iters) {
  goEdgeNode(iters,
             {"Tim Duncan", "Kobe Bryant", "Stephen Curry", "Manu Ginobili", "Joel Embiid", "Giannis Antetokounmpo",
              "Yao Ming", "Damian Lillard", "Dirk Nowitzki", "Klay Thompson"},
             {"name"}, {"teamName"});
}
