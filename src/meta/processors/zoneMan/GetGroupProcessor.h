...
class GetGroupProcessor : public BaseProcessor<cpp2::GetGroupResp> {
 public:
  static GetGroupProcessor* instance(kvstore::KVStore* kvstore) { return new GetGroupProcessor(kvstore); }

  void process(const cpp2::GetGroupReq& req);

 private:
  explicit GetGroupProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::GetGroupResp>(kvstore) {}
};
