...
class SignInFTServiceProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static SignInFTServiceProcessor* instance(kvstore::KVStore* kvstore) { return new SignInFTServiceProcessor(kvstore); }

  void process(const cpp2::SignInFTServiceReq& req);

 private:
  explicit SignInFTServiceProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
...
class SignOutFTServiceProcessor : public BaseProcessor<cpp2::ExecResp> {
 ...
 private:
  explicit SignOutFTServiceProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};

class ListFTClientsProcessor : public BaseProcessor<cpp2::ListFTClientsResp> {
 public:
  static ListFTClientsProcessor* instance(kvstore::KVStore* kvstore) { return new ListFTClientsProcessor(kvstore); }

  void process(const cpp2::ListFTClientsReq& req);

 private:
  explicit ListFTClientsProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListFTClientsResp>(kvstore) {}
};
}  // namespace meta
}  // namespace nebula
