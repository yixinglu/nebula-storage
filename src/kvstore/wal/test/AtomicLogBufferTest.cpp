...
void checkIterator(std::shared_ptr<AtomicLogBuffer> logBuffer, LogID from, LogID to, LogID expected) {
      ...
      while (times-- > 0) {
        ...
        for (; iter->valid(); ++(*iter)) {
          ...
          EXPECT_EQ(expected.size(), rec->msg_.size())
              << "wp " << wp << ", start " << start << ", logId " << logId << ", end " << end << ", curr node "
              << node->firstLogId_ << ", pos " << node->pos_ << ", curr index " << iter->currIndex()
              << ", head lastLodId " << logBuffer->lastLogId();
          EXPECT_EQ(expected, rec->msg_) << "expected size " << expected.size() << ", actual size " << rec->msg_.size();
          num++;
        }
      }
