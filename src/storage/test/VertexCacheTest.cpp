...
void getVertices(storage::StorageEnv* env, int totalParts, VertexCache* cache, TagID tagId,
                 std::vector<VertexID>& vertices, int expectNum) {
...
TEST(VertexCacheTest, GetNeighborsTest) {
  ...
  // Delete vertices
  // If there is data in vertexCache, delete vertice only evicts vertex from
  // VertexCache.
  {
    auto* processor = DeleteVerticesProcessor::instance(env, nullptr, &cache);
    LOG(INFO) << "Build DeleteVerticesRequest...";
...
TEST(VertexCacheTest, GetVertexPropWithTTLTest) {
  ...
  // Get vertex prop
  {
    ...
    // other attributes are empty value, edge returns a row with an empty value
    // fields.
