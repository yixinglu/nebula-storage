...
class TagNode final : public IterateNode<VertexID> {
 public:
  ...
  TagNode(PlanContext* planCtx, TagContext* ctx, TagID tagId, const std::vector<PropContext>* props,
          StorageExpressionContext* expCtx = nullptr, Expression* exp = nullptr)
      : planContext_(planCtx), tagContext_(ctx), tagId_(tagId), props_(props), expCtx_(expCtx), exp_(exp) {
    UNUSED(expCtx_);
    UNUSED(exp_);
  ...
  kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
    ...
    VLOG(1) << "partId " << partId << ", vId " << vId << ", tagId " << tagId_ << ", prop size " << props_->size();
  ...
  kvstore::ResultCode collectTagPropsIfValid(NullHandler nullHandler, PropHandler valueHandler) {
    if (!valid()) {
  ...
  bool valid() const override { return !stopSearching_ && reader_ != nullptr; }
  ...
  TagID getTagId() { return tagId_; }

 private:
  bool resetReader(const VertexID& vId) {
    reader_.reset(*schemas_, value_);
    if (!reader_ || (ttl_.hasValue() && CommonUtils::checkDataExpiredForTTL(schemas_->back().get(), reader_.get(),
