...
int main(int argc, char *argv[]) {
  ...
  auto hostName = FLAGS_local_ip != "" ? FLAGS_local_ip : nebula::network::NetworkUtils::getHostname();
  ...
  std::transform(paths.begin(), paths.end(), paths.begin(), [](auto &p) { return folly::trimWhitespace(p).str(); });
  if (paths.empty()) {
  ...
  gStorageServer =
      std::make_unique<nebula::storage::StorageServer>(host, metaAddrsRet.value(), paths, FLAGS_listener_path);
