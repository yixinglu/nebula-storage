...
kvstore::ResultCode ActiveHostsMan::updateHostInfo(kvstore::KVStore* kv, const HostAddr& hostAddr, const HostInfo& info,
                                                   const LeaderParts* leaderParts) {
  CHECK_NOTNULL(kv);
  std::vector<kvstore::KV> data;
  data.emplace_back(MetaServiceUtils::hostKey(hostAddr.host, hostAddr.port), HostInfo::encodeV2(info));
  ...
  kv->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&](kvstore::ResultCode code) {
    ret = code;
    baton.post();
  });
  baton.wait();
...
std::vector<HostAddr> ActiveHostsMan::getActiveHosts(kvstore::KVStore* kv, int32_t expiredTTL, cpp2::HostRole role) {
  ...
  int64_t threshold = (expiredTTL == 0 ? FLAGS_heartbeat_interval_secs * FLAGS_expired_time_factor : expiredTTL) * 1000;
...
std::vector<HostAddr> ActiveHostsMan::getActiveHostsInZone(kvstore::KVStore* kv, const std::string& zoneName,
                                                           int32_t expiredTTL) {
...
std::vector<HostAddr> ActiveHostsMan::getActiveHostsWithGroup(kvstore::KVStore* kv, GraphSpaceID spaceId,
                                                              int32_t expiredTTL) {
...
std::vector<HostAddr> ActiveHostsMan::getActiveAdminHosts(kvstore::KVStore* kv, int32_t expiredTTL,
                                                          cpp2::HostRole role) {
  auto hosts = getActiveHosts(kv, expiredTTL, role);
  std::vector<HostAddr> adminHosts(hosts.size());
  std::transform(hosts.begin(), hosts.end(), adminHosts.begin(),
...
kvstore::ResultCode LastUpdateTimeMan::update(kvstore::KVStore* kv, const int64_t timeInMilliSec) {
  CHECK_NOTNULL(kv);
  std::vector<kvstore::KV> data;
  data.emplace_back(MetaServiceUtils::lastUpdateTimeKey(), MetaServiceUtils::lastUpdateTimeVal(timeInMilliSec));
