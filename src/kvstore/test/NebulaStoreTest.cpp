...
std::shared_ptr<apache::thrift::concurrency::PriorityThreadManager> getHandlers() {
  auto handlersPool = apache::thrift::concurrency::PriorityThreadManager::newPriorityThreadManager(1, true /*stats*/);
...
TEST(NebulaStoreTest, SimpleTest) {
  ...
  VLOG(1) << "Total space num is " << partMan->partsMap_.size() << ", total local partitions num is "
  ...
  HostAddr local = {"", 0};
  auto store = std::make_unique<NebulaStore>(std::move(options), ioThreadPool, local, getHandlers());
  store->init();
  sleep(1);
  ...
  EXPECT_EQ(folly::stringPrintf("%s/disk1/nebula/1", rootPath.path()), store->spaces_[1]->engines_[0]->getDataRoot());
  EXPECT_EQ(folly::stringPrintf("%s/disk2/nebula/1", rootPath.path()), store->spaces_[1]->engines_[1]->getDataRoot());

  EXPECT_EQ(6, store->spaces_[2]->parts_.size());
  EXPECT_EQ(2, store->spaces_[2]->engines_.size());
  EXPECT_EQ(folly::stringPrintf("%s/disk1/nebula/2", rootPath.path()), store->spaces_[2]->engines_[0]->getDataRoot());
  EXPECT_EQ(folly::stringPrintf("%s/disk2/nebula/2", rootPath.path()), store->spaces_[2]->engines_[1]->getDataRoot());

  store->asyncMultiPut(0, 0, {{"key", "val"}},
                       [](ResultCode code) { EXPECT_EQ(ResultCode::ERR_SPACE_NOT_FOUND, code); });

  store->asyncMultiPut(1, 6, {{"key", "val"}},
                       [](ResultCode code) { EXPECT_EQ(ResultCode::ERR_PART_NOT_FOUND, code); });
...
TEST(NebulaStoreTest, ThreeCopiesTest) {
  fs::TempDir rootPath("/tmp/nebula_store_test.XXXXXX");
  auto initNebulaStore = [](const std::vector<HostAddr>& peers, int32_t index,
  ...
  for (int part = 0; part < 3; part++) {
    ...
    {
      folly::Baton<true, std::atomic> baton;
      stores[followerIndex]->asyncMultiPut(0, part, {{"key", "val"}}, [&baton](ResultCode code) {
...
TEST(NebulaStoreTest, CheckpointTest) {
  ...
  EXPECT_EQ(6, store->spaces_[1]->parts_.size());
  EXPECT_EQ(2, store->spaces_[1]->engines_.size());
  EXPECT_EQ(folly::stringPrintf("%s/disk1/nebula/1", srcPath.path()), store->spaces_[1]->engines_[0]->getDataRoot());
  EXPECT_EQ(folly::stringPrintf("%s/disk2/nebula/1", srcPath.path()), store->spaces_[1]->engines_[1]->getDataRoot());

  EXPECT_EQ(6, store->spaces_[2]->parts_.size());
  EXPECT_EQ(2, store->spaces_[2]->engines_.size());
  EXPECT_EQ(folly::stringPrintf("%s/disk1/nebula/2", srcPath.path()), store->spaces_[2]->engines_[0]->getDataRoot());
  EXPECT_EQ(folly::stringPrintf("%s/disk2/nebula/2", srcPath.path()), store->spaces_[2]->engines_[1]->getDataRoot());
...
TEST(NebulaStoreTest, ThreeCopiesCheckpointTest) {
  ...
  for (int i = 0; i < replicas; i++) {
    ...
    std::string src = folly::stringPrintf("%s/disk%d/nebula/0/checkpoints/snapshot/data", rootPath.path(), i);
