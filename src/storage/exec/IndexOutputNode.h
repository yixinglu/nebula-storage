...
class IndexOutputNode final : public RelNode<T> {
 public:
  ...
  IndexOutputNode(nebula::DataSet* result, PlanContext* planCtx, IndexScanNode<T>* indexScanNode, bool hasNullableCol,
      ...
      : result_(result),
        ...
        fields_(fields) {
    type_ = planContext_->isEdge_ ? IndexResultType::kEdgeFromIndexScan : IndexResultType::kVertexFromIndexScan;
  }

  IndexOutputNode(nebula::DataSet* result, PlanContext* planCtx, IndexEdgeNode<T>* indexEdgeNode)
      : result_(result), planContext_(planCtx), indexEdgeNode_(indexEdgeNode) {
    type_ = IndexResultType::kEdgeFromDataScan;
  }

  IndexOutputNode(nebula::DataSet* result, PlanContext* planCtx, IndexVertexNode<T>* indexVertexNode)
      : result_(result), planContext_(planCtx), indexVertexNode_(indexVertexNode) {
    type_ = IndexResultType::kVertexFromDataScan;
  }

  IndexOutputNode(nebula::DataSet* result, PlanContext* planCtx, IndexFilterNode<T>* indexFilterNode,
                  bool indexFilter = false)
      : result_(result), planContext_(planCtx), indexFilterNode_(indexFilterNode) {
    hasNullableCol_ = indexFilterNode->hasNullableCol();
    fields_ = indexFilterNode_->indexCols();
    if (indexFilter) {
      type_ = planContext_->isEdge_ ? IndexResultType::kEdgeFromIndexFilter : IndexResultType::kVertexFromIndexFilter;
    } else {
      type_ = planContext_->isEdge_ ? IndexResultType::kEdgeFromDataFilter : IndexResultType::kVertexFromDataFilter;
    }
  }
 ...
 private:
  ...
  kvstore::ResultCode vertexRowsFromData(const std::vector<kvstore::KV>& data) {
    const auto& schemas =
        type_ == IndexResultType::kVertexFromDataScan ? indexVertexNode_->getSchemas() : indexFilterNode_->getSchemas();
    if (schemas.empty()) {
  ...
  kvstore::ResultCode edgeRowsFromData(const std::vector<kvstore::KV>& data) {
    const auto& schemas =
        type_ == IndexResultType::kEdgeFromDataScan ? indexEdgeNode_->getSchemas() : indexFilterNode_->getSchemas();
    if (schemas.empty()) {
  ...
  Status addIndexValue(Row& row, RowReader* reader, const kvstore::KV& data, const std::string& col,
  ...
  Status addIndexValue(Row& row, const kvstore::KV& data, const std::string& col) {
    switch (QueryUtils::toReturnColType(col)) {
      ...
      default: {
        auto v = IndexKeyUtils::getValueFromIndexKey(planContext_->vIdLen_, data.first, col, fields_,
                                                     planContext_->isEdge_, hasNullableCol_);
