...
folly::Future<Status> AdminClient::transLeader(GraphSpaceID spaceId, PartitionID partId, const HostAddr& leader,
                                               const HostAddr& dst) {
  ...
  return getResponse(
      Utils::getAdminAddrFromStoreAddr(leader), std::move(req),
...
folly::Future<Status> AdminClient::addPart(GraphSpaceID spaceId, PartitionID partId, const HostAddr& host,
                                           bool asLearner) {
  ...
  return getResponse(
      Utils::getAdminAddrFromStoreAddr(host), std::move(req),
...
folly::Future<Status> AdminClient::addLearner(GraphSpaceID spaceId, PartitionID partId, const HostAddr& learner) {
  ...
  getResponse(
      getAdminAddrFromPeers(peers), 0, std::move(req),
      [](auto client, auto request) { return client->future_addLearner(request); }, 0, std::move(pro),
      FLAGS_max_retry_times_admin_op);
  return f;
}

folly::Future<Status> AdminClient::waitingForCatchUpData(GraphSpaceID spaceId, PartitionID partId,
  ...
  getResponse(
      getAdminAddrFromPeers(peers), 0, std::move(req),
      [](auto client, auto request) { return client->future_waitingForCatchUpData(request); }, 0, std::move(pro), 3);
  return f;
}

folly::Future<Status> AdminClient::memberChange(GraphSpaceID spaceId, PartitionID partId, const HostAddr& peer,
                                                bool added) {
  ...
  getResponse(
      getAdminAddrFromPeers(peers), 0, std::move(req),
      [](auto client, auto request) { return client->future_memberChange(request); }, 0, std::move(pro),
      FLAGS_max_retry_times_admin_op);
  return f;
}

folly::Future<Status> AdminClient::updateMeta(GraphSpaceID spaceId, PartitionID partId, const HostAddr& src,
                                              const HostAddr& dst) {
  CHECK_NOTNULL(kv_);
  auto ret = getPeers(spaceId, partId);
  if (!ret.ok()) {
  ...
  LOG(INFO) << "[space:" << spaceId << ", part:" << partId << "] Update original peers " << strHosts(peers)
            << " remove " << src << ", add " << dst;
...
folly::Future<Status> AdminClient::removePart(GraphSpaceID spaceId, PartitionID partId, const HostAddr& host) {
  ...
  return getResponse(
      ...
      [](auto&& resp) -> Status {
        if (resp.get_code() == storage::cpp2::ErrorCode::SUCCEEDED) {
          ...
          return Status::Error("Remove part failed! code=%d", static_cast<int32_t>(resp.get_code()));
        }
      });
}
...
folly::Future<Status> AdminClient::checkPeers(GraphSpaceID spaceId, PartitionID partId) {
  ...
  for (auto& p : peers) {
    ...
    auto f = getResponse(
        Utils::getAdminAddrFromStoreAddr(p), req,
        [](auto client, auto request) { return client->future_checkPeers(request); },
        [](auto&& resp) -> Status {
          if (resp.get_code() == storage::cpp2::ErrorCode::SUCCEEDED) {
            return Status::OK();
          } else {
            return Status::Error("Check peers failed! code=%d", static_cast<int32_t>(resp.get_code()));
          }
        });
    futures.emplace_back(std::move(f));
  }

  folly::collectAll(std::move(futures)).via(ioThreadPool_.get()).thenTry([p = std::move(pro)](auto&& t) mutable {
    if (t.hasException()) {
...
folly::Future<Status> AdminClient::getResponse(const HostAddr& host, Request req, RemoteFunc remoteFunc,
                                               RespGenerator respGen) {
  folly::Promise<Status> pro;
  auto f = pro.getFuture();
  auto* evb = ioThreadPool_->getEventBase();
  auto partId = req.get_part_id();
  folly::via(evb, [evb, pro = std::move(pro), host, req = std::move(req), partId, remoteFunc = std::move(remoteFunc),
                   respGen = std::move(respGen), this]() mutable {
    auto client = clientsMan_->client(host, evb);
    remoteFunc(client, std::move(req))
        .via(evb)
        .then([p = std::move(pro), partId, respGen = std::move(respGen),
               host](folly::Try<storage::cpp2::AdminExecResp>&& t) mutable {
          // exception occurred during RPC
          if (t.hasException()) {
            p.setValue(Status::Error(folly::stringPrintf("[%s] RPC failure in AdminClient: %s", host.toString().c_str(),
...
void AdminClient::getResponse(std::vector<HostAddr> hosts, int32_t index, Request req, RemoteFunc remoteFunc,
                              int32_t retry, folly::Promise<Status> pro, int32_t retryLimit, HandleResultOpt respGen) {
  auto* evb = ioThreadPool_->getEventBase();
  CHECK_GE(index, 0);
  CHECK_LT(index, hosts.size());
  folly::via(evb, [evb, hosts = std::move(hosts), index, req = std::move(req), remoteFunc = std::move(remoteFunc),
                   respGen = std::move(respGen), retry, pro = std::move(pro), retryLimit, this]() mutable {
    ...
    remoteFunc(client, req)
        ...
        .then([p = std::move(pro), hosts = std::move(hosts), index, req = std::move(req),
               remoteFunc = std::move(remoteFunc), respGen = std::move(respGen), retry, retryLimit,
               this](folly::Try<storage::cpp2::AdminExecResp>&& t) mutable {
          // exception occurred during RPC
          if (t.hasException()) {
            if (retry < retryLimit) {
              LOG(INFO) << "Rpc failure to " << hosts[index] << ", retry " << retry << ", limit " << retryLimit
                        << ", error: " << t.exception();
              index = (index + 1) % hosts.size();
              getResponse(std::move(hosts), index, std::move(req), std::move(remoteFunc), retry + 1, std::move(p),
                          retryLimit, std::move(respGen));
              return;
            }
            p.setValue(
                Status::Error(folly::stringPrintf("RPC failure in AdminClient: %s", t.exception().what().c_str())));
            return;
          }
          ...
          switch (resp.get_code()) {
            case storage::cpp2::ErrorCode::E_LEADER_CHANGED: {
              if (retry < retryLimit) {
                ...
                if (leaderIndex == (int32_t)hosts.size()) {
                  ...
                  // remove phase, and the new host is elected as leader
                  // somehow), the peers of this partition in meta doesn't
                  // include new host, which will make this task failed forever.
                  index = leaderIndex;
                  hosts.emplace_back(leader);
                }
                LOG(INFO) << "Return leader change from " << hosts[index] << ", new leader is " << leader << ", retry "
                          << retry << ", limit " << retryLimit;
                CHECK_LT(leaderIndex, hosts.size());
                getResponse(std::move(hosts), leaderIndex, std::move(req), std::move(remoteFunc), retry + 1,
                            std::move(p), retryLimit, std::move(respGen));
                return;
              }
            ...
            default: {
              if (retry < retryLimit) {
                LOG(INFO) << "Unknown code " << static_cast<int32_t>(resp.get_code()) << " from " << hosts[index]
                          << ", retry " << retry << ", limit " << retryLimit;
                index = (index + 1) % hosts.size();
                getResponse(std::move(hosts), index, std::move(req), std::move(remoteFunc), retry + 1, std::move(p),
                            retryLimit, std::move(respGen));
...
std::vector<HostAddr> AdminClient::getAdminAddrFromPeers(const std::vector<HostAddr>& peers) {
  ...
  std::transform(peers.begin(), peers.end(), adminHosts.begin(),
                 [](const auto& h) { return Utils::getAdminAddrFromStoreAddr(h); });
  return adminHosts;
}

void AdminClient::getLeaderDist(const HostAddr& host, folly::Promise<StatusOr<storage::cpp2::GetLeaderPartsResp>>&& pro,
                                int32_t retry, int32_t retryLimit) {
  auto* evb = ioThreadPool_->getEventBase();
  folly::via(evb, [evb, host, pro = std::move(pro), retry, retryLimit, this]() mutable {
    ...
    client->future_getLeaderParts(std::move(req))
        .via(evb)
        .then([pro = std::move(pro), host, retry, retryLimit,
               this](folly::Try<storage::cpp2::GetLeaderPartsResp>&& t) mutable {
          if (t.hasException()) {
            LOG(ERROR) << folly::stringPrintf("RPC failure in AdminClient: %s", t.exception().what().c_str());
            if (retry < retryLimit) {
              usleep(1000 * 50);
              getLeaderDist(Utils::getAdminAddrFromStoreAddr(host), std::move(pro), retry + 1, retryLimit);
            } else {
...
folly::Future<StatusOr<std::string>> AdminClient::createSnapshot(GraphSpaceID spaceId, const std::string& name,
                                                                 const HostAddr& host) {
  ...
  folly::via(evb, [evb, storageHost, pro = std::move(pro), spaceId, name, this]() mutable {
    ...
    client->future_createCheckpoint(std::move(req))
        .via(evb)
        .then([p = std::move(pro), storageHost](folly::Try<storage::cpp2::CreateCPResp>&& t) mutable {
          if (t.hasException()) {
            LOG(ERROR) << folly::stringPrintf("RPC failure in AdminClient: %s", t.exception().what().c_str());
...
folly::Future<Status> AdminClient::dropSnapshot(GraphSpaceID spaceId, const std::string& name, const HostAddr& host) {
  ...
  auto f = pro.getFuture();
  getResponse(
      {Utils::getAdminAddrFromStoreAddr(host)}, 0, std::move(req),
      [](auto client, auto request) { return client->future_dropCheckpoint(request); }, 0, std::move(pro),
      3 /*The snapshot operation need to retry 3 times*/);
  return f;
}

folly::Future<Status> AdminClient::blockingWrites(GraphSpaceID spaceId, storage::cpp2::EngineSignType sign,
                                                  const HostAddr& host) {
  ...
  getResponse(
      {Utils::getAdminAddrFromStoreAddr(host)}, 0, std::move(req),
      [](auto client, auto request) { return client->future_blockingWrites(request); }, 0, std::move(pro),
      32 /*The blocking need to retry 32 times*/);
  return f;
}

folly::Future<Status> AdminClient::addTask(cpp2::AdminCmd cmd, int32_t jobId, int32_t taskId, GraphSpaceID spaceId,
                                           const std::vector<HostAddr>& targetHost,
                                           const std::vector<std::string>& taskSpecficParas,
                                           std::vector<PartitionID> parts, int concurrency,
  ...
  auto f = pro.getFuture();
  getResponse(
      hosts, 0, std::move(req), [](auto client, auto request) { return client->future_addAdminTask(request); }, 0,
      std::move(pro), 0, std::move(respGen));
  return f;
}

folly::Future<Status> AdminClient::stopTask(const std::vector<HostAddr>& target, int32_t jobId, int32_t taskId) {
  ...
  auto f = pro.getFuture();
  getResponse(
      hosts, 0, std::move(req), [](auto client, auto request) { return client->future_stopAdminTask(request); }, 0,
      std::move(pro), 1);
  return f;
}
}  // namespace meta
