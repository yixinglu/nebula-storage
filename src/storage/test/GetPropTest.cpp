...
cpp2::GetPropRequest buildVertexRequest(int32_t totalParts, const std::vector<VertexID>& vertices,
...
cpp2::GetPropRequest buildEdgeRequest(int32_t totalParts, const std::vector<cpp2::EdgeKey>& edgeKeys,
...
TEST(GetPropTest, PropertyTest) {
  ...
  }
  {
    ...
    ASSERT_EQ(0, (*resp.result_ref()).failed_parts.size());
    nebula::DataSet expected;
    expected.colNames = {kVid,        std::string("1.").append(kVid), std::string("1.").append(kTag), "1.name", "1.age",
                         "1.avgScore"};
  ...
  }
  {
    ...
    std::vector<std::pair<TagID, std::vector<std::string>>> edges;
    edges.emplace_back(serve, std::vector<std::string>{kSrc, kType, kRank, kDst, "teamName", "startYear", "endYear"});
    auto req = buildEdgeRequest(totalParts, edgeKeys, edges);
...
TEST(GetPropTest, AllPropertyInOneSchemaTest) {
...

  {
    ...
    ASSERT_EQ(0, (*resp.result_ref()).failed_parts.size());
    nebula::DataSet expected;
    expected.colNames = {kVid,        "1.name",  "1.age",      "1.playing",    "1.career",  "1.startYear",
                         "1.endYear", "1.games", "1.avgScore", "1.serveTeams", "1.country", "1.champions"};
    nebula::Row row({"Tim Duncan", "Tim Duncan", 44, false, 19, 1997, 2016, 1392, 19.0, 1, "America", 5});
    expected.rows.emplace_back(std::move(row));
    ASSERT_EQ(expected, *resp.props_ref());
  }
  {
    ...
    ASSERT_EQ(0, (*resp.result_ref()).failed_parts.size());
    nebula::DataSet expected;
    expected.colNames = {"101.playerName", "101.teamName",     "101.startYear", "101.endYear",  "101.teamCareer",
...
TEST(GetPropTest, AllPropertyInAllSchemaTest) {
...

  {
    ...
    ASSERT_EQ(0, (*resp.result_ref()).failed_parts.size());
    {
      ...
      std::vector<Value> values{"Tim Duncan", "Tim Duncan", 44, false, 19, 1997, 2016, 1392, 19.0, 1, "America", 5};
  ...
  }
  {
    ...
    ASSERT_EQ(0, (*resp.result_ref()).failed_parts.size());
    {
      std::vector<nebula::Row> expected;
      {
        nebula::Row row;
        // The first one is kVid, and player have 11 properties in key and value
        std::vector<Value> values{"Tony Parker", "Tony Parker", 38, false, 18, 2001, 2019, 1254, 15.5, 2, "France", 4};
        // team: 1 property, tag3: 11 property, in total 12
        for (size_t i = 0; i < 12; i++) {
          values.emplace_back(Value());
        }
