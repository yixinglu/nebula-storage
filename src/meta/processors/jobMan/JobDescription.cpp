...
JobDescription::JobDescription(JobID id, cpp2::AdminCmd cmd, std::vector<std::string> paras, Status status,
                               int64_t startTime, int64_t stopTime)
    : id_(id), cmd_(cmd), paras_(std::move(paras)), status_(status), startTime_(startTime), stopTime_(stopTime) {}

folly::Optional<JobDescription> JobDescription::makeJobDescription(folly::StringPiece rawkey,
...
std::string JobDescription::makeJobKey(JobID iJob) {
  ...
  str.append(jPrefix.data(), jPrefix.size()).append(reinterpret_cast<const char*>(&iJob), sizeof(JobID));
  return str;
}
...
std::tuple<AdminCmd, std::vector<std::string>, Status, int64_t, int64_t> JobDescription::decodeValV1(
    const folly::StringPiece& rawVal) {
...
std::string JobDescription::archiveKey() {
  ...
  str.append(aPrefix.data(), aPrefix.size()).append(reinterpret_cast<const char*>(&id_), sizeof(id_));
  return str;
}
...
folly::Optional<JobDescription> JobDescription::loadJobDescription(JobID iJob, nebula::kvstore::KVStore* kv) {
