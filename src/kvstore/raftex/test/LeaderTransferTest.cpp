...
TEST(LeaderTransferTest, ChangeLeaderServalTimesTest) {
  ...
  while (++times <= 10) {
    ...
    LOG(INFO) << times << " ===== Let's transfer the leader from " << allHosts[leaderIndex] << " to "
              << allHosts[nLeaderIndex];
    leader.reset();
    auto f = copies[leaderIndex]->sendCommandAsync(test::encodeTransferLeader(allHosts[nLeaderIndex]));
    f.wait();
    waitUntilLeaderElected(copies, leader);
    checkLeadership(copies, nLeaderIndex, leader);
  }
