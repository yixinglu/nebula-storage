...
const size_t kMaxRowKeyLength = sizeof(PartitionID) + sizeof(VertexID) + sizeof(EdgeType) + sizeof(VertexID) +
...
void HBaseStore::init() {
  LOG(INFO) << "Connect to the HBase thrift server " << network::NetworkUtils::intToIPv4(options_.hbaseServer_.first)
...
std::shared_ptr<const meta::SchemaProviderIf> HBaseStore::getSchema(GraphSpaceID spaceId, const std::string& key,
                                                                    SchemaVer version) {
...
std::vector<KV> HBaseStore::decode(GraphSpaceID spaceId, const std::string& key, std::string& encoded) {
...
ResultCode HBaseStore::range(GraphSpaceID spaceId, const std::string& start, const std::string& end,
...
ResultCode HBaseStore::prefix(GraphSpaceID spaceId, const std::string& prefix,
                              std::unique_ptr<KVIterator>* storageIter) {
  auto tableName = this->spaceIdToTableName(spaceId);
...
ResultCode HBaseStore::rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
...
ResultCode HBaseStore::get(GraphSpaceID spaceId, PartitionID partId, const std::string& key, std::string* value,
                           bool canReadFromFollower) {
  UNUSED(partId);
  UNUSED(canReadFromFollower);
  auto tableName = this->spaceIdToTableName(spaceId);
...
std::pair<ResultCode, std::vector<Status>> HBaseStore::multiGet(GraphSpaceID spaceId, PartitionID partId,
...
ResultCode HBaseStore::range(GraphSpaceID spaceId, PartitionID partId, const std::string& start, const std::string& end,
...
ResultCode HBaseStore::prefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix,
                              std::unique_ptr<KVIterator>* iter, bool canReadFromFollower) {
  UNUSED(partId);
  UNUSED(canReadFromFollower);
...
void HBaseStore::asyncMultiPut(GraphSpaceID spaceId, PartitionID partId, std::vector<KV>&& keyValues, KVCallback cb) {
  UNUSED(partId);
...
void HBaseStore::asyncRemove(GraphSpaceID spaceId, PartitionID partId, const std::string& key, KVCallback cb) {
  UNUSED(partId);
...
void HBaseStore::asyncMultiRemove(GraphSpaceID spaceId, PartitionID partId, std::vector<std::string> keys,
                                  KVCallback cb) {
  UNUSED(partId);
  return cb(this->multiRemove(spaceId, keys));
}

void HBaseStore::asyncRemoveRange(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
...
void HBaseStore::asyncRemovePrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix, KVCallback cb) {
  UNUSED(partId);
