...
class BalanceProcessor : public BaseProcessor<cpp2::BalanceResp> {
 public:
  static BalanceProcessor* instance(kvstore::KVStore* kvstore) { return new BalanceProcessor(kvstore); }

  void process(const cpp2::BalanceReq& req);

 private:
  explicit BalanceProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::BalanceResp>(kvstore) {}
};
