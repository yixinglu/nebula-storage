...
void checkTagVertexData(int32_t spaceVidLen, GraphSpaceID spaceId, TagID tagId, int parts, StorageEnv* env,
                        int expectNum, bool distinguishMultiVer = false) {
  int totalCount = 0;
  // partId start 1
...
void checkEdgeData(int32_t spaceVidLen, GraphSpaceID spaceId, EdgeType type, int parts, StorageEnv* env, int expectNum,
      ...
      if (NebulaKeyUtils::isEdge(spaceVidLen, key)) {
        ...
        if (eType == type) {
          if (distinguishMultiVer) {
            ...
            if (source == lastSrcVertexId && ranking == lastRank && destination == lastDstVertexId) {
              // Multi version
            } else {
...
void checkIndexData(GraphSpaceID spaceId, IndexID indexId, int parts, StorageEnv* env, int expectNum) {
  int totalCount = 0;
  // partId start 1
  std::unique_ptr<kvstore::KVIterator> iter;
