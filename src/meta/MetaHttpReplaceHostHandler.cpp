...
void MetaHttpReplaceHostHandler::onEOM() noexcept {
  switch (err_) {
    ...
    case HttpCode::E_ILLEGAL_ARGUMENT:
      LOG(INFO) << errMsg_;
      ResponseBuilder(downstream_).status(WebServiceUtils::to(HttpStatusCode::BAD_REQUEST), errMsg_).sendWithEOM();
      return;
    default:
      break;
  }
...
void MetaHttpReplaceHostHandler::onError(ProxygenError error) noexcept {
  LOG(ERROR) << "Web Service MetaHttpReplaceHostHandler got error : " << proxygen::getErrorString(error);
}
