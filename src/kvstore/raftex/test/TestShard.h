...
class TestShard : public RaftPart {
 ...
 public:
  TestShard(size_t idx, std::shared_ptr<RaftexService> svc, PartitionID partId, HostAddr addr,
            const folly::StringPiece walRoot, std::shared_ptr<folly::IOThreadPoolExecutor> ioPool,
            std::shared_ptr<thread::GenericThreadPool> workers, std::shared_ptr<folly::Executor> handlersPool,
...
class SnapshotManagerImpl : public SnapshotManager {
 public:
  explicit SnapshotManagerImpl(RaftexService* service) : service_(service) { CHECK_NOTNULL(service); }

  ~SnapshotManagerImpl() { LOG(INFO) << "~SnapshotManagerImpl()"; }

  void accessAllRowsInSnapshot(GraphSpaceID spaceId, PartitionID partId, SnapshotCallback cb) override {
    ...
    for (auto& row : part->data_) {
      if (data.size() > 100) {
        LOG(INFO) << part->idStr_ << "Send snapshot total rows " << data.size() << ", total count sended " << totalCount
                  << ", total size sended " << totalSize << ", finished false";
    ...
    LOG(INFO) << part->idStr_ << "Send snapshot total rows " << data.size() << ", total count sended " << totalCount
              << ", total size sended " << totalSize << ", finished true";
