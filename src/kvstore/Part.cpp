...
Part::Part(GraphSpaceID spaceId, PartitionID partId, HostAddr localAddr, const std::string& walPath, KVEngine* engine,
           std::shared_ptr<folly::IOThreadPoolExecutor> ioPool, std::shared_ptr<thread::GenericThreadPool> workers,
           std::shared_ptr<folly::Executor> handlers, std::shared_ptr<raftex::SnapshotManager> snapshotMan,
           std::shared_ptr<RaftClient> clientMan)
    : RaftPart(FLAGS_cluster_id, spaceId, partId, localAddr, walPath, ioPool, workers, handlers, snapshotMan,
               clientMan),
      spaceId_(spaceId),
      partId_(partId),
      walPath_(walPath),
...
void Part::asyncPut(folly::StringPiece key, folly::StringPiece value, KVCallback cb) {
  ...
  appendAsync(FLAGS_cluster_id, std::move(log))
      .thenValue([this, callback = std::move(cb)](AppendLogResult res) mutable { callback(this->toResultCode(res)); });
}
...
void Part::sync(KVCallback cb) {
  sendCommandAsync("").thenValue(
      [this, callback = std::move(cb)](AppendLogResult res) mutable { callback(this->toResultCode(res)); });
}

void Part::asyncAtomicOp(raftex::AtomicOp op, KVCallback cb) {
  atomicOpAsync(std::move(op)).thenValue([this, callback = std::move(cb)](AppendLogResult res) mutable {
...
void Part::asyncAddLearner(const HostAddr& learner, KVCallback cb) {
  ...
  sendCommandAsync(std::move(log)).thenValue([callback = std::move(cb), learner, this](AppendLogResult res) mutable {
    LOG(INFO) << idStr_ << "add learner " << learner << ", result: " << static_cast<int32_t>(this->toResultCode(res));
    callback(this->toResultCode(res));
  });
}
...
void Part::asyncTransferLeader(const HostAddr& target, KVCallback cb) {
  ...
  sendCommandAsync(std::move(log)).thenValue([callback = std::move(cb), target, this](AppendLogResult res) mutable {
...
void Part::asyncAddPeer(const HostAddr& peer, KVCallback cb) {
  ...
  sendCommandAsync(std::move(log)).thenValue([callback = std::move(cb), peer, this](AppendLogResult res) mutable {
    LOG(INFO) << idStr_ << "add peer " << peer << ", result: " << static_cast<int32_t>(this->toResultCode(res));
    callback(this->toResultCode(res));
  });
}
...
void Part::asyncRemovePeer(const HostAddr& peer, KVCallback cb) {
  ...
  sendCommandAsync(std::move(log)).thenValue([callback = std::move(cb), peer, this](AppendLogResult res) mutable {
    LOG(INFO) << idStr_ << "remove peer " << peer << ", result: " << static_cast<int32_t>(this->toResultCode(res));
    callback(this->toResultCode(res));
  });
}
...
void Part::onElected(TermID term) { VLOG(1) << "Being elected as the leader for the term " << term; }
...
bool Part::commitLogs(std::unique_ptr<LogIterator> iter) {
    ...
    switch (log[sizeof(int64_t)]) {
      ...
      case OP_TRANS_LEADER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip commit stale transfer leader " << newLeader << ", the part is opened at "
      ...
      case OP_REMOVE_PEER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip commit stale remove peer " << peer << ", the part is opened at " << startTimeMs_
  ...
  return engine_->commitBatchWrite(std::move(batch), FLAGS_rocksdb_disable_wal, FLAGS_rocksdb_wal_sync) ==
         ResultCode::SUCCEEDED;
}

std::pair<int64_t, int64_t> Part::commitSnapshot(const std::vector<std::string>& rows, LogID committedLogId,
                                                 TermID committedLogTerm, bool finished) {
  auto batch = engine_->startBatchWrite();
  int64_t count = 0;
  int64_t size = 0;
  for (auto& row : rows) {
    ...
    switch (log[sizeof(int64_t)]) {
      case OP_ADD_LEARNER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip stale add learner " << learner << ", the part is opened at " << startTimeMs_
                    << ", but the log timestamp is " << ts;
        }
        break;
      }
      case OP_TRANS_LEADER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip stale transfer leader " << newLeader << ", the part is opened at "
                    << startTimeMs_ << ", but the log timestamp is " << ts;
        }
        break;
      }
      case OP_ADD_PEER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip stale add peer " << peer << ", the part is opened at " << startTimeMs_
                    << ", but the log timestamp is " << ts;
        }
        break;
      }
      case OP_REMOVE_PEER: {
        ...
        if (ts > startTimeMs_) {
          ...
          LOG(INFO) << idStr_ << "Skip stale remove peer " << peer << ", the part is opened at " << startTimeMs_
                    << ", but the log timestamp is " << ts;
        }
        break;
      }
