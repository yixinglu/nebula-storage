...
class AppendLogsIterator final : public LogIterator {
 public:
  AppendLogsIterator(LogID firstLogId, TermID termId, RaftPart::LogCache logs, OpProcessor opCB)
      : firstLogId_(firstLogId), termId_(termId), logId_(firstLogId), logs_(std::move(logs)), opCB_(std::move(opCB)) {
 ...
 *
 *******************************************************/
RaftPart::RaftPart(ClusterID clusterId, GraphSpaceID spaceId, PartitionID partId, HostAddr localAddr,
                   const folly::StringPiece walRoot, std::shared_ptr<folly::IOThreadPoolExecutor> pool,
                   std::shared_ptr<thread::GenericThreadPool> workers, std::shared_ptr<folly::Executor> executor,
    ...
    : idStr_{folly::stringPrintf("[Port: %d, Space: %d, Part: %d] ", localAddr.port, spaceId, partId)},
  ...
  wal_ = FileBasedWal::getWal(walRoot, idStr_, policy,
...
void RaftPart::start(std::vector<HostAddr>&& peers, bool asLearner) {
  ...
  if (lastLogId_ < committedLogId_) {
    LOG(INFO) << idStr_ << "Reset lastLogId " << lastLogId_ << " to be the committedLogId " << committedLogId_;
    lastLogId_ = committedLogId_;
    lastLogTerm_ = term_;
    wal_->reset();
  }
  LOG(INFO) << idStr_ << "There are " << peers.size() << " peer hosts, and total " << peers.size() + 1
            << " copies. The quorum is " << quorum_ + 1 << ", as learner " << asLearner << ", lastLogId " << lastLogId_
            << ", lastLogTerm " << lastLogTerm_ << ", committedLogId " << committedLogId_ << ", term " << term_;
  ...
  bgWorkers_->addDelayTask(delayMS,
                           [self = shared_from_this(), startTime = startTimeMs_] { self->statusPolling(startTime); });
}
...
void RaftPart::addLearner(const HostAddr& addr) {
  ...
  auto it = std::find_if(hosts_.begin(), hosts_.end(), [&addr](const auto& h) { return h->address() == addr; });
  if (it == hosts_.end()) {
...
void RaftPart::preProcessTransLeader(const HostAddr& target) {
  ...
  switch (role_) {
    ...
    default: {
      LOG(INFO) << idStr_ << "My role is " << roleStr(role_) << ", so do nothing when pre process transfer leader";
      break;
    }
  }
}
  ...
  switch (role_) {
    case Role::LEADER: {
      if (target != addr_ && !hosts_.empty()) {
        auto iter = std::find_if(hosts_.begin(), hosts_.end(), [](const auto& h) { return !h->isLearner(); });
        if (iter != hosts_.end()) {
...
void RaftPart::addPeer(const HostAddr& peer) {
  ...
  auto it = std::find_if(hosts_.begin(), hosts_.end(), [&peer](const auto& h) { return h->address() == peer; });
  if (it == hosts_.end()) {
...
cpp2::ErrorCode RaftPart::checkPeer(const HostAddr& candidate) {
  ...
  auto it = std::find_if(hosts.begin(), hosts.end(), [&candidate](const auto& h) { return h->address() == candidate; });
  if (it == hosts.end()) {
...
void RaftPart::addListenerPeer(const HostAddr& listener) {
  ...
  auto it = std::find_if(hosts_.begin(), hosts_.end(), [&listener](const auto& h) { return h->address() == listener; });
  if (it == hosts_.end()) {
...
folly::Future<AppendLogResult> RaftPart::appendLogAsync(ClusterID source, LogType logType, std::string log,
                                                        AtomicOp op) {
  if (blocking_) {
  ...
  AppendLogsIterator it(firstId, termId, std::move(swappedOutLogs),
...
void RaftPart::appendLogsInternal(AppendLogsIterator iter, TermID termId) {
  ...
  LogID lastId = 0;
  if (iter.valid()) {
    VLOG(2) << idStr_ << "Ready to append logs from id " << iter.logId() << " (Current term is " << currTerm << ")";
  } else {
  ...
  do {
    ...
    VLOG(2) << idStr_ << "Succeeded writing logs [" << iter.firstLogId() << ", " << lastId << "] to WAL";
  } while (false);
...
void RaftPart::replicateLogs(folly::EventBase* eb, AppendLogsIterator iter, TermID currTerm, LogID lastLogId,
                             LogID committedId, TermID prevLogTerm, LogID prevLogId) {
  ...
  do {
    ...
    if (term_ != currTerm) {
      VLOG(2) << idStr_ << "Term has been updated, previous " << currTerm << ", current " << term_;
      currTerm = term_;
    }
  } while (false);
  ...
  collectNSucceeded(gen::from(hosts) |
                        gen::map([self = shared_from_this(), eb, currTerm, lastLogId, prevLogId, prevLogTerm,
      ...
      .via(executor_.get())
      .then([self = shared_from_this(), eb, it = std::move(iter), currTerm, lastLogId, committedId, prevLogId,
             prevLogTerm, pHosts = std::move(hosts), tracker](folly::Try<AppendLogResponses>&& result) mutable {
        VLOG(2) << self->idStr_ << "Received enough response";
        CHECK(!result.hasException());
        if (tracker.slow()) {
          tracker.output(self->idStr_, folly::stringPrintf("Total send logs: %ld", lastLogId - prevLogId + 1));
        }
        self->processAppendLogResponses(*result, eb, std::move(it), currTerm, lastLogId, committedId, prevLogTerm,
                                        prevLogId, std::move(pHosts));

        return *result;
      });
}

void RaftPart::processAppendLogResponses(const AppendLogResponses& resps, folly::EventBase* eb, AppendLogsIterator iter,
                                         TermID currTerm, LogID lastLogId, LogID committedId, TermID prevLogTerm,
                                         LogID prevLogId, std::vector<std::shared_ptr<Host>> hosts) {
  // Make sure majority have succeeded
  size_t numSucceeded = 0;
  for (auto& res : resps) {
    if (!hosts[res.first]->isLearner() && res.second.get_error_code() == cpp2::ErrorCode::SUCCEEDED) {
      ++numSucceeded;
    }
  }
  ...
  if (numSucceeded >= quorum_) {
    ...
    do {
      ...
      if (tracker.slow()) {
        tracker.output(idStr_, folly::stringPrintf("Total commit: %ld", committedLogId_ - committedId));
      }
      VLOG(2) << idStr_ << "Leader succeeded in committing the logs " << committedId + 1 << " to " << lastLogId;
    ...
    // the log replication
    {
      ...
      if (iter.empty()) {
        ...
        if (logs_.size() > 0) {
          ...
          sendingPromise_ = std::move(cachingPromise_);
          cachingPromise_.reset();
          iter = AppendLogsIterator(firstLogId, currTerm, std::move(logs_),
    ...
    LOG_EVERY_N(WARNING, 100) << idStr_ << "Only " << numSucceeded << " hosts succeeded, Need to try again";
...
bool RaftPart::needToStartElection() {
  ...
  if (status_ == Status::RUNNING && role_ == Role::FOLLOWER &&
      (lastMsgRecvDur_.elapsedInMSec() >= weight_ * FLAGS_raft_heartbeat_interval_secs * 1000 || isBlindFollower_)) {
    LOG(INFO) << idStr_ << "Start leader election, reason: lastMsgDur " << lastMsgRecvDur_.elapsedInMSec() << ", term "
              << term_;
...
bool RaftPart::prepareElectionRequest(cpp2::AskForVoteRequest& req, std::vector<std::shared_ptr<Host>>& hosts) {
  std::lock_guard<std::mutex> g(raftLock_);
...
typename RaftPart::Role RaftPart::processElectionResponses(const RaftPart::ElectionResponses& results,
...
bool RaftPart::leaderElection() {
  ...
  LOG(INFO) << idStr_ << "Sending out an election request "
            ...
            << ", lastLogTerm = " << voteReq.get_last_log_term() << ", candidateIP = " << voteReq.get_candidate_addr()
  ...
  if (hosts.empty()) {
    ...
    auto eb = ioThreadPool_->getEventBase();
    auto futures =
        collectNSucceeded(gen::from(hosts) | gen::map([eb, self = shared_from_this(), &voteReq](auto& host) {
    ...
    futures.wait();
    CHECK(!futures.hasException()) << "Got exception -- " << futures.result().exception().what().toStdString();
    ...
    case Role::LEADER: {
      ...
      LOG(INFO) << idStr_ << "The partition is elected as the leader";
      {
        ...
        if (status_ == Status::RUNNING) {
          ...
          bgWorkers_->addTask([self = shared_from_this(), term = voteReq.get_term()] { self->onElected(term); });
...
void RaftPart::statusPolling(int64_t startTime) {
  ...
  }
  {
    ...
    if (status_ == Status::RUNNING || status_ == Status::WAITING_SNAPSHOT) {
      ...
      bgWorkers_->addDelayTask(delay, [self = shared_from_this(), startTime] { self->statusPolling(startTime); });
    }
  }
}
...
void RaftPart::processAskForVoteRequest(const cpp2::AskForVoteRequest& req, cpp2::AskForVoteResponse& resp) {
  ...
  LOG(INFO) << idStr_ << "The partition currently is a " << roleStr(role_) << ", lastLogId " << lastLogId_
            << ", lastLogTerm " << lastLogTerm_ << ", committedLogId " << committedLogId_ << ", term " << term_;
  if (role_ == Role::LEARNER) {
  ...
  if (req.get_last_log_term() == lastLogTerm_) {
    ...
    if (req.get_last_log_id() < lastLogId_) {
      LOG(INFO) << idStr_ << "The partition's last log id is " << lastLogId_ << ". The candidate's last log id "
                << req.get_last_log_id() << " is smaller, so it will be rejected, candidate is " << candidate;
  ...
  if (votedAddr_ != HostAddr("", 0)) {
    if (proposedTerm_ > req.get_term() || (proposedTerm_ == req.get_term() && votedAddr_ != candidate)) {
      LOG(INFO) << idStr_ << "We have voted " << votedAddr_ << " on term " << proposedTerm_
                << ", so we should reject the candidate " << candidate << " request on term " << req.get_term();
      resp.set_error_code(cpp2::ErrorCode::E_TERM_OUT_OF_DATE);
      return;
    }
  }
...
void RaftPart::processAppendLogRequest(const cpp2::AppendLogRequest& req, cpp2::AppendLogResponse& resp) {
  if (FLAGS_trace_raft) {
    LOG(INFO) << idStr_ << "Received logAppend "
              << ": GraphSpaceId = " << req.get_space() << ", partition = " << req.get_part()
              << ", leaderIp = " << req.get_leader_addr() << ", leaderPort = " << req.get_leader_port()
              << ", current_term = " << req.get_current_term() << ", lastLogId = " << req.get_last_log_id()
              << ", committedLogId = " << req.get_committed_log_id()
              << ", lastLogIdSent = " << req.get_last_log_id_sent()
              << ", lastLogTermSent = " << req.get_last_log_term_sent()
              << folly::stringPrintf(", num_logs = %ld, logTerm = %ld", req.get_log_str_list().size(),
                                     req.get_log_term())
              << ", sendingSnapshot = " << req.get_sending_snapshot() << ", local lastLogId = " << lastLogId_
              << ", local lastLogTerm = " << lastLogTerm_ << ", local committedLogId = " << committedLogId_
  ...
  if (req.get_last_log_id_sent() < committedLogId_ && req.get_last_log_term_sent() <= term_) {
    LOG(INFO) << idStr_ << "Stale log! The log " << req.get_last_log_id_sent() << ", term "
              << req.get_last_log_term_sent() << " i had committed yet. My committedLogId is " << committedLogId_
              << ", term is " << term_;
    resp.set_error_code(cpp2::ErrorCode::E_LOG_STALE);
    return;
  } else if (req.get_last_log_id_sent() < committedLogId_) {
    LOG(INFO) << idStr_ << "What?? How it happens! The log id is " << req.get_last_log_id_sent() << ", the log term is "
              << req.get_last_log_term_sent() << ", but my committedLogId is " << committedLogId_ << ", my term is "
              << term_ << ", to make the cluster stable i will follow the high term"
  ...
  if (lastLogTerm_ > 0 && req.get_last_log_term_sent() != lastLogTerm_) {
    LOG(INFO) << idStr_ << "The local last log term is " << lastLogTerm_
              << ", which is different from the leader's prevLogTerm " << req.get_last_log_term_sent()
              << ", the prevLogId is " << req.get_last_log_id_sent() << ". So need to rollback to last committedLogId_ "
              << committedLogId_;
    if (wal_->rollbackToLog(committedLogId_)) {
      ...
      resp.set_last_log_id(lastLogId_);
      resp.set_last_log_term(lastLogTerm_);
      LOG(INFO) << idStr_ << "Rollback succeeded! lastLogId is " << lastLogId_ << ", logLogTerm is " << lastLogTerm_
                << ", committedLogId is " << committedLogId_ << ", term is " << term_;
    }
    resp.set_error_code(cpp2::ErrorCode::E_LOG_GAP);
    return;
    ...
    LOG(INFO) << idStr_ << "Stale log! Local lastLogId " << lastLogId_ << ", lastLogTerm " << lastLogTerm_
              << ", lastLogIdSent " << req.get_last_log_id_sent() << ", lastLogTermSent "
  ...
  if (req.get_committed_log_id() > committedLogId_) {
    ...
    if (commitLogs(wal_->iterator(committedLogId_ + 1, lastLogIdCanCommit))) {
      VLOG(1) << idStr_ << "Follower succeeded committing log " << committedLogId_ + 1 << " to " << lastLogIdCanCommit;
      committedLogId_ = lastLogIdCanCommit;
      resp.set_committed_log_id(lastLogIdCanCommit);
    } else {
      LOG(ERROR) << idStr_ << "Failed to commit log " << committedLogId_ + 1 << " to " << req.get_committed_log_id();
      resp.set_error_code(cpp2::ErrorCode::E_WAL_FAIL);
      return;
    }
  }
...
cpp2::ErrorCode RaftPart::verifyLeader(const cpp2::AppendLogRequest& req) {
  ...
  if (req.get_current_term() < term_) {
    LOG_EVERY_N(INFO, 100) << idStr_ << "The current role is " << roleStr(role_) << ". The local term is " << term_
    ...
    do {
      ...
      if (role_ != Role::LEADER && candidate == leader_) {
        ...
        LOG_EVERY_N(INFO, 100) << idStr_ << "The local term is same as remote term " << term_ << ", my role is "
                               << roleStr(role_) << ", reject it!";
        return cpp2::ErrorCode::E_TERM_OUT_OF_DATE;
      }
    } while (false);
  }
  ...
  LOG(INFO) << idStr_ << "The current role is " << roleStr(role_) << ". Will follow the new leader "
            << req.get_leader_addr() << ":" << req.get_leader_port() << " [Term: " << req.get_current_term() << "]";
...
void RaftPart::processSendSnapshotRequest(const cpp2::SendSnapshotRequest& req, cpp2::SendSnapshotResponse& resp) {
  VLOG(1) << idStr_ << "Receive snapshot, total rows " << req.get_rows().size() << ", total count received "
          << req.get_total_count() << ", total size received " << req.get_total_size() << ", finished "
          << req.get_done();
  std::lock_guard<std::mutex> g(raftLock_);
  // Check status
  if (UNLIKELY(status_ == Status::STOPPED)) {
  ...
  if (UNLIKELY(leader_ != HostAddr(req.get_leader_addr(), req.get_leader_port()) || term_ != req.get_term())) {
    LOG(ERROR) << idStr_ << "Term out of date, current term " << term_ << ", received term " << req.get_term();
    resp.set_error_code(cpp2::ErrorCode::E_TERM_OUT_OF_DATE);
    return;
  }
  ...
  auto ret = commitSnapshot(req.get_rows(), req.get_committed_log_id(), req.get_committed_log_term(), req.get_done());
  ...
  if (lastTotalCount_ != req.get_total_count() || lastTotalSize_ != req.get_total_size()) {
    LOG(ERROR) << idStr_ << "Bad snapshot, total rows received " << lastTotalCount_ << ", total rows sended "
               << req.get_total_count() << ", total size received " << lastTotalSize_ << ", total size sended "
               << req.get_total_size();
  ...
  if (req.get_done()) {
    ...
    status_ = Status::RUNNING;
    LOG(INFO) << idStr_ << "Receive all snapshot, committedLogId_ " << committedLogId_ << ", lastLodId " << lastLogId_
...
AppendLogResult RaftPart::isCatchedUp(const HostAddr& peer) {
  ...
  for (auto& host : hosts_) {
    if (host->addr_ == peer) {
      if (host->followerCommittedLogId_ == 0 || host->followerCommittedLogId_ < wal_->firstLogId()) {
      ...
      return host->sendingSnapshot_ ? AppendLogResult::E_SENDING_SNAPSHOT : AppendLogResult::SUCCEEDED;
    }
  }
