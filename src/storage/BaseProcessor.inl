...
template <typename RESP>
void BaseProcessor<RESP>::handleAsync(GraphSpaceID spaceId, PartitionID partId, kvstore::ResultCode code) {
...
template <typename RESP>
void BaseProcessor<RESP>::handleAsync(GraphSpaceID, PartitionID partId, cpp2::ErrorCode code) {
  VLOG(3) << "partId:" << partId << ", code:" << static_cast<int32_t>(code);
...
template <typename RESP>
meta::cpp2::ColumnDef BaseProcessor<RESP>::columnDef(std::string name, meta::cpp2::PropertyType type) {
...
template <typename RESP>
void BaseProcessor<RESP>::pushResultCode(cpp2::ErrorCode code, PartitionID partId, HostAddr leader) {
  if (code != cpp2::ErrorCode::SUCCEEDED) {
    cpp2::PartitionResult thriftRet;
    thriftRet.set_code(code);
    thriftRet.set_part_id(partId);
...
template <typename RESP>
void BaseProcessor<RESP>::handleErrorCode(kvstore::ResultCode code, GraphSpaceID spaceId, PartitionID partId) {
...
void BaseProcessor<RESP>::handleLeaderChanged(GraphSpaceID spaceId, PartitionID partId) {
  ...
  if (ok(addrRet)) {
    ...
    LOG(ERROR) << "Fail to get part leader, spaceId: " << spaceId << ", partId: " << partId
               << ", ResultCode: " << error(addrRet);
    this->pushResultCode(to(error(addrRet)), partId);
  }
}

template <typename RESP>
void BaseProcessor<RESP>::doPut(GraphSpaceID spaceId, PartitionID partId, std::vector<kvstore::KV>&& data) {
  this->env_->kvstore_->asyncMultiPut(
      spaceId, partId, std::move(data),
      [spaceId, partId, this](kvstore::ResultCode code) { handleAsync(spaceId, partId, code); });
}

template <typename RESP>
kvstore::ResultCode BaseProcessor<RESP>::doSyncPut(GraphSpaceID spaceId, PartitionID partId,
                                                   std::vector<kvstore::KV>&& data) {
  folly::Baton<true, std::atomic> baton;
  kvstore::ResultCode ret = kvstore::ResultCode::SUCCEEDED;
  env_->kvstore_->asyncMultiPut(spaceId, partId, std::move(data), [&ret, &baton](kvstore::ResultCode code) {
...
template <typename RESP>
void BaseProcessor<RESP>::doRemove(GraphSpaceID spaceId, PartitionID partId, std::vector<std::string>&& keys) {
  this->env_->kvstore_->asyncMultiRemove(
      spaceId, partId, std::move(keys),
      [spaceId, partId, this](kvstore::ResultCode code) { handleAsync(spaceId, partId, code); });
}

template <typename RESP>
void BaseProcessor<RESP>::doRemoveRange(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                                        const std::string& end) {
  this->env_->kvstore_->asyncRemoveRange(
      spaceId, partId, start, end,
      [spaceId, partId, this](kvstore::ResultCode code) { handleAsync(spaceId, partId, code); });
}
...
StatusOr<std::string> BaseProcessor<RESP>::encodeRowVal(const meta::NebulaSchemaProvider* schema,
                                                        ...
                                                        const std::vector<Value>& props, WriteResult& wRet) {
