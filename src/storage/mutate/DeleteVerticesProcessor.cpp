    ...
    for (auto& part : partVertices) {
      ...
      for (auto& vid : vertexIds) {
        ...
        if (retRes != kvstore::ResultCode::SUCCEEDED) {
          VLOG(3) << "Error! ret = " << static_cast<int32_t>(retRes) << ", spaceID " << spaceId_;
          code = to(retRes);
          break;
        }
        while (iter->valid()) {
...
ErrorOr<kvstore::ResultCode, std::string> DeleteVerticesProcessor::deleteVertices(PartitionID partId,
    ...
    while (iter->valid()) {
      ...
      for (auto& index : indexes_) {
        if (index->get_schema_id().get_tag_id() == tagId) {
          ...
          if (reader == nullptr) {
            reader = RowReaderWrapper::getTagPropReader(env_->schemaMan_, spaceId_, tagId, iter->val());
            if (reader == nullptr) {
          ...
          auto indexKey = IndexKeyUtils::vertexIndexKey(spaceVidLen_, partId, indexId, vertex.getStr(),
