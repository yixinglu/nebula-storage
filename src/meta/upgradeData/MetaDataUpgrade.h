...
class MetaDataUpgrade final {
 ...
 private:
  Status put(const folly::StringPiece &key, const folly::StringPiece &val) {
    ...
    kv_->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&ret, &baton](kvstore::ResultCode code) {
  ...
  Status remove(const folly::StringPiece &key) {
    ...
    folly::Baton<true, std::atomic> baton;
    auto ret = kvstore::ResultCode::SUCCEEDED;
    kv_->asyncMultiRemove(kDefaultSpaceId, kDefaultPartId, std::move(keys), [&ret, &baton](kvstore::ResultCode code) {
      if (kvstore::ResultCode::SUCCEEDED != code) {
        ret = code;
