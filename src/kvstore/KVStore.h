...
class KVStore {
 public:
  ...
  // Read a single key
  virtual ResultCode get(GraphSpaceID spaceId, PartitionID partId, const std::string& key, std::string* value,
  ...
  // Status::KeyNotFound
  virtual std::pair<ResultCode, std::vector<Status>> multiGet(GraphSpaceID spaceId, PartitionID partId,
                                                              const std::vector<std::string>& keys,
                                                              std::vector<std::string>* values,
                                                              bool canReadFromFollower = false) = 0;

  // Get all results in range [start, end)
  virtual ResultCode range(GraphSpaceID spaceId, PartitionID partId, const std::string& start, const std::string& end,
                           std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) = 0;
  ...
  virtual ResultCode range(GraphSpaceID spaceId, PartitionID partId, std::string&& start, std::string&& end,
                           std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) = delete;

  // Get all results with prefix.
  virtual ResultCode prefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix,
                            std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) = 0;

  // To forbid to pass rvalue via the `prefix' parameter.
  virtual ResultCode prefix(GraphSpaceID spaceId, PartitionID partId, std::string&& prefix,
                            std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) = delete;

  // Get all results with prefix starting from start
  virtual ResultCode rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                                     const std::string& prefix, std::unique_ptr<KVIterator>* iter,
                                     bool canReadFromFollower = false) = 0;

  // To forbid to pass rvalue via the `rangeWithPrefix' parameter.
  virtual ResultCode rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, std::string&& start,
                                     std::string&& prefix, std::unique_ptr<KVIterator>* iter,
                                     bool canReadFromFollower = false) = delete;

  virtual ResultCode sync(GraphSpaceID spaceId, PartitionID partId) = 0;

  virtual void asyncMultiPut(GraphSpaceID spaceId, PartitionID partId, std::vector<KV>&& keyValues, KVCallback cb) = 0;

  // Asynchronous version of remove methods
  virtual void asyncRemove(GraphSpaceID spaceId, PartitionID partId, const std::string& key, KVCallback cb) = 0;

  virtual void asyncMultiRemove(GraphSpaceID spaceId, PartitionID partId, std::vector<std::string>&& keys,
                                KVCallback cb) = 0;

  virtual void asyncRemoveRange(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                                const std::string& end, KVCallback cb) = 0;

  virtual void asyncAtomicOp(GraphSpaceID spaceId, PartitionID partId, raftex::AtomicOp op, KVCallback cb) = 0;
   ...
   */
  virtual void asyncAppendBatch(GraphSpaceID spaceId, PartitionID partId, std::string&& batch, KVCallback cb) = 0;

  virtual ResultCode ingest(GraphSpaceID spaceId) = 0;

  virtual int32_t allLeader(std::unordered_map<GraphSpaceID, std::vector<PartitionID>>& leaderIds) = 0;

  virtual ErrorOr<ResultCode, std::shared_ptr<Part>> part(GraphSpaceID spaceId, PartitionID partId) = 0;

  virtual ResultCode compact(GraphSpaceID spaceId) = 0;

  virtual ResultCode flush(GraphSpaceID spaceId) = 0;

  virtual ErrorOr<ResultCode, std::string> createCheckpoint(GraphSpaceID spaceId, const std::string& name) = 0;
  ...
  virtual ResultCode restoreFromFiles(GraphSpaceID spaceId, const std::vector<std::string>& files) = 0;

  virtual ResultCode multiPutWithoutReplicator(GraphSpaceID spaceId, std::vector<KV> keyValues) = 0;
