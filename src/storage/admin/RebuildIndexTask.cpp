...
ErrorOr<cpp2::ErrorCode, std::vector<AdminSubTask>> RebuildIndexTask::genSubTasks() {
  CHECK_NOTNULL(env_->kvstore_);
  space_ = *ctx_.parameters_.space_id_ref();
  auto parts = *ctx_.parameters_.parts_ref();

  IndexItems items;
  if (!ctx_.parameters_.task_specfic_paras_ref().has_value() || (*ctx_.parameters_.task_specfic_paras_ref()).empty()) {
  ...
  for (const auto& part : parts) {
    ...
    std::function<kvstore::ResultCode()> task = std::bind(&RebuildIndexTask::invoke, this, space_, part, items);
    tasks.emplace_back(std::move(task));
  }
  return tasks;
}

kvstore::ResultCode RebuildIndexTask::invoke(GraphSpaceID space, PartitionID part, const IndexItems& items) {
  ...
  if (result != kvstore::ResultCode::SUCCEEDED) {
    LOG(ERROR) << folly::sformat("Building index with operation logs failed, space={}, part={}", space, part);
...
kvstore::ResultCode RebuildIndexTask::buildIndexOnOperations(GraphSpaceID space, PartitionID part) {
  if (canceled_) {
  ...
  while (true) {
    ...
    if (static_cast<int32_t>(operations.size()) < FLAGS_rebuild_index_locked_threshold) {
      ...
      // finished.
      if (stateIter != env_->rebuildIndexGuard_->cend() && stateIter->second == IndexState::BUILDING) {
...
kvstore::ResultCode RebuildIndexTask::removeLegacyLogs(GraphSpaceID space, PartitionID part) {
  std::unique_ptr<kvstore::KVIterator> operationIter;
  auto operationPrefix = OperationKeyUtils::operationPrefix(part);
  auto operationRet = env_->kvstore_->prefix(space, part, operationPrefix, &operationIter);
  if (operationRet != kvstore::ResultCode::SUCCEEDED) {
...
kvstore::ResultCode RebuildIndexTask::writeData(GraphSpaceID space, PartitionID part, std::vector<kvstore::KV> data) {
  folly::Baton<true, std::atomic> baton;
  kvstore::ResultCode result = kvstore::ResultCode::SUCCEEDED;
  env_->kvstore_->asyncMultiPut(space, part, std::move(data), [&result, &baton](kvstore::ResultCode code) {
...
kvstore::ResultCode RebuildIndexTask::removeData(GraphSpaceID space, PartitionID part, std::string&& key) {
  folly::Baton<true, std::atomic> baton;
  kvstore::ResultCode result = kvstore::ResultCode::SUCCEEDED;
  env_->kvstore_->asyncRemove(space, part, std::move(key), [&result, &baton](kvstore::ResultCode code) {
    if (code != kvstore::ResultCode::SUCCEEDED) {
...
kvstore::ResultCode RebuildIndexTask::cleanupOperationLogs(GraphSpaceID space, PartitionID part,
                                                           std::vector<std::string> keys) {
  folly::Baton<true, std::atomic> baton;
  kvstore::ResultCode result = kvstore::ResultCode::SUCCEEDED;
  env_->kvstore_->asyncMultiRemove(space, part, std::move(keys), [&result, &baton](kvstore::ResultCode code) {
    if (code != kvstore::ResultCode::SUCCEEDED) {
