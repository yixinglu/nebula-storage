  ...
  for (auto& part : partEdges) {
    ...
    for (auto& newEdge : newEdges) {
      ...
      VLOG(3) << "PartitionID: " << partId << ", VertexID: " << *edgeKey.src_ref()
              << ", EdgeType: " << *edgeKey.edge_type_ref() << ", EdgeRanking: " << *edgeKey.ranking_ref()
              << ", VertexID: " << *edgeKey.dst_ref();

      if (!NebulaKeyUtils::isValidVidLen(spaceVidLen_, (*edgeKey.src_ref()).getStr(), (*edgeKey.dst_ref()).getStr())) {
      ...
      auto key = NebulaKeyUtils::edgeKey(spaceVidLen_, partId, (*edgeKey.src_ref()).getStr(), *edgeKey.edge_type_ref(),
                                         *edgeKey.ranking_ref(), (*edgeKey.dst_ref()).getStr());
...
void AddEdgesProcessor::doProcessWithIndex(const cpp2::AddEdgesRequest& req) {
  ...
  for (auto& part : partEdges) {
    ...
    for (auto& newEdge : newEdges) {
      ...
      if (*edgeKey.edge_type_ref() > 0) {
        ...
        if (nebula::ok(obsIdx)) {
          if (!nebula::value(obsIdx).empty()) {
            oReader = RowReaderWrapper::getEdgePropReader(env_->schemaMan_, spaceId_, *edgeKey.edge_type_ref(),
                                                          nebula::value(obsIdx));
          }
        } else {
        ...
        if (!retEnc.value().empty()) {
          nReader =
              RowReaderWrapper::getEdgePropReader(env_->schemaMan_, spaceId_, *edgeKey.edge_type_ref(), retEnc.value());
        }
          ...
          if (*edgeKey.edge_type_ref() == index->get_schema_id().get_edge_type()) {
            ...
            if (nReader != nullptr) {
              ...
              if (!ni.empty()) {
                ...
                auto indexState = env_->getIndexState(spaceId_, partId);
                if (env_->checkRebuilding(indexState)) {
                  auto opKey = OperationKeyUtils::modifyOperationKey(partId, std::move(ni));
      ...
      dummyLock.emplace_back(std::make_tuple(spaceId_, partId, (*edgeKey.src_ref()).getStr(), *edgeKey.edge_type_ref(),
                                             *edgeKey.ranking_ref(), (*edgeKey.dst_ref()).getStr()));
    }
    if (code != cpp2::ErrorCode::SUCCEEDED) {
      handleAsync(spaceId_, partId, code);
    ...
    env_->kvstore_->asyncAppendBatch(
        spaceId_, partId, std::move(batch),
...
ErrorOr<kvstore::ResultCode, std::string> AddEdgesProcessor::addEdges(PartitionID partId,
  ...
  std::for_each(edges.begin(), edges.end(), [&newEdges](const auto& e) { newEdges[e.first] = e.second; });
    ...
    for (auto& index : indexes_) {
      if (edgeType == index->get_schema_id().get_edge_type()) {
        ...
        if (val.empty()) {
          ...
          if (!val.empty()) {
            oReader = RowReaderWrapper::getEdgePropReader(env_->schemaMan_, spaceId_, edgeType, val);
            if (oReader == nullptr) {
         ...
         */
        if (nReader == nullptr) {
          nReader = RowReaderWrapper::getEdgePropReader(env_->schemaMan_, spaceId_, edgeType, e.second);
          if (nReader == nullptr) {
            LOG(ERROR) << "Bad format row";
            return kvstore::ResultCode::ERR_INVALID_DATA;
          }
        ...
        if (!ni.empty()) {
          ...
          auto indexState = env_->getIndexState(spaceId_, partId);
          if (env_->checkRebuilding(indexState)) {
            auto modifyOpKey = OperationKeyUtils::modifyOperationKey(partId, std::move(ni));
...
ErrorOr<kvstore::ResultCode, std::string> AddEdgesProcessor::findOldValue(PartitionID partId,
                                                                          const folly::StringPiece& rawKey) {
  auto prefix = NebulaKeyUtils::edgePrefix(spaceVidLen_, partId, NebulaKeyUtils::getSrcId(spaceVidLen_, rawKey).str(),
...
std::string AddEdgesProcessor::indexKey(PartitionID partId, RowReader* reader, const folly::StringPiece& rawKey,
                                        std::shared_ptr<nebula::meta::cpp2::IndexItem> index) {
  auto values = IndexKeyUtils::collectIndexValues(reader, index->get_fields());
  if (!values.ok()) {
    return "";
  }
  return IndexKeyUtils::edgeIndexKey(spaceVidLen_, partId, index->get_index_id(),
                                     NebulaKeyUtils::getSrcId(spaceVidLen_, rawKey).str(),
                                     NebulaKeyUtils::getRank(spaceVidLen_, rawKey),
                                     NebulaKeyUtils::getDstId(spaceVidLen_, rawKey).str(), std::move(values).value());
}
