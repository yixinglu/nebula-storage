...
class IndexFilterNode final : public RelNode<T> {
 public:
  ...
  // data anymore.
  IndexFilterNode(IndexScanNode<T>* indexScanNode, StorageExpressionContext* exprCtx = nullptr,
                  Expression* exp = nullptr, bool isEdge = false)
  ...
  // need to read data.
  IndexFilterNode(IndexEdgeNode<T>* indexEdgeNode, StorageExpressionContext* exprCtx = nullptr,
  ...
  // evalExprByIndex_ is false, some fileds in filter is out of index, which
  // need to read data.
  IndexFilterNode(IndexVertexNode<T>* indexVertexNode, StorageExpressionContext* exprCtx = nullptr,
                  Expression* exp = nullptr)
  ...
  kvstore::ResultCode execute(PartitionID partId) override {
    ...
    for (const auto& k : data) {
      if (evalExprByIndex_) {
        ...
        const auto& schemas = isEdge_ ? indexEdgeNode_->getSchemas() : indexVertexNode_->getSchemas();
