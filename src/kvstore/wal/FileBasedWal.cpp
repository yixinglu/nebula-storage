 ...
 *********************************************/
// static
std::shared_ptr<FileBasedWal> FileBasedWal::getWal(const folly::StringPiece dir, const std::string& idStr,
                                                   FileBasedWalPolicy policy, PreProcessor preProcessor) {
  return std::shared_ptr<FileBasedWal>(new FileBasedWal(dir, idStr, std::move(policy), std::move(preProcessor)));
}

FileBasedWal::FileBasedWal(const folly::StringPiece dir, const std::string& idStr, FileBasedWalPolicy policy,
  ...
  if (!walFiles_.empty()) {
    ...
    LOG(INFO) << idStr_ << "lastLogId in wal is " << lastLogId_ << ", lastLogTerm is " << lastLogTerm_ << ", path is "
              << info->path();
    currFd_ = open(info->path(), O_WRONLY | O_APPEND);
    currInfo_ = info;
    if (currFd_ < 0) {
      LOG(FATAL) << "Failed to open the file \"" << info->path() << "\" (" << errno << "): " << strerror(errno);
    }
  }
}
...
void FileBasedWal::scanAllWalFiles() {
  ...
  for (auto& fn : files) {
    ...
    WalFileInfoPtr info = std::make_shared<WalFileInfo>(FileUtils::joinPath(dir_, fn), startIdFromName);
    ...
    if (fd < 0) {
      LOG(ERROR) << "Failed to open the file \"" << fn << "\" (" << errno << "): " << strerror(errno);
      continue;
    }
    ...
    if (read(fd, &firstLogId, sizeof(LogID)) != sizeof(LogID)) {
      LOG(ERROR) << "Failed to read the first log id from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }

    if (firstLogId != startIdFromName) {
      LOG(ERROR) << "The first log id " << firstLogId << " does not match the file name \"" << fn << "\", ignore it!";
      close(fd);
      continue;
    }
    ...
    if (lseek(fd, -sizeof(int32_t), SEEK_END) < 0) {
      LOG(ERROR) << "Failed to seek the last log length from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
    int32_t succMsgLen;
    if (read(fd, &succMsgLen, sizeof(int32_t)) != sizeof(int32_t)) {
      LOG(ERROR) << "Failed to read the last log length from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
    ...
    if (lseek(fd, -(sizeof(int32_t) * 2 + succMsgLen + sizeof(ClusterID) + sizeof(TermID)), SEEK_END) < 0) {
      LOG(ERROR) << "Failed to seek the last log term from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
    TermID term = -1;
    if (read(fd, &term, sizeof(TermID)) != sizeof(TermID)) {
      LOG(ERROR) << "Failed to read the last log term from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
    ...
    if (lseek(fd, -(sizeof(int32_t) * 2 + succMsgLen + sizeof(ClusterID) + sizeof(TermID) + sizeof(LogID)), SEEK_END) <
        0) {
      LOG(ERROR) << "Failed to seek the last log id from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
    LogID lastLogId = -1;
    if (read(fd, &lastLogId, sizeof(LogID)) != sizeof(LogID)) {
      LOG(ERROR) << "Failed to read the last log id from \"" << fn << "\" (" << errno << "): " << strerror(errno);
      close(fd);
      continue;
    }
  ...
  if (!walFiles_.empty()) {
    ...
    for (++it; it != walFiles_.end(); ++it) {
      if (it->second->firstId() > prevLastId + 1) {
        // Found a gap
        LOG(ERROR) << "Found a log id gap before " << it->second->firstId() << ", the previous log id is "
                   << prevLastId;
...
void FileBasedWal::prepareNewFile(LogID startLogId) {
  ...
  currFd_ = open(info->path(), O_CREAT | O_EXCL | O_WRONLY | O_APPEND | O_CLOEXEC | O_LARGEFILE, 0644);
  if (currFd_ < 0) {
    LOG(FATAL) << "Failed to open file \"" << info->path() << "\" (errno: " << errno << "): " << strerror(errno);
  }
  currInfo_ = info;
}
...
void FileBasedWal::rollbackInFile(WalFileInfoPtr info, LogID logId) {
  ...
  if (fd < 0) {
    LOG(FATAL) << "Failed to open file \"" << path << "\" (errno: " << errno << "): " << strerror(errno);
  }
  ...
  while (true) {
    ...
    if (pread(fd, &len, sizeof(int32_t), pos + sizeof(LogID) + sizeof(TermID)) != sizeof(int32_t)) {
      LOG(ERROR) << "Failed to read the message length (errno " << errno << "): " << strerror(errno);
      break;
    }
  ...
  if (pos < FileUtils::fileSize(path)) {
    ...
    if (ftruncate(fd, pos) < 0) {
      LOG(FATAL) << "Failed to truncate file \"" << path << "\" (errno: " << errno << "): " << strerror(errno);
    }
    info->setSize(pos);
  }
  info->setLastId(id);
...
void FileBasedWal::scanLastWal(WalFileInfoPtr info, LogID firstId) {
  ...
  while (true) {
    ...
    // Read the message length
    if (pread(fd, &head, sizeof(int32_t), pos + sizeof(LogID) + sizeof(TermID)) != sizeof(int32_t)) {
      break;
    }

    if (pread(fd, &foot, sizeof(int32_t),
              pos + sizeof(LogID) + sizeof(TermID) + sizeof(int32_t) + sizeof(ClusterID) + head) != sizeof(int32_t)) {
      break;
    }
    ...
    // Move to the next log
    pos += sizeof(LogID) + sizeof(TermID) + sizeof(ClusterID) + sizeof(int32_t) + head + sizeof(int32_t);
...
bool FileBasedWal::appendLogInternal(LogID id, TermID term, ClusterID cluster, std::string msg) {
  ...
  std::string strBuf;
  strBuf.reserve(sizeof(LogID) + sizeof(TermID) + sizeof(ClusterID) + msg.size() + 2 * sizeof(int32_t));
...
bool FileBasedWal::appendLogs(LogIterator& iter) {
  for (; iter.valid(); ++iter) {
    if (!appendLogInternal(iter.logId(), iter.logTerm(), iter.logSource(), iter.logMsg().toString())) {
...
bool FileBasedWal::linkCurrentWAL(const char* newPath) {
  ...
  if (link(it->second->path(), targetFile.data()) != 0) {
    LOG(INFO) << idStr_ << "Create link failed for " << it->second->path() << " on " << newPath
              << ", error:" << strerror(errno);
    return false;
  }
  LOG(INFO) << idStr_ << "Create link success for " << it->second->path() << " on " << newPath;
  return true;
}

bool FileBasedWal::rollbackToLog(LogID id) {
  if (id < firstLogId_ - 1 || id > lastLogId_) {
    LOG(ERROR) << idStr_ << "Rollback target id " << id << " is not in the range of [" << firstLogId_ << ","
               << lastLogId_ << "] of WAL";
    return false;
  }
...

  {
    ...
    if (walFiles_.empty()) {
      ...
      VLOG(1) << "Roll back to log " << id << ", the last WAL file is now \"" << walFiles_.rbegin()->second->path()
              << "\"";
...
void FileBasedWal::cleanWAL() {
  ...
  while (it != walFiles_.end()) {
    ...
    if (index++ < size - 2 && (now - it->second->mtime() > walTTL)) {
      VLOG(1) << "Clean wals, Remove " << it->second->path() << ", now: " << now << ", mtime: " << it->second->mtime();
      unlink(it->second->path());
      it = walFiles_.erase(it);
      count++;
    } else {
...
void FileBasedWal::cleanWAL(LogID id) {
  ...
  if (walFiles_.rbegin()->second->lastId() < id) {
    LOG(WARNING) << "Try to clean wal not existed " << id << ", lastWal is " << walFiles_.rbegin()->second->lastId();
    return;
  }
