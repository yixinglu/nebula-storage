...
TEST(RocksEngineTest, RangeTest) {
  ...
  auto checkRange = [&](int32_t start, int32_t end, int32_t expectedFrom, int32_t expectedTotal) {
    VLOG(1) << "start " << start << ", end " << end << ", expectedFrom " << expectedFrom << ", expectedTotal "
            << expectedTotal;
...
TEST(RocksEngineTest, PrefixTest) {
  ...
  auto checkPrefix = [&](const std::string& prefix, int32_t expectedFrom, int32_t expectedTotal) {
    VLOG(1) << "prefix " << prefix << ", expectedFrom " << expectedFrom << ", expectedTotal " << expectedTotal;
...
TEST(RocksEngineTest, OptionTest) {
  ...
  auto engine = std::make_unique<RocksEngine>(0, kDefaultVIdLen, rootPath.path());
  EXPECT_EQ(ResultCode::SUCCEEDED, engine->setOption("disable_auto_compactions", "true"));
  EXPECT_EQ(ResultCode::ERR_INVALID_ARGUMENT, engine->setOption("disable_auto_compactions_", "true"));
  EXPECT_EQ(ResultCode::ERR_INVALID_ARGUMENT, engine->setOption("disable_auto_compactions", "bad_value"));
  EXPECT_EQ(ResultCode::SUCCEEDED, engine->setDBOption("max_background_compactions", "2"));
  EXPECT_EQ(ResultCode::ERR_INVALID_ARGUMENT, engine->setDBOption("max_background_compactions_", "2"));
  EXPECT_EQ(ResultCode::SUCCEEDED, engine->setDBOption("max_background_compactions", "2_"));
  EXPECT_EQ(ResultCode::ERR_INVALID_ARGUMENT, engine->setDBOption("max_background_compactions", "bad_value"));
}
