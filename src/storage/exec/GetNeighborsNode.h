...
class GetNeighborsNode : public QueryNode<VertexID> {
 public:
  ...
  GetNeighborsNode(PlanContext* planCtx, IterateNode<VertexID>* hashJoinNode, IterateNode<VertexID>* upstream,
                   EdgeContext* edgeContext, nebula::DataSet* resultDataSet, int64_t limit = 0)
 ...
 protected:
  ...
  virtual kvstore::ResultCode iterateEdges(std::vector<Value>& row) {
    ...
    for (; upstream_->valid(); upstream_->next(), ++edgeRowCount) {
      ...
      if (!QueryUtils::collectEdgeProps(key, planContext_->vIdLen_, planContext_->isIntId_, reader, props, list).ok()) {
...
class GetNeighborsSampleNode : public GetNeighborsNode {
 public:
  GetNeighborsSampleNode(PlanContext* planCtx, IterateNode<VertexID>* hashJoinNode, IterateNode<VertexID>* upstream,
                         EdgeContext* edgeContext, nebula::DataSet* resultDataSet, int64_t limit)
 ...
 private:
  using Sample = std::tuple<EdgeType, std::string, std::string, const std::vector<PropContext>*, size_t>;
  ...
  kvstore::ResultCode iterateEdges(std::vector<Value>& row) override {
    ...
    for (auto& sample : samples) {
      ...
      reader = RowReaderWrapper::getEdgePropReader(planContext_->env_->schemaMan_, planContext_->spaceId_,
      ...
      if (!QueryUtils::collectEdgeProps(key, planContext_->vIdLen_, planContext_->isIntId_, reader.get(), props, list)
               .ok()) {
        return kvstore::ResultCode::ERR_EDGE_PROP_NOT_FOUND;
      }
      auto& cell = row[columnIdx].mutableList();
