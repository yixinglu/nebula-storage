...
inline std::ostream& operator<<(std::ostream& os, const ResultCode& rc) { return os << static_cast<int32_t>(rc); }
...
class KVFilter {
 public:
   ...
   * false.
   * */
  virtual bool filter(GraphSpaceID spaceId, const folly::StringPiece& key, const folly::StringPiece& val) const = 0;
};
...
inline rocksdb::Slice toSlice(const folly::StringPiece& str) { return rocksdb::Slice(str.begin(), str.size()); }
