...
BENCHMARK_RELATIVE(AtomicLogBufferWritePush2Short, iters) { runAtomicLogBufferWriteTestPush2(iters, 16); }
...
BENCHMARK_RELATIVE(AtomicLogBufferWritePush2Middle, iters) { runAtomicLogBufferWriteTestPush2(iters, 128); }
BENCHMARK_DRAW_LINE();
...
BENCHMARK_RELATIVE(AtomicLogBufferWritePush2Long, iters) { runAtomicLogBufferWriteTestPush2(iters, 1024); }
BENCHMARK_DRAW_LINE();

BENCHMARK(InMemoryLogBufferWriteVeryLong, iters) { runInMemoryLogBufferWriteTest(iters, 4096); }

BENCHMARK_RELATIVE(AtomicLogBufferWriteVeryLong, iters) { runAtomicLogBufferWriteTest(iters, 4096); }

BENCHMARK_RELATIVE(AtomicLogBufferWritePush2VeryLong, iters) { runAtomicLogBufferWriteTestPush2(iters, 4096); }
BENCHMARK_DRAW_LINE();
...
BENCHMARK_RELATIVE(AtomicLogBufferReadLatest1024) { runAtomicLogBufferReadLatestN(totalLogs, 1024); }

BENCHMARK_DRAW_LINE();

BENCHMARK(InMemoryLogBufferReadLatest6000) { runInMemoryLogBufferReadLatestN(totalLogs, 6000); }

BENCHMARK_RELATIVE(AtomicLogBufferReadLatest6000) { runAtomicLogBufferReadLatestN(totalLogs, 6000); }
...
std::vector<std::unique_ptr<std::thread>> runSingleWriterMultiReadersInBackground(LogBufferPtr logBuffer,
                                                                                  bool startWriter, int32_t readersNum,
