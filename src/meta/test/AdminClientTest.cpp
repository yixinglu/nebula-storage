...
class TestStorageService : public storage::cpp2::StorageAdminServiceSvIf {
 public:
  folly::Future<storage::cpp2::AdminExecResp> future_transLeader(const storage::cpp2::TransLeaderReq& req) override {
    RETURN_OK(req);
  }

  folly::Future<storage::cpp2::AdminExecResp> future_addPart(const storage::cpp2::AddPartReq& req) override {
    RETURN_OK(req);
  }

  folly::Future<storage::cpp2::AdminExecResp> future_addLearner(const storage::cpp2::AddLearnerReq& req) override {
    RETURN_OK(req);
  }
  ...
  folly::Future<storage::cpp2::AdminExecResp> future_removePart(const storage::cpp2::RemovePartReq& req) override {
    RETURN_OK(req);
  }

  folly::Future<storage::cpp2::AdminExecResp> future_memberChange(const storage::cpp2::MemberChangeReq& req) override {
    RETURN_OK(req);
  }
  ...
  folly::Future<storage::cpp2::AdminExecResp> future_dropCheckpoint(const storage::cpp2::DropCPRequest& req) override {
    RETURN_OK(req);
  }
...
TEST(AdminClientTest, SimpleTest) {
...

  {
    ...
    client->transLeader(0, 0, {localIp, rpcServer->port_}, HostAddr("1", 1)).thenValue([&baton](auto&&) {
      baton.post();
    });
    baton.wait();
  }
  {
    ...
    folly::Baton<true, std::atomic> baton;
    client->addPart(0, 0, {localIp, rpcServer->port_}, true).thenValue([&baton](auto&&) { baton.post(); });
    baton.wait();
  }
  {
    LOG(INFO) << "Test removePart...";
    folly::Baton<true, std::atomic> baton;
    client->removePart(0, 0, {localIp, rpcServer->port_}).thenValue([&baton](auto&&) { baton.post(); });
    baton.wait();
  }
}
...
TEST(AdminClientTest, RetryTest) {
  ...
  std::unique_ptr<kvstore::KVStore> kv(MockCluster::initMetaKV(rootPath.path()));
  {
    ...
    folly::Baton<true, std::atomic> baton;
    kv->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&baton](kvstore::ResultCode code) {
  ...
  auto client = std::make_unique<AdminClient>(kv.get());
  {
    ...
    folly::Baton<true, std::atomic> baton;
    client->transLeader(0, 1, Utils::getStoreAddrFromAdminAddr({localIp, rpcServer2->port_}), HostAddr("1", 1))
...
TEST(AdminClientTest, SnapshotTest) {
  ...
  ActiveHostsMan::updateHostInfo(kv.get(), storageHost, HostInfo(now, meta::cpp2::HostRole::STORAGE, ""));
  ASSERT_EQ(1, ActiveHostsMan::getActiveHosts(kv.get()).size());

  auto client = std::make_unique<AdminClient>(kv.get());
  {
    LOG(INFO) << "Test Blocking Writes On...";
    auto status = client->blockingWrites(1, storage::cpp2::EngineSignType::BLOCK_ON, storageHost).get();
  ...
  }
  {
    ...
    auto status = client->blockingWrites(1, storage::cpp2::EngineSignType::BLOCK_OFF, storageHost).get();
    ASSERT_TRUE(status.ok());
  }
}
}  // namespace meta
