...
class ListSpacesProcessor : public BaseProcessor<cpp2::ListSpacesResp> {
 public:
  static ListSpacesProcessor* instance(kvstore::KVStore* kvstore) { return new ListSpacesProcessor(kvstore); }

  void process(const cpp2::ListSpacesReq& req);

 private:
  explicit ListSpacesProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListSpacesResp>(kvstore) {}
};
