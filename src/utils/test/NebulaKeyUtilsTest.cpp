...
class KeyUtilsTestBase : public ::testing::Test {
 public:
  ...
  void verifyVertex(PartitionID partId, VertexID vId, TagID tagId, size_t actualSize) {
    ...
    ASSERT_EQ(vertexKey.substr(0, sizeof(PartitionID) + vIdLen_), NebulaKeyUtils::vertexPrefix(vIdLen_, partId, vId));
  ...
  void verifyEdge(PartitionID partId, VertexID srcId, EdgeType type, EdgeRanking rank, VertexID dstId,
                  EdgeVerPlaceHolder edgeVersion, size_t actualSize) {
    ...
    ASSERT_EQ(edgeKey.substr(0, sizeof(PartitionID) + vIdLen_), NebulaKeyUtils::edgePrefix(vIdLen_, partId, srcId));
