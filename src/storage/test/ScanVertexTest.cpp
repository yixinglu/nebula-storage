...
cpp2::ScanVertexRequest buildRequest(PartitionID partId, const std::string& cursor,
                                     const std::pair<TagID, std::vector<std::string>>& tag, int64_t rowLimit = 100,
                                     int64_t startTime = 0, int64_t endTime = std::numeric_limits<int64_t>::max(),
...
void checkResponse(const nebula::DataSet& dataSet, const std::pair<TagID, std::vector<std::string>>& tag,
                   size_t expectColumnCount, size_t& totalRowCount) {
...
TEST(ScanVertexTest, PropertyTest) {
...

  {
    ...
    auto tag = std::make_pair(player, std::vector<std::string>{kVid, kTag, "name", "age", "avgScore"});
