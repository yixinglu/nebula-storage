...
void InterTxnProcessor::process(const cpp2::InternalTxnRequest& req) {
  ...
  LOG_IF(INFO, FLAGS_trace_toss) << "process req, txnId=" << txnId << ", spaceId=" << spaceId << ", partId=" << partId;
  ...
  env_->txnMan_->commitBatch(spaceId, partId, std::move(data))
      ...
      .thenValue([=](kvstore::ResultCode rc) {
        LOG_IF(INFO, FLAGS_trace_toss) << "txnId=" << txnId << " commitBatch ret rc=" << static_cast<int32_t>(rc);
        auto code = to(rc);
        ...
        for (auto& p : codes_) {
          if (p.leader_ref().has_value()) {
            LOG(INFO) << "txnId=" << txnId << ", part=" << *p.part_id_ref()
                      << ", code=" << static_cast<int32_t>(*p.code_ref()) << ", leader=" << *p.leader_ref();
          } else {
            LOG(INFO) << "txnId=" << txnId << ", part=" << *p.part_id_ref()
