...
class TransLeaderProcessor : public BaseProcessor<cpp2::AdminExecResp> {
 public:
  ...
  void process(const cpp2::TransLeaderReq& req) {
    ...
    LOG(INFO) << "Receive transfer leader for space " << req.get_space_id() << ", part " << req.get_part_id()
              << ", to [" << req.get_new_leader().host << ", " << req.get_new_leader().port << "]";
    ...
    if (part->isLeader() && part->address() == host) {
      LOG(INFO) << "I am already leader of space " << spaceId << " part " << partId << ", skip transLeader";
      onFinished();
      return;
    }
    ...
    if (partMeta.hosts_.size() == 1) {
      LOG(INFO) << "Skip transfer leader of space " << spaceId << ", part " << partId << " because of single replica.";
    ...
    part->asyncTransferLeader(host, [this, spaceId, partId, part](kvstore::ResultCode code) {
      if (code == kvstore::ResultCode::ERR_LEADER_CHANGED) {
        ...
        // To avoid dead lock, we use another ioThreadPool to check the leader
        // information.
        folly::via(folly::getIOExecutor().get(), [this, part, spaceId, partId] {
          ...
          while (retry-- > 0) {
            ...
            if (leader != HostAddr("", 0) && leader != store->address()) {
              LOG(INFO) << "Found new leader of space " << spaceId << " part " << partId << ": " << leader;
              onFinished();
              return;
            } else if (leader != HostAddr("", 0)) {
              LOG(INFO) << "I am choosen as leader of space " << spaceId << " part " << partId << " again!";
              this->pushResultCode(cpp2::ErrorCode::E_TRANSFER_LEADER_FAILED, partId);
              onFinished();
              return;
            }
            LOG(INFO) << "Can't find leader for space " << spaceId << " part " << partId << " on " << store->address();
...
class MemberChangeProcessor : public BaseProcessor<cpp2::AdminExecResp> {
 public:
  ...
  void process(const cpp2::MemberChangeReq& req) {
    ...
    auto spaceId = req.get_space_id();
    auto partId = req.get_part_id();
    LOG(INFO) << "Receive member change for space " << spaceId << ", part " << partId << ", add/remove "
              << (req.get_add() ? "add" : "remove");
    auto ret = env_->kvstore_->part(spaceId, partId);
    if (!ok(ret)) {
      LOG(ERROR) << "Space: " << spaceId << " Part: " << partId << " not found";
      this->pushResultCode(to(error(ret)), partId);
...
class WaitingForCatchUpDataProcessor : public BaseProcessor<cpp2::AdminExecResp> {
 public:
  static WaitingForCatchUpDataProcessor* instance(StorageEnv* env) { return new WaitingForCatchUpDataProcessor(env); }
  ...
  void process(const cpp2::CatchUpDataReq& req) {
    auto spaceId = req.get_space_id();
    auto partId = req.get_part_id();
    LOG(INFO) << "Received waiting for catching up data for space " << spaceId << ", part " << partId;
    auto ret = env_->kvstore_->part(spaceId, partId);
    if (!ok(ret)) {
      this->pushResultCode(to(error(ret)), partId);
      onFinished();
    ...
    folly::async([this, part, peer, spaceId, partId] {
      ...
      while (retry-- > 0) {
        ...
        LOG(INFO) << "Waiting for catching up data, peer " << peer << ", space " << spaceId << ", part " << partId
        ...
        switch (res) {
          ...
          case raftex::AppendLogResult::E_SENDING_SNAPSHOT:
            LOG(INFO) << "Space " << spaceId << ", partId " << partId << " is still sending snapshot, please wait...";
            break;
          default:
 ...
 private:
  explicit WaitingForCatchUpDataProcessor(StorageEnv* env) : BaseProcessor<cpp2::AdminExecResp>(env) {}
};
