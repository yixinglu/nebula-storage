  ...
  if (indexes_.empty()) {
    ...
    for (auto& part : partEdges) {
      ...
      for (auto& edgeKey : part.second) {
        if (!NebulaKeyUtils::isValidVidLen(spaceVidLen_, (*edgeKey.src_ref()).getStr(),
        ...
        auto edge =
            NebulaKeyUtils::edgeKey(spaceVidLen_, partId, (*edgeKey.src_ref()).getStr(), *edgeKey.edge_type_ref(),
    ...
    for (auto& part : partEdges) {
      ...
      for (const auto& edgeKey : part.second) {
        dummyLock.emplace_back(std::make_tuple(spaceId_, partId, (*edgeKey.src_ref()).getStr(),
                                               *edgeKey.edge_type_ref(), *edgeKey.ranking_ref(),
                                               (*edgeKey.dst_ref()).getStr()));
      }
      ...
      env_->kvstore_->asyncAppendBatch(
          spaceId_, partId, std::move(nebula::value(batch)),
...
ErrorOr<kvstore::ResultCode, std::string> DeleteEdgesProcessor::deleteEdges(PartitionID partId,
    ...
    if (iter->valid() && NebulaKeyUtils::isEdge(spaceVidLen_, iter->key())) {
      ...
      for (auto& index : indexes_) {
        if (type == index->get_schema_id().get_edge_type()) {
          ...
          if (reader == nullptr) {
            reader = RowReaderWrapper::getEdgePropReader(env_->schemaMan_, spaceId_, type, iter->val());
            if (reader == nullptr) {
          ...
          auto indexKey = IndexKeyUtils::edgeIndexKey(spaceVidLen_, partId, indexId, srcId, rank, dstId,
