...
TEST(BalanceTest, BalanceTaskTest) {
  ...
  DefaultValue<folly::Future<Status>>::SetFactory([] { return folly::Future<Status>(Status::OK()); });
  {
...
TEST(BalanceTest, SimpleTestWithZone) {
  ...
  FLAGS_heartbeat_interval_secs = 1;
  {
    ...
    ZoneInfo zoneInfo = {
        {"zone_0", {{"0", 0}}}, {"zone_1", {{"1", 1}}}, {"zone_2", {{"2", 2}}}, {"zone_3", {{"3", 3}}}};
...
TEST(BalanceTest, ExpansionHostIntoZoneTest) {
  ...
  ASSERT_EQ(cpp2::ErrorCode::E_BALANCED, error(ret));
  {
    ...
    TestUtils::registerHB(kv, hosts);
    ZoneInfo zoneInfo = {
        {"zone_0", {{"0", 0}, {"3", 3}}}, {"zone_1", {{"1", 1}, {"4", 4}}}, {"zone_2", {{"2", 2}, {"5", 5}}}};
    GroupInfo groupInfo = {{"default_group", {"zone_0", "zone_1", "zone_2"}}};
    TestUtils::assembleGroupAndZone(kv, zoneInfo, groupInfo);
  }
  {
...
TEST(BalanceTest, ShrinkHostFromZoneTest) {
  ...
  {
    ZoneInfo zoneInfo = {{"zone_0", {{"0", 0}}}, {"zone_1", {{"1", 1}, {"4", 4}}}, {"zone_2", {{"2", 2}, {"5", 5}}}};
    GroupInfo groupInfo = {{"default_group", {"zone_0", "zone_1", "zone_2"}}};
    TestUtils::assembleGroupAndZone(kv, zoneInfo, groupInfo);
  }
  ret = balancer.balance({{"3", 3}});
...
TEST(BalanceTest, BalanceWithComplexZoneTest) {
  ...
  {
    ZoneInfo zoneInfo = {
        {"zone_0", {HostAddr("0", 0), HostAddr("1", 1)}},     {"zone_1", {HostAddr("2", 2), HostAddr("3", 3)}},
        {"zone_2", {HostAddr("4", 4), HostAddr("5", 5)}},     {"zone_3", {HostAddr("6", 6), HostAddr("7", 7)}},
        {"zone_4", {HostAddr("8", 8), HostAddr("9", 9)}},     {"zone_5", {HostAddr("10", 10), HostAddr("11", 11)}},
        {"zone_6", {HostAddr("12", 12), HostAddr("13", 13)}}, {"zone_7", {HostAddr("14", 14), HostAddr("15", 15)}},
    ...
    {
      GroupInfo groupInfo = {
          {"group_1", {"zone_0", "zone_1", "zone_2", "zone_3", "zone_4", "zone_5", "zone_6", "zone_7", "zone_8"}}};
      TestUtils::assembleGroupAndZone(kv, zoneInfo, groupInfo);
    }
  }
  {
...
TEST(BalanceTest, DispatchTasksTest) {
  {
    ...
    for (int i = 0; i < 20; i++) {
      BalanceTask task(0, 0, 0, HostAddr(std::to_string(i), 0), HostAddr(std::to_string(i), 1), nullptr, nullptr);
  ...
  }
  {
    ...
    BalancePlan plan(0L, nullptr, nullptr);
    for (int i = 0; i < 5; i++) {
      BalanceTask task(0, 0, i, HostAddr(std::to_string(i), 0), HostAddr(std::to_string(i), 1), nullptr, nullptr);
      plan.addTask(std::move(task));
    }
    plan.dispatchTasks();
  ...
  }
  {
    ...
    for (int i = 0; i < 10; i++) {
      BalanceTask task(0, 0, i, HostAddr(std::to_string(i), 2), HostAddr(std::to_string(i), 3), nullptr, nullptr);
      plan.addTask(std::move(task));
    }
    plan.dispatchTasks();
...
TEST(BalanceTest, BalancePlanTest) {
  ...
  DefaultValue<folly::Future<Status>>::SetFactory([] { return folly::Future<Status>(Status::OK()); });
  {
    ...
    for (int i = 0; i < 10; i++) {
      BalanceTask task(0, 0, 0, HostAddr(std::to_string(i), 0), HostAddr(std::to_string(i), 1), kv, &client);
      plan.addTask(std::move(task));
    }
    folly::Baton<true, std::atomic> b;
  ...
  }
  {
    ...
    for (int i = 0; i < 10; i++) {
      BalanceTask task(0, 0, i, HostAddr(std::to_string(i), 0), HostAddr(std::to_string(i), 1), kv, &client);
      plan.addTask(std::move(task));
    }
    folly::Baton<true, std::atomic> b;
    plan.onFinished_ = [&plan, &b]() {
  ...
  }
  {
    ...
    {
      for (int i = 0; i < 9; i++) {
        BalanceTask task(0, 0, i, HostAddr(std::to_string(i), 0), HostAddr(std::to_string(i), 1), kv, &client1);
        plan.addTask(std::move(task));
      }
    }
    {
...
void verifyBalanceTask(kvstore::KVStore* kv, BalanceID balanceId, BalanceTaskStatus status, BalanceTaskResult result,
                       std::unordered_map<HostAddr, int32_t>& partCount, int32_t exceptNumber) {
...
TEST(BalanceTest, NormalTest) {
  ...
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::END, BalanceTaskResult::SUCCEEDED, partCount, 6);
}
...
TEST(BalanceTest, SpecifyMultiHostTest) {
  ...
  // If {"2", 2} is dead, {"3", 3} stiil alive, each part has majority hosts
  // alive
  ...
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::END, BalanceTaskResult::SUCCEEDED, partCount, 13);
...
TEST(BalanceTest, MockReplaceMachineTest) {
  ...
  std::unordered_map<HostAddr, int32_t> partCount;
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::END, BalanceTaskResult::SUCCEEDED, partCount, 12);
}
...
TEST(BalanceTest, SingleReplicaTest) {
  ...
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::END, BalanceTaskResult::SUCCEEDED, partCount, 4);
...
TEST(BalanceTest, TryToRecoveryTest) {
  ...
  std::unordered_map<HostAddr, int32_t> partCount;
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::CATCH_UP_DATA, BalanceTaskResult::FAILED, partCount, 6);
  ...
  sleep(1);
  ASSERT_EQ(1, verifyBalancePlan(kv, balanceId, BalanceStatus::SUCCEEDED));
  verifyBalanceTask(kv, balanceId, BalanceTaskStatus::START, BalanceTaskResult::INVALID, partCount, 6);
}
...
TEST(BalanceTest, StopPlanTest) {
  ...
  EXPECT_CALL(delayClient, waitingForCatchUpData(_, _, _))
      ...
      // skipped,
      .Times(1)
      .WillOnce(Return(ByMove(folly::makeFuture<Status>(Status::OK()).delayed(std::chrono::seconds(3)))));
...
void verifyLeaderBalancePlan(HostLeaderMap& hostLeaderMap, const LeaderBalancePlan& plan, size_t minLoad,
                             size_t maxLoad) {
...
TEST(BalanceTest, SimpleLeaderBalancePlanTest) {
  ...
  Balancer balancer(kv, &client);
  {
    ...
    auto leaderBalanceResult = balancer.buildLeaderBalancePlan(&hostLeaderMap, 1, 3, false, plan, false);
    ...
    auto tempLeaderBalanceResult = balancer.buildLeaderBalancePlan(&tempMap, 1, 3, false, tempPlan, false);
...
TEST(BalanceTest, LeaderBalanceWithZoneTest) {
...

  {
    ...
    LeaderBalancePlan plan;
    auto leaderBalanceResult = balancer.buildLeaderBalancePlan(&hostLeaderMap, 1, 3, true, plan, true);
    ASSERT_TRUE(leaderBalanceResult);
    verifyLeaderBalancePlan(hostLeaderMap, plan, 1, 2);
  }
  {
...
TEST(BalanceTest, LeaderBalanceWithComplexZoneTest) {
...

  {
    ...
    hostLeaderMap[HostAddr("12", 12)][3] = {1, 3, 6, 8, 11, 14, 16, 19, 22, 24, 27, 30, 46, 48, 51, 54};
    hostLeaderMap[HostAddr("13", 13)][3] = {10, 15, 18, 31, 52, 69, 81};
    hostLeaderMap[HostAddr("14", 14)][3] = {5, 13, 21, 32, 35, 40, 43, 56, 60, 66, 72, 78};
    hostLeaderMap[HostAddr("15", 15)][3] = {2, 4, 7, 9, 12, 17, 20, 23, 25, 28, 33, 39, 41, 44, 47, 49, 55, 57, 63, 75};
    hostLeaderMap[HostAddr("16", 16)][3] = {26};
    hostLeaderMap[HostAddr("17", 17)][3] = {36, 38};

    LeaderBalancePlan plan;
    auto leaderBalanceResult = balancer.buildLeaderBalancePlan(&hostLeaderMap, 3, 3, true, plan, true);
    ASSERT_TRUE(leaderBalanceResult);
