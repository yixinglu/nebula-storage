  ...
  class Iterator : public LogIterator {
   ...
   public:
    ...
    LogIterator& operator++() override {
      ...
      VLOG(3) << "currNode firstLogId = " << currNode_->firstLogId_ << ", currIndex = " << currIndex_
              << ", currNode pos " << pos;
      if (currIndex_ >= pos) {
  ...
  void push(LogID logId, Record&& record) {
    ...
    if (size_ + recSize > capacity_) {
      ...
      if (tail != head) {
        ...
        auto marked = tail->markDeleted_.compare_exchange_strong(expected, true, std::memory_order_relaxed);
 ...
 private:
  ...
  Node* seek(LogID logId) {
    ...
    if (head != nullptr && logId > head->lastLogId()) {
      VLOG(3) << "Bad seek, the seeking logId " << logId << " is greater than the latest logId in buffer "
              << head->lastLogId();
      return nullptr;
    }
    auto* p = head;
    ...
    while (p != tail->next_ && !p->markDeleted_) {
      VLOG(3) << "current node firstLogId = " << p->firstLogId_ << ", the seeking logId = " << logId;
