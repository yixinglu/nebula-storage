...
bool encodeV2(const meta::NebulaSchemaProvider* schema, const std::string& key, const std::vector<Value>& props,
...
bool encodeV1(const meta::NebulaSchemaProvider* schema, const std::string& key, const std::vector<Value>& props,
              std::vector<kvstore::KV>& data) {
...
bool mockVertexData(storage::StorageEnv* ev, int32_t totalParts, int32_t vidLen, bool isVersionV2) {
  ...
  ev->kvstore_->asyncMultiPut(spaceId, pId, std::move(data), [&](kvstore::ResultCode code) {
...
bool mockEdgeData(storage::StorageEnv* ev, int32_t totalParts, int32_t vidLen, bool isVersionV2) {
  ...
  auto key =
      NebulaKeyUtils::edgeKey(vidLen, pId, edgedata.srcId_, edgedata.type_, edgedata.rank_, edgedata.dstId_, version);
...
void insertVertex(int32_t iters) {
  ...
  for (decltype(iters) i = 0; i < iters; i++) {
    ...
    auto* processor = nebula::storage::AddVerticesProcessor::instance(nebula::storage::env, nullptr);
...
void updateVertex(int32_t iters, bool isVersion2) {
  ...
  for (decltype(iters) i = 0; i < iters; i++) {
    ...
    auto* processor = nebula::storage::UpdateVertexProcessor::instance(nebula::storage::env, nullptr);
    auto f = processor->getFuture();
    processor->process(req);
    auto resp = std::move(f).get();
    if (!resp.result.failed_parts.empty()) {
