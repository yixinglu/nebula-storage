...
class GetZoneProcessor : public BaseProcessor<cpp2::GetZoneResp> {
 public:
  static GetZoneProcessor* instance(kvstore::KVStore* kvstore) { return new GetZoneProcessor(kvstore); }

  void process(const cpp2::GetZoneReq& req);

 private:
  explicit GetZoneProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::GetZoneResp>(kvstore) {}
};
