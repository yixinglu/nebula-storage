...
void CreateSpaceProcessor::process(const cpp2::CreateSpaceReq& req) {
  ...
  if (spaceRet.ok()) {
    ...
    if (req.get_if_not_exists()) {
      ...
      LOG(ERROR) << "Create Space Failed : Space " << properties.get_space_name() << " have existed!";
  ...
  if (vidType != cpp2::PropertyType::INT64 && vidType != cpp2::PropertyType::FIXED_STRING) {
    LOG(ERROR) << "Create Space Failed : vid_type is illegal: " << apache::thrift::util::enumNameSafe(vidType);
    handleErrorCode(cpp2::ErrorCode::E_INVALID_PARM);
    onFinished();
    return;
  }
  if (vidType == cpp2::PropertyType::INT64 && vidSize != 8) {
    LOG(ERROR) << "Create Space Failed : vid_size should be 8 if vid type is "
                  "interger: "
               << vidSize;
    handleErrorCode(cpp2::ErrorCode::E_INVALID_PARM);
    onFinished();
  ...
  if (properties.group_name_ref().has_value()) {
    ...
    for (auto partId = 1; partId <= partitionNum; partId++) {
      ...
      data.emplace_back(MetaServiceUtils::partKey(spaceId, partId), MetaServiceUtils::partVal(partHosts));
    }
  } else {
    ...
    if ((int32_t)hosts.size() < replicaFactor) {
      LOG(ERROR) << "Not enough hosts existed for replica " << replicaFactor << ", hosts num " << hosts.size();
...
Hosts CreateSpaceProcessor::pickHosts(PartitionID partId, const Hosts& hosts, int32_t replicaFactor) {
...
StatusOr<std::unordered_map<HostAddr, int32_t>> CreateSpaceProcessor::getHostLoading() {
...
StatusOr<Hosts> CreateSpaceProcessor::pickHostsWithZone(const std::vector<std::string>& zones,
