...
void randomRead(SchemaWriter* schema, const std::string& encoded, const std::vector<size_t>& randomList, size_t iters) {
  auto reader = RowReaderWrapper::getRowReader(schema, encoded);
  DCHECK_EQ(reader->readerVer(), ((encoded[0] & 0x18) >> 3) + 1);
...
void sequentialTest(SchemaWriter* schema, const std::string& encodedV1, const std::string& encodedV2) {
...
void randomTest(SchemaWriter* schema, const std::string& encodedV1, const std::string& encodedV2,
...
BENCHMARK(random_read_short_v1, iters) { randomRead(&schemaShort, dataShortV1, shortRandom, iters); }
BENCHMARK_RELATIVE(random_read_short_v2, iters) { randomRead(&schemaShort, dataShortV2, shortRandom, iters); }

BENCHMARK_DRAW_LINE();

BENCHMARK(random_read_long_v1, iters) { randomRead(&schemaLong, dataLongV1, longRandom, iters); }
BENCHMARK_RELATIVE(random_read_long_v2, iters) { randomRead(&schemaLong, dataLongV2, longRandom, iters); }
/*************************
 * End of benchmarks
 ************************/
