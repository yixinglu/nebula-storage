...
class TransactionManager {
 public:
   ...
   * */
  folly::Future<cpp2::ErrorCode> addSamePartEdges(size_t vIdLen, GraphSpaceID spaceId, PartitionID localPart,
                                                  PartitionID remotePart, std::vector<KV>& localEdges,
   ...
   * */
  folly::Future<cpp2::ErrorCode> updateEdgeAtomic(size_t vIdLen, GraphSpaceID spaceId, PartitionID partId,
                                                  const cpp2::EdgeKey& edgeKey, GetBatchFunc batchGetter);
   ...
   * */
  folly::Future<cpp2::ErrorCode> resumeTransaction(size_t vIdLen, GraphSpaceID spaceId, std::string lockKey,
                                                   ResumedResult result = nullptr);

  folly::SemiFuture<kvstore::ResultCode> commitBatch(GraphSpaceID spaceId, PartitionID partId, std::string&& batch);
 ...
 protected:
  folly::SemiFuture<kvstore::ResultCode> commitEdgeOut(GraphSpaceID spaceId, PartitionID partId, std::string&& key,
                                                       std::string&& props);

  folly::SemiFuture<kvstore::ResultCode> commitEdge(GraphSpaceID spaceId, PartitionID partId, std::string& key,
                                                    std::string& encodedProp);

  folly::SemiFuture<cpp2::ErrorCode> eraseKey(GraphSpaceID spaceId, PartitionID partId, const std::string& key);
