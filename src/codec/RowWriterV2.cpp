...
void RowWriterV2::processV2EncodedStr() noexcept {
  ...
  CHECK_EQ(ver, schema_->getVersion()) << "The data is encoded by schema version " << ver
...
WriteResult RowWriterV2::write(ssize_t index, uint8_t v) noexcept { return write(index, static_cast<int8_t>(v)); }
...
WriteResult RowWriterV2::write(ssize_t index, uint16_t v) noexcept { return write(index, static_cast<int16_t>(v)); }
...
WriteResult RowWriterV2::write(ssize_t index, uint32_t v) noexcept { return write(index, static_cast<int32_t>(v)); }
...
WriteResult RowWriterV2::write(ssize_t index, uint64_t v) noexcept { return write(index, static_cast<int64_t>(v)); }
...
WriteResult RowWriterV2::write(ssize_t index, const char* v) noexcept { return write(index, folly::StringPiece(v)); }
...
WriteResult RowWriterV2::write(ssize_t index, const Date& v) noexcept {
  ...
  switch (field->type()) {
    ...
    case meta::cpp2::PropertyType::DATETIME:
      ...
      buf_[offset + sizeof(int16_t)] = v.month;
      buf_[offset + sizeof(int16_t) + sizeof(int8_t)] = v.day;
      memset(&buf_[offset + sizeof(int16_t) + 2 * sizeof(int8_t)], 0, 3 * sizeof(int8_t) + 2 * sizeof(int32_t));
      break;
    default:
      return WriteResult::TYPE_MISMATCH;
  }
...
WriteResult RowWriterV2::write(ssize_t index, const Time& v) noexcept {
  ...
  switch (field->type()) {
    case meta::cpp2::PropertyType::TIME:
      ...
      memcpy(&buf_[offset + 3 * sizeof(int8_t)], reinterpret_cast<const void*>(&v.microsec), sizeof(int32_t));
      break;
    default:
      return WriteResult::TYPE_MISMATCH;
  }
...
WriteResult RowWriterV2::write(ssize_t index, const DateTime& v) noexcept {
  ...
  switch (field->type()) {
    ...
    case meta::cpp2::PropertyType::DATETIME:
      ...
      memcpy(&buf_[offset + sizeof(int16_t) + 5 * sizeof(int8_t)], reinterpret_cast<const void*>(&v.microsec),
             sizeof(int32_t));
      break;
    default:
      return WriteResult::TYPE_MISMATCH;
      ...
      if (field->hasDefault()) {
        ...
        switch (defVal.type()) {
          ...
          case Value::Type::DATETIME:
            ...
            break;
          default:
            LOG(FATAL) << "Unsupported default value type: " << defVal.typeName() << ", default value: " << defVal
