...
#define PRINT_ERROR_MESSAGE(code, value)                                                        \
  if (ResultType::SUCCEEDED == code) {                                                          \
    ...
    LOG(ERROR) << "ResultType : " << static_cast<int>(code) << " Value " << value << std::endl; \
  }

namespace nebula {
namespace dataman {

std::string NebulaCodecImpl::encode(std::vector<Value> values, std::shared_ptr<const meta::SchemaProviderIf> schema) {
