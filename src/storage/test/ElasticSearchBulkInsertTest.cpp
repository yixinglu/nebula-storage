 ...
 protected:
  bool getESEngine() {
    ...
    if (fs::FileUtils::exist(esBin)) {
      ...
      cmd = folly::stringPrintf("cd %s && %s && mkdir es && %s", rootPath_->path(), cp.c_str(), unzip);
    } else {
      auto get =
  ...
  void startESFun() {
    ...
    auto cmd = folly::stringPrintf(
        ...
        "-E cluster.name=%s "
        "> ./output.log 2>&1 &",
        rootPath_->path(), esPort_, cluster);
  ...
  bool waitESReady() {
    auto cmd = folly::stringPrintf("curl -XGET 'http://127.0.0.1:%u/_cluster/health?pretty'", esPort_);
    auto retry = 20;
    while (--retry > 0) {
      auto ret = nebula::ProcessUtils::runCommand(cmd.c_str());
      if (!ret.ok() || ret.value().empty()) {
