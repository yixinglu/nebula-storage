...
class MockHdfsOKHelper : public nebula::hdfs::HdfsHelper {
 public:
  StatusOr<std::string> ls(const std::string& hdfsHost, int32_t hdfsPort, const std::string& hdfsPath) override {
    UNUSED(hdfsHost);
    UNUSED(hdfsPort);
    UNUSED(hdfsPath);
    sleep(1);
  ...
  StatusOr<std::string> copyToLocal(const std::string& hdfsHost, int32_t hdfsPort, const std::string& hdfsPath,
