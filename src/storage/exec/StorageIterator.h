...
class SingleEdgeIterator : public StorageIterator {
 public:
  ...
  SingleEdgeIterator(PlanContext* planCtx, std::unique_ptr<kvstore::KVIterator> iter, EdgeType edgeType,
                     const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>* schemas,
                     const folly::Optional<std::pair<std::string, int64_t>>* ttl, bool moveToValidRecord = true)
    ...
    // If moveToValidRecord is true, iterator will try to move to first valid
    // record, which is used in GetNeighbors. If it is false, it will only check
    // the latest record, which is used in GetProps and UpdateEdge.
