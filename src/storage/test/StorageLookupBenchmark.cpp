...
std::string genVertexIndexKey(meta::SchemaManager* schemaMan, const std::string& prop,
                              std::shared_ptr<nebula::cpp2::IndexItem>& index, VertexID vId) {
...
std::unique_ptr<AdHocSchemaManager> mockSchemaMan() {
  ...
  schemaMan->addTagSchema(spaceId, tagId, std::make_shared<ResultSchemaProvider>(std::move(schema)));
  return schemaMan;
}
...
bool genData(kvstore::KVStore* kv, meta::SchemaManager* schemaMan, std::shared_ptr<nebula::cpp2::IndexItem>& index) {
  ...
  bool ret = false;
  kv->asyncMultiPut(spaceId, partId, std::move(data), [&ret, &baton](kvstore::ResultCode code) {
...
bool processLookup(kvstore::KVStore* kv, meta::SchemaManager* schemaMan, meta::IndexManager* indexMan,
                   const std::string& filter, size_t expectRowNum) {
  ...
  BENCHMARK_SUSPEND {
    ...
    if (resp.rows.size() != expectRowNum) {
      LOG(ERROR) << "Row number error , expected : " << expectRowNum << " actual ： " << resp.rows.size();
      return false;
    }
  };
  return true;
...
void lookupExec(int32_t expectRowNum, const std::string& filter) {
  ...
  BENCHMARK_SUSPEND {
    std::string rootPath;
    rootPath = folly::stringPrintf("%s/%s_%d", FLAGS_root_data_path.c_str(), "Precise", expectRowNum);
