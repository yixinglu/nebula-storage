...
class AddEdgesProcessor : public BaseProcessor<cpp2::ExecResponse> {
 ...
 public:
  static AddEdgesProcessor* instance(StorageEnv* env, const ProcessorCounters* counters = &kAddEdgesCounters) {
 ...
 private:
  ...
  ErrorOr<kvstore::ResultCode, std::string> addEdges(PartitionID partId, const std::vector<kvstore::KV>& edges);

  ErrorOr<kvstore::ResultCode, std::string> findOldValue(PartitionID partId, const folly::StringPiece& rawKey);

  std::string indexKey(PartitionID partId, RowReader* reader, const folly::StringPiece& rawKey,
