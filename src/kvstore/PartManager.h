...
class Handler {
 public:
  ...
  virtual void addPart(GraphSpaceID spaceId, PartitionID partId, bool asLearner,
                       const std::vector<HostAddr>& peers) = 0;

  virtual void updateSpaceOption(GraphSpaceID spaceId, const std::unordered_map<std::string, std::string>& options,
                                 bool isDbOption) = 0;
  ...
  virtual void addListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type,
                           const std::vector<HostAddr>& peers) = 0;

  virtual void removeListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type) = 0;

  virtual void checkRemoteListeners(GraphSpaceID spaceId, PartitionID partId,
...
class MemPartManager final : public PartManager {
 ...
 public:
  ...
  StatusOr<std::vector<meta::RemoteListenerInfo>> listenerPeerExist(GraphSpaceID spaceId, PartitionID partId) override;
...
class MetaServerBasedPartManager : public PartManager, public meta::MetaChangedListener {
 public:
  ...
  void onSpaceOptionUpdated(GraphSpaceID spaceId, const std::unordered_map<std::string, std::string>& options) override;
  ...
  void fetchLeaderInfo(std::unordered_map<GraphSpaceID, std::vector<PartitionID>>& leaderParts) override;

  void onListenerAdded(GraphSpaceID spaceId, PartitionID partId, const meta::ListenerHosts& listenerHosts) override;

  void onListenerRemoved(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type) override;
