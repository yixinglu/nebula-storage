...
struct PropContext {
 public:
  ...
  PropContext(const char* name, const meta::SchemaProviderIf::Field* field, bool returned, bool filtered,
...
struct TagContext {
  ...
  // tagId -> tag schema
  std::unordered_map<TagID, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>> schemas_;
  // tagId -> tag ttl info
...
struct EdgeContext {
  ...
  std::unordered_map<EdgeType, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>> schemas_;
...
class QueryBaseProcessor : public BaseProcessor<RESP> {
 ...
 protected:
  explicit QueryBaseProcessor(StorageEnv* env, const ProcessorCounters* counters, folly::Executor* executor = nullptr,
  ...
  cpp2::ErrorCode checkExp(const Expression* exp, bool returned, bool filtered, bool updated = false);

  void addReturnPropContext(std::vector<PropContext>& ctxs, const char* propName,
  ...
  void addPropContextIfNotExists(std::vector<std::pair<SchemaID, std::vector<PropContext>>>& props,
                                 ...
                                 std::unordered_map<SchemaID, std::string>& names, int32_t entryId,
                                 const std::string* entryName, const std::string* propName,
                                 const meta::SchemaProviderIf::Field* field, bool returned, bool filtered,
                                 const std::pair<size_t, cpp2::StatType>* statInfo = nullptr);
