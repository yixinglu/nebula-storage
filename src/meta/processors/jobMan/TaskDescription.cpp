...
std::string TaskDescription::taskKey() {
  std::string str;
  str.reserve(32);
  str.append(reinterpret_cast<const char*>(JobUtil::jobPrefix().data()), JobUtil::jobPrefix().size())
...
std::string TaskDescription::archiveKey() {
  std::string str;
  str.reserve(32);
  str.append(reinterpret_cast<const char*>(JobUtil::archivePrefix().data()), JobUtil::archivePrefix().size())
      .append(reinterpret_cast<const char*>(&iJob_), sizeof(JobID))
      .append(reinterpret_cast<const char*>(&iTask_), sizeof(TaskID));
  return str;
}
...
std::tuple<HostAddr, Status, int64_t, int64_t> TaskDescription::parseVal(const folly::StringPiece& rawVal) {
  size_t offset = 0;
