...
TestShard::TestShard(size_t idx, std::shared_ptr<RaftexService> svc, PartitionID partId, HostAddr addr,
    ...
    : RaftPart(1,  // clusterId
               1,  // spaceId
               partId, addr, walRoot, ioPool, workers, handlersPool, snapshotMan, getClientMan()),
      idx_(idx),
      service_(svc),
...
std::pair<int64_t, int64_t> TestShard::commitSnapshot(const std::vector<std::string>& data, LogID committedLogId,
  ...
  if (finished) {
    ...
    LOG(INFO) << idStr_ << "Commit the snapshot committedLogId " << committedLogId << ", term " << committedLogTerm;
  }
