...
class TestSingleEdgeIterator : public storage::StorageIterator {
 public:
  explicit TestSingleEdgeIterator(std::unique_ptr<kvstore::KVIterator> iter) : iter_(std::move(iter)) {}
...
TEST_P(ScanEdgePropBench, ProcessEdgeProps) {
  ...
  ASSERT_TRUE(storage::QueryTestUtils::mockBenchEdgeData(env, totalParts, schemaVerCount, rankCount));
  ...
  auto prefix = NebulaKeyUtils::edgePrefix(vIdLen, partId, vId, edgeType);
  {
    ...
    for (; iter->valid(); iter->next(), edgeRowCount++) {
      ...
      auto code = storage::QueryUtils::collectEdgeProps(key, vIdLen, isIntId, wrapper.get(), &props, list);
      ASSERT_TRUE(code.ok());
      result.mutableList().values.emplace_back(std::move(list));
    }
    LOG(WARNING) << "ProcessEdgeProps with schema from map: process " << edgeRowCount << " edges takes "
  ...
  }
  {
    ...
    for (; iter->valid(); iter->next(), edgeRowCount++) {
      auto key = iter->key();
      auto val = iter->val();
      auto reader = RowReaderWrapper::getEdgePropReader(env->schemaMan_, spaceId, std::abs(edgeType), val);
      ASSERT_TRUE(reader.get() != nullptr);
      auto code = storage::QueryUtils::collectEdgeProps(key, vIdLen, isIntId, reader.get(), &props, list);
      ASSERT_TRUE(code.ok());
      result.mutableList().values.emplace_back(std::move(list));
    }
    LOG(WARNING) << "ProcessEdgeProps new reader each time: process " << edgeRowCount << " edges takes "
                 << watch.elapsed().count() << " us.";
  }
  {
    ...
    for (; iter->valid(); iter->next(), edgeRowCount++) {
      auto key = iter->key();
      auto val = iter->val();
      reader = RowReaderWrapper::getEdgePropReader(env->schemaMan_, spaceId, std::abs(edgeType), val);
      ASSERT_TRUE(reader.get() != nullptr);
      auto code = storage::QueryUtils::collectEdgeProps(key, vIdLen, isIntId, reader.get(), &props, list);
      ASSERT_TRUE(code.ok());
      result.mutableList().values.emplace_back(std::move(list));
  ...
  }
  {
    ...
    LOG(WARNING) << "ProcessEdgeProps reader reset with vector schmeas: process " << edgeRowCount << " edges takes "
                 << watch.elapsed().count() << " us.";
  }
  {
    // use the schema saved in processor
    ...
    for (; iter->valid(); iter->next(), edgeRowCount++) {
      ...
      auto code = storage::QueryUtils::collectEdgeProps(key, vIdLen, isIntId, &reader, &props, list);
      ASSERT_TRUE(code.ok());
      result.mutableList().values.emplace_back(std::move(list));
    }
...
INSTANTIATE_TEST_CASE_P(ScanEdgePropBench, ScanEdgePropBench,
                        ::testing::Values(std::make_pair(1, 10000), std::make_pair(10, 10000), std::make_pair(1, 100),
