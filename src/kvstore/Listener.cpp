...
Listener::Listener(GraphSpaceID spaceId, PartitionID partId, HostAddr localAddr, const std::string& walPath,
                   std::shared_ptr<folly::IOThreadPoolExecutor> ioPool,
                   std::shared_ptr<thread::GenericThreadPool> workers, std::shared_ptr<folly::Executor> handlers,
                   std::shared_ptr<raftex::SnapshotManager> snapshotMan, std::shared_ptr<RaftClient> clientMan,
...
void Listener::start(std::vector<HostAddr>&& peers, bool) {
  ...
  LOG(INFO) << idStr_ << "Listener start"
            ...
            << committedLogId_ << ", lastApplyLogId " << lastApplyLogId_ << ", term " << term_;
...
bool Listener::preProcessLog(LogID logId, TermID termId, ClusterID clusterId, const std::string& log) {
  UNUSED(logId);
  UNUSED(termId);
  UNUSED(clusterId);
  if (!log.empty()) {
...
void Listener::doApply() {
  ...
  folly::via(executor_.get(), [this] {
    SCOPE_EXIT { bgWorkers_->addDelayTask(FLAGS_listener_commit_interval_secs * 1000, &Listener::doApply, this); };
...
std::pair<int64_t, int64_t> Listener::commitSnapshot(const std::vector<std::string>& rows, LogID committedLogId,
                                                     TermID committedLogTerm, bool finished) {
  LOG(WARNING) << "Listener snapshot has not implemented yet";
  UNUSED(committedLogId);
  UNUSED(committedLogTerm);
  UNUSED(finished);
  int64_t count = 0;
  int64_t size = 0;
