...
class NebulaKeyUtils final {
 public:
  ...
  static std::string edgeKey(size_t vIdLen, PartitionID partId, const VertexID& srcId, EdgeType type, EdgeRanking rank,
                             const VertexID& dstId, EdgeVerPlaceHolder ev = 1);
   ...
   * Prefix for vertex
   * */
  static std::string vertexPrefix(size_t vIdLen, PartitionID partId, const VertexID& vId, TagID tagId);
   ...
   * Prefix for edge
   * */
  static std::string edgePrefix(size_t vIdLen, PartitionID partId, const VertexID& srcId, EdgeType type);
  ...
  static bool isLock(size_t vIdLen, const folly::StringPiece& rawKey) { return isEdge(vIdLen, rawKey, kLockVersion); }
  ...
  static void dumpBadKey(const folly::StringPiece& rawKey, size_t expect, size_t vIdLen) {
    ...
    msg << "rawKey.size() != expect size"
        << ", rawKey.size() = " << rawKey.size() << ", expect = " << expect << ", vIdLen = " << vIdLen
        << ", rawkey hex format:\n";
