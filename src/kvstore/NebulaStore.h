...
class NebulaStore : public KVStore, public Handler {
 ...
 public:
  NebulaStore(KVOptions options, std::shared_ptr<folly::IOThreadPoolExecutor> ioPool, HostAddr serviceAddr,
    ...
    clientMan_ = std::make_shared<thrift::ThriftClientManager<raftex::cpp2::RaftexServiceAsyncClient>>();
  }

  ~NebulaStore();

  // Calculate the raft service address based on the storage service address
  static HostAddr getRaftAddr(const HostAddr& srvcAddr) { return Utils::getRaftAddrFromStoreAddr(srvcAddr); }

  static HostAddr getStoreAddr(const HostAddr& raftAddr) { return Utils::getStoreAddrFromRaftAddr(raftAddr); }
  ...
  ResultCode get(GraphSpaceID spaceId, PartitionID partId, const std::string& key, std::string* value,
                 bool canReadFromFollower = false) override;

  std::pair<ResultCode, std::vector<Status>> multiGet(GraphSpaceID spaceId, PartitionID partId,
                                                      const std::vector<std::string>& keys,
                                                      std::vector<std::string>* values,
                                                      bool canReadFromFollower = false) override;

  // Get all results in range [start, end)
  ResultCode range(GraphSpaceID spaceId, PartitionID partId, const std::string& start, const std::string& end,
                   std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) override;

  // Delete the overloading with a rvalue `start' and `end'
  ResultCode range(GraphSpaceID spaceId, PartitionID partId, std::string&& start, std::string&& end,
                   std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) override = delete;

  // Get all results with prefix.
  ResultCode prefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix,
                    std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) override;

  // Delete the overloading with a rvalue `prefix'
  ResultCode prefix(GraphSpaceID spaceId, PartitionID partId, std::string&& prefix, std::unique_ptr<KVIterator>* iter,
                    bool canReadFromFollower = false) override = delete;

  // Get all results with prefix starting from start
  ResultCode rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                             const std::string& prefix, std::unique_ptr<KVIterator>* iter,
                             bool canReadFromFollower = false) override;

  // Delete the overloading with a rvalue `prefix'
  ResultCode rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, std::string&& start, std::string&& prefix,
                             std::unique_ptr<KVIterator>* iter, bool canReadFromFollower = false) override = delete;

  ResultCode sync(GraphSpaceID spaceId, PartitionID partId) override;

  // async batch put.
  void asyncMultiPut(GraphSpaceID spaceId, PartitionID partId, std::vector<KV>&& keyValues, KVCallback cb) override;

  void asyncRemove(GraphSpaceID spaceId, PartitionID partId, const std::string& key, KVCallback cb) override;

  void asyncMultiRemove(GraphSpaceID spaceId, PartitionID partId, std::vector<std::string>&& keys,
                        KVCallback cb) override;

  void asyncRemoveRange(GraphSpaceID spaceId, PartitionID partId, const std::string& start, const std::string& end,
                        KVCallback cb) override;

  void asyncAppendBatch(GraphSpaceID spaceId, PartitionID partId, std::string&& batch, KVCallback cb) override;

  void asyncAtomicOp(GraphSpaceID spaceId, PartitionID partId, raftex::AtomicOp op, KVCallback cb) override;

  ErrorOr<ResultCode, std::shared_ptr<Part>> part(GraphSpaceID spaceId, PartitionID partId) override;

  ResultCode ingest(GraphSpaceID spaceId) override;

  ResultCode setOption(GraphSpaceID spaceId, const std::string& configKey, const std::string& configValue);

  ResultCode setDBOption(GraphSpaceID spaceId, const std::string& configKey, const std::string& configValue);

  ResultCode compact(GraphSpaceID spaceId) override;

  ResultCode flush(GraphSpaceID spaceId) override;

  ErrorOr<ResultCode, std::string> createCheckpoint(GraphSpaceID spaceId, const std::string& name) override;
  ...
  void addPart(GraphSpaceID spaceId, PartitionID partId, bool asLearner,
  ...
  ResultCode restoreFromFiles(GraphSpaceID spaceId, const std::vector<std::string>& files) override;

  void addListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type,
                   const std::vector<HostAddr>& peers) override;

  void removeListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type) override;

  void checkRemoteListeners(GraphSpaceID spaceId, PartitionID partId,
                            const std::vector<HostAddr>& remoteListeners) override;

  ResultCode multiPutWithoutReplicator(GraphSpaceID spaceId, std::vector<KV> keyValues) override;
 ...
 private:
  ...
  void updateSpaceOption(GraphSpaceID spaceId, const std::unordered_map<std::string, std::string>& options,
                         bool isDbOption) override;

  std::unique_ptr<KVEngine> newEngine(GraphSpaceID spaceId, const std::string& path);

  std::shared_ptr<Part> newPart(GraphSpaceID spaceId, PartitionID partId, KVEngine* engine, bool asLearner,
                                const std::vector<HostAddr>& defaultPeers);

  std::shared_ptr<Listener> newListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type,
