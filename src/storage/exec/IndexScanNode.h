...
class IndexScanNode : public RelNode<T> {
 public:
  ...
  IndexScanNode(PlanContext* planCtx, IndexID indexId, std::vector<cpp2::IndexColumnHint> columnHints)
      : planContext_(planCtx), indexId_(indexId), columnHints_(std::move(columnHints)) {
    /**
  ...
  kvstore::ResultCode execute(PartitionID partId) override {
    ...
    ret = isRangeScan_ ? planContext_->env_->kvstore_->range(planContext_->spaceId_, partId, scanPair_.first,
                                                             scanPair_.second, &iter)
                       : planContext_->env_->kvstore_->prefix(planContext_->spaceId_, partId, scanPair_.first, &iter);
    if (ret == kvstore::ResultCode::SUCCEEDED && iter && iter->valid()) {
      planContext_->isEdge_ ? iter_.reset(new EdgeIndexIterator(std::move(iter), planContext_->vIdLen_))
 ...
 private:
  StatusOr<std::pair<std::string, std::string>> scanStr(PartitionID partId) {
    auto iRet = planContext_->isEdge_ ? planContext_->env_->indexMan_->getEdgeIndex(planContext_->spaceId_, indexId_)
  ...
  StatusOr<std::pair<std::string, std::string>> getPrefixStr(
    ...
    for (auto& col : columnHints_) {
      auto iter = std::find_if(fields.begin(), fields.end(),
                               [col](const auto& field) { return col.get_column_name() == field.get_name(); });
      if (iter == fields.end()) {
