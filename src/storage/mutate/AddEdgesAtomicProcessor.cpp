...
void AddEdgesAtomicProcessor::processByChain(const cpp2::AddEdgesRequest& req) {
  ...
  for (auto& part : *req.parts_ref()) {
    ...
    for (auto& edge : part.second) {
      auto stPartId = env_->metaClient_->partId(spaceId_, (*(*edge.key_ref()).dst_ref()).getStr());
      if (!stPartId.ok()) {
  ...
  for (auto& chain : edgesByChain) {
    ...
    futures.emplace_back(
        env_->txnMan_->addSamePartEdges(vIdLen_, spaceId_, localPart, remotePart, localData, processor_.get())
            .thenTry([=](auto&& t) {
              ...
              LOG_IF(INFO, FLAGS_trace_toss) << folly::sformat(
                  ...
                  "code={}",
                  spaceId_, localPart, remotePart, static_cast<int32_t>(code));
              if (code != cpp2::ErrorCode::SUCCEEDED) {
                pushResultCode(code, localPart);
              }
            }));
  }
  folly::collectAll(futures).via(env_->txnMan_->getExecutor()).thenValue([=](auto&&) { onFinished(); });
}

cpp2::ErrorCode AddEdgesAtomicProcessor::encodeSingleEdgeProps(const cpp2::NewEdge& e, std::string& encodedVal) {
