...
TEST(StorageClientTest, VerticesInterfacesTest) {
  ...
  auto addrsRet = network::NetworkUtils::toHosts(folly::stringPrintf("127.0.0.1:%d", localMetaPort));
  ...
  // VerticesInterfacesTest(addVertices and getVertexProps)
  {
    ...
    if (!resp.succeeded()) {
      for (auto& err : resp.failedParts()) {
        LOG(ERROR) << "Partition " << err.first << " failed: " << static_cast<int32_t>(err.second);
      }
...

  {
    ...
    for (int i = 0; i < 3; i++) {
      retCols.emplace_back(
          TestUtils::vertexPropDef(folly::stringPrintf("tag_%d_col_%d", 3001 + i * 2, i * 2), 3001 + i * 2));
    }
    ...
    for (auto& vp : results[0].vertices) {
      auto size = std::accumulate(vp.tag_data.cbegin(), vp.tag_data.cend(), 0, [vschema](int acc, auto& td) {
      ...
      checkTagData<std::string>(vp.tag_data, 3005, "tag_3005_col_4", vschema, folly::stringPrintf("string_col_4"));
    }
  }
  ...
  }
  {
    ...
    // Delete a vertex
    {
      ...
      std::vector<cpp2::PropDef> retCols;
      retCols.emplace_back(TestUtils::vertexPropDef(folly::stringPrintf("tag_%d_col_%d", 3001, 0), 3001));
...
class TestStorageServiceRetry : public storage::cpp2::StorageServiceSvIf {
 public:
  ...
  folly::Future<cpp2::QueryResponse> future_getBound(const cpp2::GetNeighborsRequest& req) override {
...
TEST(StorageClientTest, LeaderChangeTest) {
  ...
  tsc.getNeighbors(1, {1, 2, 3}, {0}, "", {}).via(threadPool.get()).thenValue([&](auto&&) { baton.post(); });
  baton.wait();
