...
class ListEdgesProcessor : public BaseProcessor<cpp2::ListEdgesResp> {
 public:
   ...
   * invoked
   */
  static ListEdgesProcessor* instance(kvstore::KVStore* kvstore) { return new ListEdgesProcessor(kvstore); }

  void process(const cpp2::ListEdgesReq& req);

 private:
  explicit ListEdgesProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListEdgesResp>(kvstore) {}
};
