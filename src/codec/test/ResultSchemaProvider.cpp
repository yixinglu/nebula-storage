 ...
 *
 **********************************/
ResultSchemaProvider::ResultSchemaField::ResultSchemaField(std::string name, meta::cpp2::PropertyType type,
                                                           int16_t size, bool nullable, int32_t offset,
                                                           size_t nullFlagPos, Expression* defaultValue)
    : name_(std::move(name)),
      type_(type),
      size_(size),
      nullable_(nullable),
...
bool ResultSchemaProvider::ResultSchemaField::hasDefault() const { return defaultValue_ != nullptr; }

bool ResultSchemaProvider::ResultSchemaField::nullable() const { return nullable_; }

Expression* ResultSchemaProvider::ResultSchemaField::defaultValue() const { return defaultValue_.get(); }
...
const meta::SchemaProviderIf::Field* ResultSchemaProvider::field(const folly::StringPiece name) const {
  auto index = getFieldIndex(name);
  if (index < 0) {
    return nullptr;
  }
