...
class ListEdgeIndexesProcessor : public BaseProcessor<cpp2::ListEdgeIndexesResp> {
 public:
  static ListEdgeIndexesProcessor* instance(kvstore::KVStore* kvstore) { return new ListEdgeIndexesProcessor(kvstore); }

  void process(const cpp2::ListEdgeIndexesReq& req);

 private:
  explicit ListEdgeIndexesProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListEdgeIndexesResp>(kvstore) {}
};
