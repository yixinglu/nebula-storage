...
DEFINE_int64(rocksdb_block_cache, 1024, "The default block cache size used in BlockBasedTable. The unit is MB");

DEFINE_bool(enable_partitioned_index_filter, false, "True for partitioned index filters");

DEFINE_string(rocksdb_compression, "snappy",
...
DEFINE_bool(enable_rocksdb_prefix_filtering, false, "Whether or not to enable rocksdb's prefix bloom filter.");
DEFINE_bool(enable_rocksdb_whole_key_filtering, true, "Whether or not to enable the whole key filtering.");
...
class GraphPrefixTransform : public rocksdb::SliceTransform {
 ...
 public:
  ...
  rocksdb::Slice Transform(const rocksdb::Slice& src) const override { return rocksdb::Slice(src.data(), prefixLen_); }
...
static rocksdb::Status initRocksdbCompression(rocksdb::Options& baseOpts) {
  static std::unordered_map<std::string, rocksdb::CompressionType> m = {{"no", rocksdb::kNoCompression},
...
rocksdb::Status initRocksdbOptions(rocksdb::Options& baseOpts, int32_t vidLen) {
  ...
  s = GetBlockBasedTableOptionsFromMap(rocksdb::BlockBasedTableOptions(), bbtOptsMap, &bbtOpts, true);
  if (!s.ok()) {
    return s;
  }
  ...
  if (FLAGS_enable_rocksdb_prefix_filtering) {
    ...
    baseOpts.prefix_extractor.reset(new GraphPrefixTransform(lengthBeforeVid + vidLen));
  }
