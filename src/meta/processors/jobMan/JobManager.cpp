...
cpp2::ErrorCode JobManager::saveTaskStatus(TaskDescription& td, const cpp2::ReportTaskReq& req) {
  auto code = req.get_code();
  auto status = code == cpp2::ErrorCode::SUCCEEDED ? cpp2::JobStatus::FINISHED : cpp2::JobStatus::FAILED;
  td.setStatus(status);
...
cpp2::ErrorCode JobManager::reportTaskFinish(const cpp2::ReportTaskReq& req) {
  ...
  if (status_ == JbmgrStatus::STOPPED || status_ == JbmgrStatus::NOT_START) {
    LOG(INFO) << folly::sformat("report to an in-active job manager, job={}, task={}", jobId, taskId);
    return cpp2::ErrorCode::E_UNKNOWN;
  }
  ...
  auto task = std::find_if(tasks.begin(), tasks.end(),
                           [&](auto& it) { return it.getJobId() == jobId && it.getTaskId() == taskId; });
  if (task == tasks.end()) {
  ...
  auto allTaskFinished =
      std::none_of(tasks.begin(), tasks.end(), [](auto& tsk) { return tsk.status_ == cpp2::JobStatus::RUNNING; });

  if (allTaskFinished) {
    auto jobStatus =
        std::all_of(tasks.begin(), tasks.end(), [](auto& tsk) { return tsk.status_ == cpp2::JobStatus::FINISHED; })
...
size_t JobManager::jobSize() const { return highPriorityQueue_->size() + lowPriorityQueue_->size(); }
...
ErrorOr<cpp2::ErrorCode, std::vector<cpp2::JobDesc>> JobManager::showJobs() {
  std::unique_ptr<kvstore::KVIterator> iter;
  ...
  std::sort(ret.begin(), ret.end(), [](const auto& a, const auto& b) { return a.get_id() > b.get_id(); });
  return ret;
}

bool JobManager::isExpiredJob(const cpp2::JobDesc& jobDesc) {
  if (*jobDesc.status_ref() == cpp2::JobStatus::QUEUE || *jobDesc.status_ref() == cpp2::JobStatus::RUNNING) {
    return false;
  }
...
bool JobManager::checkJobExist(const cpp2::AdminCmd& cmd, const std::vector<std::string>& paras, JobID& iJob) {
...
ErrorOr<cpp2::ErrorCode, std::pair<cpp2::JobDesc, std::vector<cpp2::TaskDesc>>> JobManager::showJob(JobID iJob) {
...
ResultCode JobManager::save(const std::string& k, const std::string& v) {
  ...
  kvStore_->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&](nebula::kvstore::ResultCode code) {
    rc = code;
    baton.post();
  });
  baton.wait();
