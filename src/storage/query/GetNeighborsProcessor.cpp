...
StoragePlan<VertexID> GetNeighborsProcessor::buildPlan(nebula::DataSet* result, int64_t limit, bool random) {
  /*
  ...
  */
  ...
  for (const auto& tc : tagContext_.propContexts_) {
    auto tag = std::make_unique<TagNode>(planContext_.get(), &tagContext_, tc.first, &tc.second);
  ...
  for (const auto& ec : edgeContext_.propContexts_) {
    auto edge = std::make_unique<SingleEdgeNode>(planContext_.get(), &edgeContext_, ec.first, &ec.second);
    edges.emplace_back(edge.get());
    plan.addNode(std::move(edge));
  }

  auto hashJoin =
      std::make_unique<HashJoinNode>(planContext_.get(), tags, edges, &tagContext_, &edgeContext_, expCtx_.get());
  for (auto* tag : tags) {
  ...
  if (filter_) {
    auto filter = std::make_unique<FilterNode<VertexID>>(planContext_.get(), upstream, expCtx_.get(), filter_.get());
  ...
  if (edgeContext_.statCount_ > 0) {
    auto agg = std::make_unique<AggregateNode<VertexID>>(planContext_.get(), upstream, &edgeContext_);
  ...
  if (random) {
    output = std::make_unique<GetNeighborsSampleNode>(planContext_.get(), join, upstream, &edgeContext_, result, limit);
  } else {
    output = std::make_unique<GetNeighborsNode>(planContext_.get(), join, upstream, &edgeContext_, result, limit);
  }
...
cpp2::ErrorCode GetNeighborsProcessor::buildTagContext(const cpp2::TraverseSpec& req) {
  ...
  if (!req.vertex_props_ref().has_value()) {
    ...
    // not use const reference because we need to modify it when all property
    // need to return
...
cpp2::ErrorCode GetNeighborsProcessor::handleEdgeStatProps(const std::vector<cpp2::StatProp>& statProps) {
    ...
    switch (exp->kind()) {
      ...
      case Expression::Kind::kEdgeProperty: {
        ...
        addPropContextIfNotExists(edgeContext_.propContexts_, edgeContext_.indexMap_, edgeContext_.edgeNames_, edgeType,
                                  edgeName, propName, field, false, false, &statInfo);
        break;
      }
      default: {
        return cpp2::ErrorCode::E_INVALID_STAT_TYPE;
