 ...
 * */
folly::Future<cpp2::ErrorCode> TransactionManager::addSamePartEdges(size_t vIdLen, GraphSpaceID spaceId,
                                                                    PartitionID localPart, PartitionID remotePart,
  ...
  if (FLAGS_trace_toss) {
    for (auto& kv : localEdges) {
      ...
      LOG(INFO) << "begin txn hexSrcDst=" << folly::hexlify(srcId) << folly::hexlify(dstId) << ", txnId=" << txnId;
    }
  }
  ...
  commitBatch(spaceId, localPart, std::move(batch))
      ...
      .thenTry([=, p = std::move(c.first)](auto&& t) mutable {
        ...
        if (code != cpp2::ErrorCode::SUCCEEDED) {
          LOG(INFO) << folly::sformat("commitBatch for ({},{},{}) failed, code={}, txnId={}", spaceId, localPart,
                                      remotePart, static_cast<int32_t>(code), txnId);
          p.setValue(code);
          return;
        }
        ...
        interClient_->forwardTransaction(txnId, spaceId, remotePart, std::move(remoteBatch))
            ...
            .thenTry([=, p = std::move(p)](auto&& _t) mutable {
              ...
              LOG_IF(INFO, FLAGS_trace_toss) << folly::sformat(
                  ...
                  "code={}",
                  txnId, spaceId, remotePart, static_cast<int32_t>(_code));
              ...
              for (auto& lock : lockData) {
                ...
                for (auto& op : operations) {
                  ...
                  auto& kv = op.second;
                  LOG_IF(INFO, FLAGS_trace_toss) << "bat op=" << static_cast<int32_t>(opType)
              ...
              commitBatch(spaceId, localPart, std::move(_batch))
                  ...
                  .thenValue([=, p = std::move(p)](auto&& rc) mutable {
                    ...
                    LOG_IF(INFO, FLAGS_trace_toss)
                        << "txnId=" << txnId << " finished, code=" << static_cast<int32_t>(commitBatchCode);
...
folly::Future<cpp2::ErrorCode> TransactionManager::updateEdgeAtomic(size_t vIdLen, GraphSpaceID spaceId,
                                                                    PartitionID partId, const cpp2::EdgeKey& edgeKey,
...
folly::Future<cpp2::ErrorCode> TransactionManager::resumeTransaction(size_t vIdLen, GraphSpaceID spaceId,
                                                                     std::string lockKey, ResumedResult result) {
  ...
  interClient_->getValue(vIdLen, spaceId, remoteKey)
      ...
      .thenValue([=](auto&& errOrVal) mutable {
        if (!nebula::ok(errOrVal)) {
          LOG_IF(INFO, FLAGS_trace_toss) << "get remote key failed, lock=" << folly::hexlify(lockKey);
        ...
        LOG_IF(INFO, FLAGS_trace_toss) << "got value=[" << kv.second
                                       << "] from remote key=" << folly::hexlify(remoteKey)
                                       << ", according to lock=" << folly::hexlify(lockKey);
      ...
      .thenValue([=](auto&&) {
        ...
        if (*spPromiseVal == cpp2::ErrorCode::SUCCEEDED || *spPromiseVal == cpp2::ErrorCode::E_KEY_NOT_FOUND ||
      ...
      .ensure([=, p = std::move(c.first)]() mutable {
        ...
        LOG_IF(INFO, FLAGS_trace_toss) << "end resume *spPromiseVal=" << static_cast<int32_t>(*spPromiseVal);
...
folly::SemiFuture<kvstore::ResultCode> TransactionManager::commitBatch(GraphSpaceID spaceId, PartitionID partId,
                                                                       std::string&& batch) {
  auto c = folly::makePromiseContract<kvstore::ResultCode>();
  env_->kvstore_->asyncAppendBatch(spaceId, partId, std::move(batch),
                                   [pro = std::move(c.first)](kvstore::ResultCode rc) mutable { pro.setValue(rc); });
 ...
 * */
folly::SemiFuture<kvstore::ResultCode> TransactionManager::commitEdgeOut(GraphSpaceID spaceId, PartitionID partId,
                                                                         std::string&& key, std::string&& props) {
  ...
  if (!indexes.empty()) {
    ...
    auto cb = [pro = std::move(c.first)](kvstore::ResultCode rc) mutable { pro.setValue(rc); };
...
folly::SemiFuture<kvstore::ResultCode> TransactionManager::commitEdge(GraphSpaceID spaceId, PartitionID partId,
                                                                      std::string& key, std::string& props) {
...
folly::SemiFuture<cpp2::ErrorCode> TransactionManager::eraseKey(GraphSpaceID spaceId, PartitionID partId,
                                                                const std::string& key) {
  LOG_IF(INFO, FLAGS_trace_toss) << "eraseKey: " << folly::hexlify(key);
  auto c = folly::makePromiseContract<cpp2::ErrorCode>();
  env_->kvstore_->asyncRemove(spaceId, partId, key, [pro = std::move(c.first)](kvstore::ResultCode code) mutable {
