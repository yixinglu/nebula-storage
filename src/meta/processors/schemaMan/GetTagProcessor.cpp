...
void GetTagProcessor::process(const cpp2::GetTagReq& req) {
  ...
  if (req.get_version() < 0) {
    ...
    if (!ret.ok()) {
      LOG(ERROR) << "Get Tag SpaceID: " << req.get_space_id() << ", tagName: " << req.get_tag_name() << ", version "
    ...
    auto tagKey = MetaServiceUtils::schemaTagKey(req.get_space_id(), tagId, req.get_version());
  ...
  VLOG(3) << "Get Tag SpaceID: " << req.get_space_id() << ", tagName: " << req.get_tag_name() << ", version "
          << req.get_version();
