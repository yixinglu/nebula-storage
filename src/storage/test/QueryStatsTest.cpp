...
void buildRequest(cpp2::GetNeighborsRequest& req) {
  ...
  for (int i = 0; i < 2; i++) {
    tmpColumns.emplace_back(TestUtils::vertexPropDef(folly::stringPrintf("tag_%d_col_%d", 3001 + i * 2, i * 2),
                                                     cpp2::StatType::AVG, 3001 + i * 2));
  }
  // Return edge props col_0, col_2, col_4 ... col_18
  for (int i = 0; i < 5; i++) {
    tmpColumns.emplace_back(TestUtils::edgePropDef(folly::stringPrintf("col_%d", i * 2), cpp2::StatType::SUM, 101));
  }
...
TEST(QueryStatsTest, StatsSimpleTest) {
  ...
  auto* processor = QueryStatsProcessor::instance(kv.get(), schemaMan.get(), nullptr, executor.get());
