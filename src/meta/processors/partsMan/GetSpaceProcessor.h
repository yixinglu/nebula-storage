...
class GetSpaceProcessor : public BaseProcessor<cpp2::GetSpaceResp> {
 public:
  static GetSpaceProcessor* instance(kvstore::KVStore* kvstore) { return new GetSpaceProcessor(kvstore); }

  void process(const cpp2::GetSpaceReq& req);

 private:
  explicit GetSpaceProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::GetSpaceResp>(kvstore) {}
};
