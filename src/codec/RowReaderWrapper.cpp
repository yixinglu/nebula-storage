...
// static
RowReaderWrapper RowReaderWrapper::getTagPropReader(meta::SchemaManager* schemaMan, GraphSpaceID space, TagID tag,
...
// static
RowReaderWrapper RowReaderWrapper::getEdgePropReader(meta::SchemaManager* schemaMan, GraphSpaceID space, EdgeType edge,
                                                     folly::StringPiece row) {
...
// static
RowReaderWrapper RowReaderWrapper::getRowReader(const meta::SchemaProviderIf* schema, folly::StringPiece row) {
  SchemaVer schemaVer;
  int32_t readerVer;
  RowReaderWrapper::getVersions(row, schemaVer, readerVer);
...
RowReaderWrapper RowReaderWrapper::getRowReader(
    const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>& schemas, folly::StringPiece row) {
  SchemaVer schemaVer;
  int32_t readerVer;
  RowReaderWrapper::getVersions(row, schemaVer, readerVer);
  if (static_cast<size_t>(schemaVer) >= schemas.size() || schemaVer != schemas[schemaVer]->getVersion()) {
    return RowReaderWrapper();
  }
  return RowReaderWrapper(schemas[schemaVer].get(), row, readerVer);
}

RowReaderWrapper::RowReaderWrapper(const meta::SchemaProviderIf* schema, const folly::StringPiece& row,
                                   int32_t& readerVer)
...
bool RowReaderWrapper::reset(meta::SchemaProviderIf const* schema, folly::StringPiece row, int32_t readerVer) noexcept {
  CHECK_NOTNULL(schema);
  readerVer_ = readerVer;
  if (readerVer_ == 1) {
    readerV1_.resetImpl(schema, row);
...
bool RowReaderWrapper::reset(meta::SchemaProviderIf const* schema, folly::StringPiece row) noexcept {
  currReader_ = nullptr;
  if (schema == nullptr) {
    return false;
  }
...
bool RowReaderWrapper::reset(const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>& schemas,
...
// static
void RowReaderWrapper::getVersions(const folly::StringPiece& row, SchemaVer& schemaVer, int32_t& readerVer) {
  size_t index = 0;
  if (row.empty()) {
