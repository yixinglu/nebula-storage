...
If we need to read value from a user defined plase, just set the related value
...
class StorageExpressionContext final : public ExpressionContext {
 public:
  explicit StorageExpressionContext(size_t vIdLen, bool isIntId, const std::string& name = "",
                                    const meta::NebulaSchemaProvider* schema = nullptr, bool isEdge = false)
      : vIdLen_(vIdLen), isIntId_(isIntId), name_(name), schema_(schema), isEdge_(isEdge) {}

  StorageExpressionContext(size_t vIdLen, bool isIntId, bool hasNullableCol,
  ...
  const Value& getVersionedVar(const std::string&, int64_t) const override { return Value::kNullValue; }

  // Get the specified property from a variable, such as $a.prop_name
  const Value& getVarProp(const std::string&, const std::string&) const override { return Value::kNullValue; }

  // Get the specified property of tagName from the destination vertex,
  // such as $$.tagName.prop_name
  const Value& getDstProp(const std::string&, const std::string&) const override { return Value::kNullValue; }
  ...
  void setVar(const std::string& name, Value val) override { valueMap_[name].emplace_back(std::move(val)); }
