...
TEST(MetaClientTest, InterfacesTest) {
  ...
  TestUtils::createSomeHosts(kv);
  {
    ...
    }
    {
      ...
      ArithmeticExpression defaultValue(Expression::Kind::kDivision, new ConstantExpression(1),
    ...
    }
    {
      ...
      auto retVer = client->getLatestEdgeVersionFromCache(spaceId, ret1.value().begin()->get_edge_type());
      CHECK(retVer.ok());
      auto ver = retVer.value();
      auto ret2 = client->getEdgeSchemaFromCache(spaceId, ret1.value().begin()->get_edge_type(), ver);
      ASSERT_TRUE(ret2.ok()) << ret2.status();
      ASSERT_EQ(ret2.value()->getNumFields(), 5);
...
TEST(MetaClientTest, TagTest) {
  ...
  // Test wrong format timestamp in default value
  {
    ...
    auto result = client->createTagSchema(spaceId, "test_tag_wrong_default_timestamp1", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test wrong format timestamp in default value
  {
    cpp2::Schema schema = genSchema("2010-10-10 10:00:00", PropertyType::TIMESTAMP, true);
    auto result = client->createTagSchema(spaceId, "test_tag_wrong_default_timestamp2", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test right format timestamp in default value
  {
    cpp2::Schema schema = genSchema("2010-10-10T10:00:00", PropertyType::TIMESTAMP, true);
    auto result = client->createTagSchema(spaceId, "test_tag_right_default_timestamp2", std::move(schema)).get();
    ASSERT_TRUE(result.ok());
  }
  // Test out of range of int8
  {
    cpp2::Schema schema = genSchema(-129, PropertyType::INT8);
    auto result = client->createTagSchema(spaceId, "test_tag_less_min_in8", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test out of range of int8
  {
    cpp2::Schema schema = genSchema(128, PropertyType::INT8);
    auto result = client->createTagSchema(spaceId, "test_tag_large_max_in8", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test border of int8
  {
  ...
  // Test out of range of int16
  {
    ...
    auto result = client->createTagSchema(spaceId, "test_tag_large_max_int16", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test out of range of int16
  {
    cpp2::Schema schema = genSchema(-32769, PropertyType::INT16);
    auto result = client->createTagSchema(spaceId, "test_tag_less_min_int16", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  ...
  // Test out of range of int32
  {
    ...
    auto result = client->createTagSchema(spaceId, "test_tag_large_max_int32", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
  // Test out of range of int32
  {
    cpp2::Schema schema = genSchema(-2147483649, PropertyType::INT32);
    auto result = client->createTagSchema(spaceId, "test_tag_less_min_int32", std::move(schema)).get();
    ASSERT_FALSE(result.ok());
  }
...
TEST(MetaClientTest, TagIndexTest) {
  ...
  }
  {
    ...
    auto result = client->createTagIndex(space, "tag_multi_field_index", "tag_0", std::move(fields)).get();
    ASSERT_TRUE(result.ok());
  ...
  }
  {
    ...
    auto result = client->createTagIndex(space, "tag_field_not_exist_index", "tag_0", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
  ...
  }
  {
    ...
    auto result = client->createTagIndex(space, "tag_not_exist_index", "tag_not_exist", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
    ASSERT_EQ(Status::Error("not existed!"), result.status());
  }
  {
    ...
    auto result = client->createTagIndex(space, "tag_conflict_index", "tag_0", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
...
TEST(MetaClientTest, EdgeIndexTest) {
  ...
  IndexID multiFieldIndexID;
  {
    for (auto i = 0; i < 2; i++) {
      ...
      cpp2::Schema schema;
      schema.set_columns(std::move(columns));
      auto result = client->createEdgeSchema(space, folly::stringPrintf("edge_%d", i), schema).get();
      ASSERT_TRUE(result.ok());
    }
  }
  {
    ...
    auto result = client->createEdgeIndex(space, "edge_single_field_index", "edge_0", {field}).get();
    ASSERT_TRUE(result.ok());
    singleFieldIndexID = result.value();
  }
  {
    ...
    fields.emplace_back(std::move(field1));
    fields.emplace_back(std::move(field2));
    auto result = client->createEdgeIndex(space, "edge_multi_field_index", "edge_0", std::move(fields)).get();
    ASSERT_TRUE(result.ok());
    multiFieldIndexID = result.value();
  }
  {
    ...
    fields.emplace_back(std::move(field1));
    fields.emplace_back(std::move(field2));
    auto result = client->createEdgeIndex(space, "edge_not_exist_index", "edge_not_exist", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
    ASSERT_EQ(Status::Error("not existed!"), result.status());
  }
  {
    ...
    fields.emplace_back(std::move(field1));
    fields.emplace_back(std::move(field2));
    auto result = client->createEdgeIndex(space, "edge_conflict_index", "edge_0", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
    ASSERT_EQ(Status::Error("conflict"), result.status());
  }
  {
    ...
    fields.emplace_back(std::move(field1));
    fields.emplace_back(std::move(field2));
    auto result = client->createEdgeIndex(space, "edge_field_not_exist_index", "edge_0", std::move(fields)).get();
    ASSERT_FALSE(result.ok());
    ASSERT_EQ(Status::Error("not existed!"), result.status());
  }
  {
...
class TestListener : public MetaChangedListener {
 public:
  ...
  void onSpaceOptionUpdated(GraphSpaceID, const std::unordered_map<std::string, std::string>& update) override {
  ...
  void fetchLeaderInfo(std::unordered_map<GraphSpaceID, std::vector<PartitionID>>&) override {
    LOG(INFO) << "Get leader distribution!";
  }

  void onListenerAdded(GraphSpaceID spaceId, PartitionID partId, const ListenerHosts& listenerHosts) override {
    UNUSED(spaceId);
    UNUSED(partId);
  ...
  void onListenerRemoved(GraphSpaceID spaceId, PartitionID partId, cpp2::ListenerType type) override {
    UNUSED(spaceId);
    UNUSED(partId);
    UNUSED(type);
    listenerPartNum--;
  ...
  void onCheckRemoteListeners(GraphSpaceID spaceId, PartitionID partId,
...
TEST(MetaClientTest, ListenerDiffTest) {
  ...
  TestUtils::registerHB(kv, hosts);
  {
    ...
    auto addRet = console->addListener(spaceId, cpp2::ListenerType::ELASTICSEARCH, {listenerHost}).get();
...
TEST(MetaClientTest, SimpleTest) {
  ...
  auto client = std::make_shared<MetaClient>(threadPool, std::vector<HostAddr>{HostAddr(localIp, mockMetad->port_)});
  {
...
cpp2::ConfigItem initConfigItem(cpp2::ConfigModule module, std::string name, cpp2::ConfigMode mode, Value value) {
...
TEST(MetaClientTest, Config) {
  ...
  // Reg
  {
    ...
    configItems.emplace_back(initConfigItem(cpp2::ConfigModule::GRAPH, "minloglevel", cpp2::ConfigMode::MUTABLE, 2));
    configItems.emplace_back(initConfigItem(cpp2::ConfigModule::META, "minloglevel", cpp2::ConfigMode::MUTABLE, 2));
    configItems.emplace_back(initConfigItem(cpp2::ConfigModule::STORAGE, "minloglevel", cpp2::ConfigMode::MUTABLE, 2));
...
TEST(MetaClientTest, ListenerTest) {
  ...
  std::vector<HostAddr> listenerHosts = {{"1", 0}, {"1", 1}, {"1", 2}, {"1", 3}};
  {
    ...
    auto addRet = client->addListener(space, cpp2::ListenerType::ELASTICSEARCH, listenerHosts).get();
    ASSERT_TRUE(addRet.ok()) << addRet.status();
  }
  {
