...
kvstore::ResultCode StatisJobExecutor::save(const std::string& key, const std::string& val) {
...
void StatisJobExecutor::doRemove(const std::string& key) {
  folly::Baton<true, std::atomic> baton;
  kvstore_->asyncRemove(kDefaultSpaceId, kDefaultPartId, key, [&](nebula::kvstore::ResultCode) { baton.post(); });
  baton.wait();
}
...
folly::Future<Status> StatisJobExecutor::executeInternal(HostAddr&& address, std::vector<PartitionID>&& parts) {
  cpp2::StatisItem item;
  statisItem_.emplace(address, item);
  return adminClient_->addTask(cpp2::AdminCmd::STATS, jobId_, taskId_++, space_, {std::move(address)}, {},
