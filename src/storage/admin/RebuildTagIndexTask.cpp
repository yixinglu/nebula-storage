...
StatusOr<std::shared_ptr<meta::cpp2::IndexItem>> RebuildTagIndexTask::getIndex(GraphSpaceID space, IndexID index) {
  return env_->indexMan_->getTagIndex(space, index);
}

kvstore::ResultCode RebuildTagIndexTask::buildIndexGlobal(GraphSpaceID space, PartitionID part,
  ...
  while (iter && iter->valid()) {
    ...
    for (const auto& item : items) {
      if (item->get_schema_id().get_tag_id() == tagID) {
        ...
        auto indexKey = IndexKeyUtils::vertexIndexKey(vidSize, part, item->get_index_id(), vertex.toString(),
