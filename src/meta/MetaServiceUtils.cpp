...
static const std::unordered_map<std::string, std::pair<std::string, bool>> systemTableMaps = {
    {"users", {"__users__", true}},           {"hosts", {"__hosts__", false}},  {"snapshots", {"__snapshots__", false}},
    {"configs", {"__configs__", true}},       {"groups", {"__groups__", true}}, {"zones", {"__zones__", true}},
    {"ft_service", {"__ft_service__", false}}};

// name => {prefix, parseSpaceid}, nullptr means that the backup should be
// skipped.
static const std::unordered_map<std::string, std::pair<std::string, std::function<decltype(MetaServiceUtils::spaceId)>>>
    tableMaps = {{"spaces", {"__spaces__", MetaServiceUtils::spaceId}},
...
bool backupTable(kvstore::KVStore* kvstore, const std::string& backupName, const std::string& tableName,
                 std::vector<std::string>& files, std::function<bool(const folly::StringPiece& key)> filter) {
  ...
  files.insert(files.end(), std::make_move_iterator(value(backupFilePath).begin()),
...
std::string MetaServiceUtils::spaceName(folly::StringPiece rawVal) { return parseSpace(rawVal).get_space_name(); }
...
PartitionID MetaServiceUtils::parsePartKeyPartId(folly::StringPiece key) {
  return *reinterpret_cast<const PartitionID*>(key.data() + kPartsTable.size() + sizeof(GraphSpaceID));
}

std::string MetaServiceUtils::partVal(const std::vector<HostAddr>& hosts) { return partValV2(hosts); }
...
std::string MetaServiceUtils::leaderKey(std::string addr, Port port) { return leaderKeyV2(addr, port); }
...
std::string MetaServiceUtils::leaderKeyV2(std::string addr, Port port) {
  ...
  key.append(kLeadersTable.data(), kLeadersTable.size()).append(MetaServiceUtils::serializeHostAddr(h));
  return key;
}
...
const std::string& MetaServiceUtils::leaderPrefix() { return kLeadersTable; }
...
std::string MetaServiceUtils::schemaEdgeKey(GraphSpaceID spaceId, EdgeType edgeType, SchemaVer version) {
...
SchemaVer MetaServiceUtils::parseEdgeVersion(folly::StringPiece key) {
  ...
  return std::numeric_limits<SchemaVer>::max() - *reinterpret_cast<const SchemaVer*>(key.begin() + offset);
}
...
IndexID MetaServiceUtils::parseIndexesKeyIndexID(folly::StringPiece key) {
  return *reinterpret_cast<const IndexID*>(key.data() + kIndexesTable.size() + sizeof(GraphSpaceID));
}
...
std::string MetaServiceUtils::rebuildIndexStatus(GraphSpaceID space, char type, const std::string& indexName) {
  std::string key;
  key.reserve(64);
...
GraphSpaceID MetaServiceUtils::parseIndexKeySpaceID(folly::StringPiece key) {
  return *reinterpret_cast<const GraphSpaceID*>(key.data() + kIndexTable.size() + sizeof(EntryType));
}
...
std::string MetaServiceUtils::assembleSegmentKey(const std::string& segment, const std::string& key) {
...
cpp2::ErrorCode MetaServiceUtils::alterColumnDefs(std::vector<cpp2::ColumnDef>& cols, cpp2::SchemaProp& prop,
                                                  const cpp2::ColumnDef col, const cpp2::AlterSchemaOp op) {
  switch (op) {
...
cpp2::ErrorCode MetaServiceUtils::alterSchemaProp(std::vector<cpp2::ColumnDef>& cols, cpp2::SchemaProp& schemaProp,
                                                  cpp2::SchemaProp alterSchemaProp, bool existIndex) {
  if (existIndex && (alterSchemaProp.ttl_duration_ref().has_value() || alterSchemaProp.ttl_col_ref().has_value())) {
  ...
  if ((schemaProp.get_ttl_duration() && (*schemaProp.get_ttl_duration() != 0)) &&
      (!schemaProp.get_ttl_col() || (schemaProp.get_ttl_col() && schemaProp.get_ttl_col()->empty()))) {
...
cpp2::ConfigItem MetaServiceUtils::parseConfigValue(folly::StringPiece rawData) {
  ...
  Value value;
  apache::thrift::CompactSerializer::deserialize(rawData.subpiece(offset, rawData.size() - offset), value);
...
std::string MetaServiceUtils::snapshotVal(const cpp2::SnapshotStatus& status, const std::string& hosts) {
  std::string val;
...
std::string MetaServiceUtils::parseSnapshotHosts(folly::StringPiece rawData) {
  return rawData.subpiece(sizeof(cpp2::SnapshotStatus), rawData.size() - sizeof(cpp2::SnapshotStatus)).str();
}
...
std::function<bool(const folly::StringPiece& key)> MetaServiceUtils::spaceFilter(
    const std::unordered_set<GraphSpaceID>& spaces, std::function<GraphSpaceID(folly::StringPiece rawKey)> parseSpace) {
...
ErrorOr<kvstore::ResultCode, std::vector<std::string>> MetaServiceUtils::backupIndexTable(
    kvstore::KVStore* kvstore, const std::unordered_set<GraphSpaceID>& spaces, const std::string& backupName,
    const std::vector<std::string>* spaceName) {
  return kvstore->backupTable(
      kDefaultSpaceId, backupName, kIndexTable, [spaces, spaceName](const folly::StringPiece& key) -> bool {
        ...
        if (type == EntryType::SPACE) {
          auto sn =
              key.subpiece(kIndexTable.size() + sizeof(EntryType), key.size() - kIndexTable.size() - sizeof(EntryType))
                  .str();
          LOG(INFO) << "sn was " << sn;
          auto it = std::find_if(spaceName->cbegin(), spaceName->cend(), [&sn](auto& name) { return sn == name; });
...
folly::Optional<std::vector<std::string>> MetaServiceUtils::backup(kvstore::KVStore* kvstore,
  ...
  for (const auto& table : tableMaps) {
    ...
    if (!backupTable(kvstore, backupName, table.second.first, files, spaceFilter(spaces, table.second.second))) {
      return folly::none;
    }
  ...
  files.insert(files.end(), std::make_move_iterator(value(ret).begin()), std::make_move_iterator(value(ret).end()));

  return files;
}

bool MetaServiceUtils::replaceHostInPartition(kvstore::KVStore* kvstore, const HostAddr& ipv4From,
                                              const HostAddr& ipv4To, bool direct) {
  ...
  kvstore->asyncMultiPut(kDefaultSpaceId, kDefaultPartId, std::move(data), [&](kvstore::ResultCode code) {
...
bool MetaServiceUtils::replaceHostInZone(kvstore::KVStore* kvstore, const HostAddr& ipv4From, const HostAddr& ipv4To,
                                         bool direct) {
  folly::SharedMutex::WriteHolder wHolder(LockUtils::spaceLock());
...
std::string MetaServiceUtils::balanceTaskKey(BalanceID balanceId, GraphSpaceID spaceId, PartitionID partId,
...
std::string MetaServiceUtils::balanceTaskVal(BalanceTaskStatus status, BalanceTaskResult retult, int64_t startTime,
                                             int64_t endTime) {
  std::string val;
  val.reserve(32);
...
std::tuple<BalanceID, GraphSpaceID, PartitionID, HostAddr, HostAddr> MetaServiceUtils::parseBalanceTaskKey(
...
std::tuple<BalanceTaskStatus, BalanceTaskResult, int64_t, int64_t> MetaServiceUtils::parseBalanceTaskVal(
...
std::string MetaServiceUtils::groupVal(const std::vector<std::string>& zones) { return folly::join(",", zones); }
...
std::string MetaServiceUtils::listenerKey(GraphSpaceID spaceId, PartitionID partId, cpp2::ListenerType type) {
  std::string key;
  key.reserve(kListenerTable.size() + sizeof(GraphSpaceID) + sizeof(cpp2::ListenerType) + sizeof(PartitionID));
...
std::string MetaServiceUtils::fulltextServiceVal(cpp2::FTServiceType type, const std::vector<cpp2::FTClient>& clients) {
  std::string val, cval;
