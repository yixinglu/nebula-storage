...
class QueryUtils final {
 public:
  ...
  static StatusOr<nebula::Value> readValue(RowReader* reader, const std::string& propName,
  ...
  static StatusOr<nebula::Value> readEdgeProp(folly::StringPiece key, size_t vIdLen, bool isIntId, RowReader* reader,
  ...
  static StatusOr<nebula::Value> readVertexProp(folly::StringPiece key, size_t vIdLen, bool isIntId, RowReader* reader,
                                                const PropContext& prop) {
    switch (prop.propInKeyType_) {
      // prop in value
      case PropContext::PropInKeyType::NONE: {
  ...
  static Status collectVertexProps(folly::StringPiece key, size_t vIdLen, bool isIntId, RowReader* reader,
                                   const std::vector<PropContext>* props, nebula::List& list) {
  ...
  static Status collectEdgeProps(folly::StringPiece key, size_t vIdLen, bool isIntId, RowReader* reader,
                                 const std::vector<PropContext>* props, nebula::List& list) {
    for (const auto& prop : *props) {
      if (prop.returned_) {
        VLOG(2) << "Collect prop " << prop.name_;
  ...
  static folly::Optional<std::pair<std::string, int64_t>> getEdgeTTLInfo(EdgeContext* edgeContext, EdgeType edgeType) {
  ...
  // return none if no valid ttl, else return the ttl property name and time
  static folly::Optional<std::pair<std::string, int64_t>> getTagTTLInfo(TagContext* tagContext, TagID tagId) {
    folly::Optional<std::pair<std::string, int64_t>> ret;
