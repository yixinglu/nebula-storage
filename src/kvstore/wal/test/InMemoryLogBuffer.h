...
class InMemoryLogBuffer final {
 public:
  explicit InMemoryLogBuffer(LogID firstLogId, const std::string& idStr = "") : firstLogId_(firstLogId), idStr_(idStr) {
    VLOG(1) << idStr_ << "InMemoryLogBuffer ctor, firstLogId " << firstLogId_;
    logs_.reserve(1024);
  }

  ~InMemoryLogBuffer() { VLOG(1) << idStr_ << "InMemoryLogBuffer dtor, firstLogId " << firstLogId_; }
  ...
  std::pair<LogID, TermID> accessAllLogs(std::function<void(LogID, TermID, ClusterID, const std::string&)> fn) const;
