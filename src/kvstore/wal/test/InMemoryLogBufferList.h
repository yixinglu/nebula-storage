  ...
  class Iterator : public LogIterator {
   ...
   public:
    Iterator(std::shared_ptr<InMemoryBufferList> imBufferList, LogID start, LogID end)
        : imBufferList_(imBufferList), currId_(start), end_(std::min(end, imBufferList->latestLogId_.load())) {
    ...
    LogIterator& operator++() override {
      ...
      if (currId_ >= nextFirstId_) {
        ...
        if (buffers_.size() == 1) {
          LOG(FATAL) << "currId_ " << currId_ << ", nextFirstId_ " << nextFirstId_ << ", firstIdInBuffer_ "
                     << firstIdInBuffer_ << ", lastId_ " << end_ << ", firstLogId in buffer "
                     << buffers_.front()->firstLogId() << ", lastLogId in buffer " << buffers_.front()->lastLogId()
