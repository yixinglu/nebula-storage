...
namespace nebula {
namespace meta {
ErrorOr<cpp2::ErrorCode, std::unordered_map<GraphSpaceID, std::vector<cpp2::CheckpointInfo>>> Snapshot::createSnapshot(
  ...
  for (const auto& spaceHosts : spacesHosts) {
    for (const auto& host : spaceHosts.second) {
      ...
      info[spaceHosts.first].emplace_back(apache::thrift::FRAGILE, host, status.value());
    }
  }
  return info;
}

cpp2::ErrorCode Snapshot::dropSnapshot(const std::string& name, const std::vector<HostAddr>& hosts) {
  auto retSpacesHosts = getSpacesHosts();
  if (!retSpacesHosts.ok()) {
    return cpp2::ErrorCode::E_STORE_FAILURE;
  }
    ...
    for (const auto& host : spaceHosts.second) {
      if (std::find(hosts.begin(), hosts.end(), host) != hosts.end()) {
        ...
        if (!status.ok()) {
          ...
          auto error = folly::stringPrintf(msg, name.c_str(), host.toString().c_str(), status.toString().c_str());
          LOG(ERROR) << error;
        }
      }
    }
