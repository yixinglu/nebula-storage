...
class TossEdgeIterator : public SingleEdgeIterator {
 public:
  TossEdgeIterator(PlanContext* planCtx, std::unique_ptr<kvstore::KVIterator> iter, EdgeType edgeType,
                   const std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>* schemas,
                   const folly::Optional<std::pair<std::string, int64_t>>* ttl, bool stopAtFirstEdge)
      : SingleEdgeIterator() {
    ...
    LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::ctor() iter_->key=" << folly::hexlify(iter_->key());
    next();
  }
  ...
  bool valid() const override {
    ...
    if (FLAGS_trace_toss) {
      ...
      LOG(INFO) << "TossEdgeIterator::valid() = " << ret << ", key=" << folly::hexlify(key) << ", val=" << val;
    }
    return ret;
  }
   ...
   */
  void next() override {
    LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::next() iter_->key=" << folly::hexlify(iter_->key())
    ...
    while (iter_->valid()) {
      ...
      if (isEdge(iter_->key())) {
        LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::next(), found an edge, hex="
        ...
        if (isLatestEdge(iter_->key()) && setReader(iter_->val())) {
          ...
          lastIsLock_ = false;
          LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::next(), return edge hex="
                                         << TransactionUtils::hexEdgeId(planContext_->vIdLen_, iter_->key());
          return;
        } else {
         ...
         */
        if (lastIsLock_) {
          LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::next(), prev is a lock, hexEdgeId="
                                         << TransactionUtils::hexEdgeId(planContext_->vIdLen_, iter_->key())
                                         << ", hex=" << folly::hexlify(iter_->key());
          auto tryLockData = recoverEdges_.back()->tryWLock();
          if (tryLockData && tryLockData->first.empty()) {
            LOG_IF(INFO, FLAGS_trace_toss) << "set edge val for key=" << folly::hexlify(iter_->key());
        ...
        LOG_IF(INFO, FLAGS_trace_toss) << "TossEdgeIterator::next() found a lock, hex="
                                       << TransactionUtils::hexEdgeId(planContext_->vIdLen_, iter_->key());
        ...
        if (rank != lastRank_ || dstId != lastDstId_) {
          recoverEdges_.emplace_back(std::make_shared<folly::Synchronized<KV>>(std::make_pair("", "")));
        }

        resumeTasks_.emplace_back(planContext_->env_->txnMan_->resumeTransaction(
            planContext_->vIdLen_, planContext_->spaceId_, iter_->key().str(), recoverEdges_.back()));
  ...
  bool isEdge(const folly::StringPiece& key) { return NebulaKeyUtils::isEdge(planContext_->vIdLen_, key); }
  ...
  bool setReader(folly::StringPiece val) {
    ...
    if (hasTtl_ && CommonUtils::checkDataExpiredForTTL(schemas_->back().get(), reader_.get(), ttlCol_, ttlDuration_)) {
      reader_.reset();
      return false;
    }
    return true;
