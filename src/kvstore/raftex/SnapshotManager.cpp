...
folly::Future<Status> SnapshotManager::sendSnapshot(std::shared_ptr<RaftPart> part, const HostAddr& dst) {
  ...
  executor_->add([this, p = std::move(p), part, dst]() mutable {
    ...
    LOG(INFO) << part->idStr_ << "Begin to send the snapshot"
              << ", commitLogId = " << commitLogIdAndTerm.first << ", commitLogTerm = " << commitLogIdAndTerm.second;
    accessAllRowsInSnapshot(
        spaceId, partId,
        [&, this, p = std::move(p)](const std::vector<std::string>& data, int64_t totalCount, int64_t totalSize,
          ...
          while (retry-- > 0) {
            auto f = send(spaceId, partId, termId, commitLogIdAndTerm.first, commitLogIdAndTerm.second, localhost, data,
                          totalSize, totalCount, dst, status == SnapshotStatus::DONE);
            ...
            try {
              ...
              if (resp.get_error_code() == cpp2::ErrorCode::SUCCEEDED) {
                ...
                if (status == SnapshotStatus::DONE) {
                  LOG(INFO) << part->idStr_ << "Finished, totalCount " << totalCount << ", totalSize " << totalSize;
                  p.setValue(Status::OK());
                }
                return true;
              } else {
                LOG(INFO) << part->idStr_ << "Sending snapshot failed, we don't retry anymore! "
                          << "The error code is " << static_cast<int32_t>(resp.get_error_code());
                p.setValue(Status::Error("Send snapshot failed!"));
                return false;
              }
            } catch (const std::exception& e) {
              LOG(ERROR) << part->idStr_ << "Send snapshot failed, exception " << e.what() << ", retry " << retry
                         << " times";
              continue;
            }
          }
...
folly::Future<raftex::cpp2::SendSnapshotResponse> SnapshotManager::send(
    GraphSpaceID spaceId, PartitionID partId, TermID termId, LogID committedLogId, TermID committedLogTerm,
    const HostAddr& localhost, const std::vector<std::string>& data, int64_t totalSize, int64_t totalCount,
