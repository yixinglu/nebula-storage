...
struct HBCounters final {
  ...
  void init() {
    if (!numCalls_.valid()) {
      ...
      latency_ = stats::StatsManager::registerHisto("heartbeat_latency_us", 1000, 0, 20000, "avg, p75, p95, p99");
...
class HBProcessor : public BaseProcessor<cpp2::HBResp> {
 ...
 public:
  static HBProcessor* instance(kvstore::KVStore* kvstore, const HBCounters* counters = &kHBCounters,
