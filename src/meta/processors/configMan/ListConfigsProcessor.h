...
class ListConfigsProcessor : public BaseProcessor<cpp2::ListConfigsResp> {
 public:
  static ListConfigsProcessor* instance(kvstore::KVStore* kvstore) { return new ListConfigsProcessor(kvstore); }

  void process(const cpp2::ListConfigsReq& req);

 private:
  explicit ListConfigsProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListConfigsResp>(kvstore) {}
};
