...
class NebulaKeyUtilsV2 final {
 public:
   ...
   * */
  static std::string vertexKey(size_t vIdLen, PartitionID partId, VertexID vId, TagID tagId, TagVersion tv);

  /**
   * Generate edge key for kv store
   * */
  static std::string edgeKey(size_t vIdLen, PartitionID partId, VertexID srcId, EdgeType type, EdgeRanking rank,
  ...
  static std::string edgePrefix(size_t vIdLen, PartitionID partId, VertexID srcId, EdgeType type, EdgeRanking rank,
                                VertexID dstId);
 ...
 private:
  ...
  static constexpr int32_t kEdgeIndexLen = sizeof(PartitionID) + sizeof(IndexID) + sizeof(EdgeRanking);
};
