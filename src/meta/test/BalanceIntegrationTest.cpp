...
TEST(BalanceIntegrationTest, BalanceTest) {
  ...
  for (int i = 0; i < replica; i++) {
    ...
    auto sc =
        storage::TestUtils::mockStorageServer(metaClients[i].get(), dataPath.c_str(), localIp, storagePorts[i], true);
  ...
  HostAddr storageAddr(localIp, storagePort);
  {
    ...
    newServer =
        storage::TestUtils::mockStorageServer(newMetaClient.get(), dataPath.c_str(), localIp, storagePort, true);
  ...
  sleep(FLAGS_raft_heartbeat_interval_secs + 5);
  {
    ...
    ASSERT_EQ(kvstore::ResultCode::SUCCEEDED, part->engine()->prefix(prefix, &iter));
    int num = 0;
    std::string lastKey = "";
    while (iter->valid()) {
...
TEST(BalanceIntegrationTest, LeaderBalanceTest) {
  ...
  while (true) {
    ...
    LOG(INFO) << "Waiting for leader election, current total leader number " << totalLeaders << ", expected "
              << partition;
    sleep(1);
  }
