...
class AddHostIntoZoneProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static AddHostIntoZoneProcessor* instance(kvstore::KVStore* kvstore) { return new AddHostIntoZoneProcessor(kvstore); }

  void process(const cpp2::AddHostIntoZoneReq& req);

 private:
  explicit AddHostIntoZoneProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
...
class DropHostFromZoneProcessor : public BaseProcessor<cpp2::ExecResp> {
 ...
 private:
  explicit DropHostFromZoneProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
