...
class DropTagIndexProcessor : public BaseProcessor<cpp2::ExecResp> {
 public:
  static DropTagIndexProcessor* instance(kvstore::KVStore* kvstore) { return new DropTagIndexProcessor(kvstore); }

  void process(const cpp2::DropTagIndexReq& req);

 private:
  explicit DropTagIndexProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ExecResp>(kvstore) {}
};
