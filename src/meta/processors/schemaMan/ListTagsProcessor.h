...
class ListTagsProcessor : public BaseProcessor<cpp2::ListTagsResp> {
 public:
   ...
   * invoked
   */
  static ListTagsProcessor* instance(kvstore::KVStore* kvstore) { return new ListTagsProcessor(kvstore); }

  void process(const cpp2::ListTagsReq& req);

 private:
  explicit ListTagsProcessor(kvstore::KVStore* kvstore) : BaseProcessor<cpp2::ListTagsResp>(kvstore) {}
};
