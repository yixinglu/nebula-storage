...
class UpgraderSpace {
 public:
  ...
  Status init(meta::MetaClient* mclient, meta::ServerBasedSchemaManager* sMan, meta::IndexManager* iMan,
 ...
 private:
  ...
  void encodeVertexValue(PartitionID partId, RowReader* reader, const meta::NebulaSchemaProvider* schema,
                         std::string& newkey, VertexID& strVid, TagID tagId, std::vector<kvstore::KV>& data);

  // Used for vertex and edge
  std::string encodeRowVal(const RowReader* reader, const meta::NebulaSchemaProvider* schema,
                           std::vector<std::string>& fieldName);

  std::string indexVertexKey(PartitionID partId, VertexID& vId, RowReader* reader,
                             std::shared_ptr<nebula::meta::cpp2::IndexItem> index);

  void encodeEdgeValue(PartitionID partId, RowReader* reader, const meta::NebulaSchemaProvider* schema,
                       std::string& newkey, VertexID& svId, EdgeType type, EdgeRanking rank, VertexID& dstId,
                       std::vector<kvstore::KV>& data);

  std::string indexEdgeKey(PartitionID partId, RowReader* reader, VertexID& svId, EdgeRanking rank, VertexID& dstId,
                           std::shared_ptr<nebula::meta::cpp2::IndexItem> index);

  WriteResult convertValue(const meta::NebulaSchemaProvider* newSchema, const meta::SchemaProviderIf* oldSchema,
 ...
 private:
  ...
  std::unordered_map<TagID, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>> tagSchemas_;
  ...
  std::unordered_map<TagID, std::unordered_set<std::shared_ptr<nebula::meta::cpp2::IndexItem>>> tagIndexes_;

  // Get all edge newest schema in space
  std::unordered_map<EdgeType, std::vector<std::shared_ptr<const meta::NebulaSchemaProvider>>> edgeSchemas_;
  ...
  std::unordered_map<EdgeType, std::unordered_set<std::shared_ptr<nebula::meta::cpp2::IndexItem>>> edgeIndexes_;
