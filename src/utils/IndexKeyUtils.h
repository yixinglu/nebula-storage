...
class IndexKeyUtils final {
 public:
  ...
  static EdgeRanking decodeRank(const folly::StringPiece& raw) { return IndexKeyUtils::decodeInt64(raw); }
  ...
  static nebula::DateTime decodeDateTime(const folly::StringPiece& raw) {
    ...
    int8_t month = *reinterpret_cast<const int8_t*>(raw.data() + sizeof(int16_t));
    int8_t day = *reinterpret_cast<const int8_t*>(raw.data() + sizeof(int16_t) + sizeof(int8_t));
    int8_t hour = *reinterpret_cast<const int8_t*>(raw.data() + sizeof(int16_t) + 2 * sizeof(int8_t));
    int8_t minute = *reinterpret_cast<const int8_t*>(raw.data() + sizeof(int16_t) + 3 * sizeof(int8_t));
    int8_t sec = *reinterpret_cast<const int8_t*>(raw.data() + sizeof(int16_t) + 4 * sizeof(int8_t));
    int32_t microsec = *reinterpret_cast<const int32_t*>(raw.data() + sizeof(int16_t) + 5 * sizeof(int8_t));
  ...
  static Value getValueFromIndexKey(size_t vIdLen, folly::StringPiece key, const std::string& prop,
                                    const std::vector<meta::cpp2::ColumnDef>& cols, bool isEdgeIndex = false,
    ...
    auto it = std::find_if(cols.begin(), cols.end(), [&prop](const auto& col) { return prop == col.get_name(); });
    if (it == cols.end()) {
      return Value(NullType::BAD_DATA);
    }
   ...
   **/
  static std::string encodeValues(std::vector<Value>&& values, const std::vector<nebula::meta::cpp2::ColumnDef>& cols);
   ...
   **/
  static std::string vertexIndexKey(size_t vIdLen, PartitionID partId, IndexID indexId, const VertexID& vId,
                                    std::string&& values);
   ...
   *nullable columns in the index, the parameter can be empty.
   **/
  static std::string edgeIndexKey(size_t vIdLen, PartitionID partId, IndexID indexId, const VertexID& srcId,
                                  EdgeRanking rank, const VertexID& dstId, std::string&& values);

  static std::string indexPrefix(PartitionID partId, IndexID indexId);

  static std::string indexPrefix(PartitionID partId);

  static StatusOr<std::string> collectIndexValues(RowReader* reader,
