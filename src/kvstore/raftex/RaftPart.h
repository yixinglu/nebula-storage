...
class RaftPart : public std::enable_shared_from_this<RaftPart> {
 ...
 public:
  ...
  void processSendSnapshotRequest(const cpp2::SendSnapshotRequest& req, cpp2::SendSnapshotResponse& resp);
 ...
 protected:
  ...
  RaftPart(ClusterID clusterId, GraphSpaceID spaceId, PartitionID partId, HostAddr localAddr,
  ...
  virtual bool preProcessLog(LogID logId, TermID termId, ClusterID clusterId, const std::string& log) = 0;

  // Return <size, count> committed;
  virtual std::pair<int64_t, int64_t> commitSnapshot(const std::vector<std::string>& data, LogID committedLogId,
                                                     TermID committedLogTerm, bool finished) = 0;
 ...
 private:
  ...
  // return FALSE
  bool prepareElectionRequest(cpp2::AskForVoteRequest& req, std::vector<std::shared_ptr<Host>>& hosts);

  // The method returns the partition's role after the election
  Role processElectionResponses(const ElectionResponses& results, std::vector<std::shared_ptr<Host>> hosts,
                                TermID proposedTerm);
  ...
  folly::Future<AppendLogResult> appendLogAsync(ClusterID source, LogType logType, std::string log,
                                                AtomicOp cb = nullptr);

  void appendLogsInternal(AppendLogsIterator iter, TermID termId);

  void replicateLogs(folly::EventBase* eb, AppendLogsIterator iter, TermID currTerm, LogID lastLogId, LogID committedId,
                     TermID prevLogTerm, LogID prevLogId);

  void processAppendLogResponses(const AppendLogResponses& resps, folly::EventBase* eb, AppendLogsIterator iter,
