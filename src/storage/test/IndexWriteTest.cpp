...
int64_t verifyResultNum(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix,
...
TEST(IndexTest, VerticesValueTest) {
  ...
  // verify insert
  {
    ...
    propNames[tagId] = {"col_bool", "col_int", "col_float", "col_float_null", "col_str", "col_date"};
    ...
    // col_date_null
    values.emplace_back(nullValue);
    auto index = IndexKeyUtils::encodeValues(std::move(values), mock::MockData::mockTypicaIndexColumns());

    for (auto partId = 1; partId <= 6; partId++) {
      auto prefix = IndexKeyUtils::indexPrefix(partId, indexId);
      auto indexKey =
          IndexKeyUtils::vertexIndexKey(vIdLen, partId, indexId, convertVertexId(vIdLen, partId), std::move(index));
      std::unique_ptr<kvstore::KVIterator> iter;
