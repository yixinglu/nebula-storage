...
using IndexFilterItem =
    std::unordered_map<int32_t, std::pair<std::unique_ptr<StorageExpressionContext>, std::unique_ptr<Expression>>>;
...
class LookupBaseProcessor : public BaseProcessor<RESP> {
 ...
 protected:
  LookupBaseProcessor(StorageEnv* env, const ProcessorCounters* counters, folly::Executor* executor = nullptr,
                      VertexCache* cache = nullptr)
      : BaseProcessor<RESP>(env, counters), executor_(executor), vertexCache_(cache) {}
  ...
  std::unique_ptr<IndexOutputNode<IndexID>> buildPlanBasic(const cpp2::IndexQueryContext& ctx,
                                                           StoragePlan<IndexID>& plan, bool hasNullableCol,
                                                           const std::vector<meta::cpp2::ColumnDef>& fields);

  std::unique_ptr<IndexOutputNode<IndexID>> buildPlanWithData(const cpp2::IndexQueryContext& ctx,
                                                              StoragePlan<IndexID>& plan);

  std::unique_ptr<IndexOutputNode<IndexID>> buildPlanWithFilter(const cpp2::IndexQueryContext& ctx,
                                                                StoragePlan<IndexID>& plan,
                                                                StorageExpressionContext* exprCtx, Expression* exp);

  std::unique_ptr<IndexOutputNode<IndexID>> buildPlanWithDataAndFilter(const cpp2::IndexQueryContext& ctx,
                                                                       StoragePlan<IndexID>& plan,
