...
cpp2::ScanEdgeRequest buildRequest(PartitionID partId, const std::string& cursor,
                                   const std::pair<EdgeType, std::vector<std::string>>& edge, int64_t rowLimit = 100,
...
void checkResponse(const nebula::DataSet& dataSet, const std::pair<EdgeType, std::vector<std::string>> edge,
...
TEST(ScanEdgeTest, PropertyTest) {
...

  {
    ...
    auto edge =
        std::make_pair(serve, std::vector<std::string>{kSrc, kType, kRank, kDst, "teamName", "startYear", "endYear"});
