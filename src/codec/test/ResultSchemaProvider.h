...
class ResultSchemaProvider : public meta::SchemaProviderIf {
 public:
  class ResultSchemaField : public meta::SchemaProviderIf::Field {
   public:
    explicit ResultSchemaField(std::string name, meta::cpp2::PropertyType type, int16_t size, bool nullable,
                               int32_t offset, size_t nullFlagPos, Expression* defaultValue = nullptr);
 ...
 public:
  ...
  meta::cpp2::PropertyType getFieldType(const folly::StringPiece name) const override;

  const meta::SchemaProviderIf::Field* field(int64_t index) const override;
  const meta::SchemaProviderIf::Field* field(const folly::StringPiece name) const override;
