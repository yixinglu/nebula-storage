    ...
    std::string prefix;
    {
      ...
      while (iter->Valid() && iter->key().starts_with(prefix)) {
        ...
        LOG(INFO) << folly::sformat(
            ...
            "{}",
            spaceId, *desc.space_name_ref(), *desc.partition_num_ref(), *desc.replica_factor_ref());
        iter->Next();
      }
    }
    {
      ...
      while (iter->Valid() && iter->key().starts_with(prefix)) {
        ...
        LOG(INFO) << folly::sformat("space id: {}, part id: {}, hosts: {}", spaceId, partId, ss.str());
        iter->Next();
      }
    }
    {
      ...
      while (iter->Valid() && iter->key().starts_with(prefix)) {
        ...
        auto time = time::TimeUtils::unixSecondsToDateTime(info.lastHBTimeInMilliSec_ / 1000);
        LOG(INFO) << folly::sformat("host addr: {}, role: {}, last hb time: {}", host.toString(), role,
                                    time.toString());
        iter->Next();
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  if (FLAGS_path.empty() || nebula::fs::FileUtils::fileType(FLAGS_path.c_str()) == nebula::fs::FileType::NOTEXIST) {
