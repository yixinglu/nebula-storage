...
class MockCluster {
 public:
  ...
  void startStorage(HostAddr addr, const std::string& rootPath, bool isGeneralService = false,
  ...
  std::unique_ptr<meta::SchemaManager> memSchemaMan(SchemaVer schemaVerCount = 1, GraphSpaceID spaceId = 1,
                                                    bool hasProp = true);

  std::unique_ptr<meta::IndexManager> memIndexMan(GraphSpaceID spaceId = 1, bool hasProp = true);

  static void waitUntilAllElected(kvstore::NebulaStore* kvstore, GraphSpaceID spaceId,
  ...
  static std::unique_ptr<kvstore::NebulaStore> initKV(kvstore::KVOptions options, HostAddr localHost = HostAddr("", 0));

  static std::unique_ptr<kvstore::NebulaStore> initMetaKV(const char* dataPath, HostAddr localHost = HostAddr("", 0));

  void initStorageKV(const char* dataPath, HostAddr localHost = HostAddr("", 0), SchemaVer schemaVerCount = 1,
                     bool hasProp = true, bool hasListener = false,
                     const std::vector<meta::cpp2::FTClient>& clients = {}, bool needCffBuilder = false);
