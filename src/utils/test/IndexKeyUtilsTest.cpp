...
TEST(IndexKeyUtilsTest, edgeIndexKeyV1) {
  auto values = getIndexValues();
  auto key = IndexKeyUtils::edgeIndexKey(8, 1, 1, getStringId(1), 1, getStringId(2), std::move(values));
  ASSERT_EQ(1, IndexKeyUtils::getIndexId(key));
...
void verifyDecodeIndexKey(bool isEdge, bool nullable, size_t vIdLen,
                          const std::vector<std::pair<VertexID, std::vector<Value>>>& data,
                          const std::vector<std::string>& indexKeys, const std::vector<meta::cpp2::ColumnDef>& cols) {
  for (size_t j = 0; j < data.size(); j++) {
    std::vector<Value> actual;
    actual.emplace_back(IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_bool", cols, isEdge, nullable));
    actual.emplace_back(IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_int", cols, isEdge, nullable));
    actual.emplace_back(IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_float", cols, isEdge, nullable));
    actual.emplace_back(
        IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_string", cols, isEdge, nullable));
    actual.emplace_back(IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_date", cols, isEdge, nullable));
    actual.emplace_back(
        IndexKeyUtils::getValueFromIndexKey(vIdLen, indexKeys[j], "col_datetime", cols, isEdge, nullable));
...
TEST(IndexKeyUtilsTest, getValueFromIndexKeyTest) {
  ...
  // vertices test without nullable column
  {
    ...
    for (auto& row : vertices) {
      ...
      indexKeys.emplace_back(IndexKeyUtils::vertexIndexKey(vIdLen, partId, indexId, row.first, std::move(values)));
    }
  ...
  // edges test without nullable column
  {
    ...
    for (auto& row : edges) {
      ...
      indexKeys.emplace_back(
          IndexKeyUtils::edgeIndexKey(vIdLen, partId, indexId, row.first, 0, row.first, std::move(values)));
    }
  ...
  // vertices test with nullable
  {
    ...
    for (auto& row : vertices) {
      auto values = IndexKeyUtils::encodeValues(std::move(row.second), cols);
      ASSERT_EQ(indexValueSize, values.size());
      auto key = IndexKeyUtils::vertexIndexKey(vIdLen, partId, indexId, row.first, std::move(values));
