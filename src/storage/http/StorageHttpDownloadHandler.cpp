...
void StorageHttpDownloadHandler::init(nebula::hdfs::HdfsHelper* helper, nebula::thread::GenericThreadPool* pool,
                                      nebula::kvstore::KVStore* kvstore, std::vector<std::string> paths) {
  helper_ = helper;
  pool_ = pool;
  kvstore_ = kvstore;
  paths_ = paths;
...
void StorageHttpDownloadHandler::onRequest(std::unique_ptr<HTTPMessage> headers) noexcept {
  ...
  if (!headers->hasQueryParam("host") || !headers->hasQueryParam("port") || !headers->hasQueryParam("path") ||
      !headers->hasQueryParam("parts") || !headers->hasQueryParam("space")) {
...
void StorageHttpDownloadHandler::onEOM() noexcept {
  ...
  if (helper_->checkHadoopPath()) {
    ...
    if (downloadSSTFiles(hdfsHost_, hdfsPort_, hdfsPath_, parts)) {
      ResponseBuilder(downstream_)
          .status(WebServiceUtils::to(HttpStatusCode::OK), WebServiceUtils::toString(HttpStatusCode::OK))
          .body("SSTFile download successfully")
          .sendWithEOM();
    } else {
      ResponseBuilder(downstream_)
          .status(WebServiceUtils::to(HttpStatusCode::FORBIDDEN), WebServiceUtils::toString(HttpStatusCode::FORBIDDEN))
    ...
    ResponseBuilder(downstream_)
        .status(WebServiceUtils::to(HttpStatusCode::NOT_FOUND), WebServiceUtils::toString(HttpStatusCode::NOT_FOUND))
        .sendWithEOM();
  }
}
...
void StorageHttpDownloadHandler::onError(ProxygenError error) noexcept {
  LOG(ERROR) << "Web Service StorageHttpDownloadHandler got error: " << proxygen::getErrorString(error);
}

bool StorageHttpDownloadHandler::downloadSSTFiles(const std::string& hdfsHost, int32_t hdfsPort,
                                                  const std::string& hdfsPath, const std::vector<std::string>& parts) {
