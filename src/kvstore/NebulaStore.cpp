      ...
      try {
        ...
        if (!options_.partMan_->spaceExist(storeSvcAddr_, spaceId).ok()) {
          if (FLAGS_auto_remove_invalid_space) {
            ...
            LOG(INFO) << "Space " << spaceId << " does not exist any more, remove the data!";
        ...
        for (auto& partId : enginePtr->allParts()) {
          if (!options_.partMan_->partExist(storeSvcAddr_, spaceId, partId).ok()) {
            ...
            if (spacePartIdSet.find(spacePart) != spacePartIdSet.end()) {
              LOG(INFO) << "Part " << partId << " has been loaded, skip current one, remove it!";
              enginePtr->removePart(partId);
            } else {
        ...
        for (auto& partId : partIds) {
          bgWorkers_->addTask([spaceId, partId, enginePtr, &counter, &baton, this]() mutable {
            auto part = std::make_shared<Part>(spaceId, partId, raftAddr_,
                                               folly::stringPrintf("%s/wal/%d", enginePtr->getDataRoot(), partId),
                                               enginePtr, ioPool_, bgWorkers_, workers_, snapshot_, clientMan_);
...
void NebulaStore::addPart(GraphSpaceID spaceId, PartitionID partId, bool asLearner,
  ...
  spaceIt->second->parts_.emplace(partId, newPart(spaceId, partId, targetEngine.get(), asLearner, peers));
  LOG(INFO) << "Space " << spaceId << ", part " << partId << " has been added, asLearner " << asLearner;
}

std::shared_ptr<Part> NebulaStore::newPart(GraphSpaceID spaceId, PartitionID partId, KVEngine* engine, bool asLearner,
                                           const std::vector<HostAddr>& defaultPeers) {
  auto part = std::make_shared<Part>(spaceId, partId, raftAddr_,
                                     folly::stringPrintf("%s/wal/%d", engine->getDataRoot(), partId), engine, ioPool_,
                                     bgWorkers_, workers_, snapshot_, clientMan_);
  std::vector<HostAddr> peers;
  if (defaultPeers.empty()) {
    // pull the information from meta
    auto metaStatus = options_.partMan_->partMeta(spaceId, partId);
    if (!metaStatus.ok()) {
      LOG(ERROR) << "options_.partMan_->partMeta(spaceId, partId); error: " << metaStatus.status().toString()
...
void NebulaStore::addListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type,
                              const std::vector<HostAddr>& peers) {
  folly::RWSpinLock::WriteHolder wh(&lock_);
  ...
  if (listener != partIt->second.end()) {
    LOG(INFO) << "Listener of type " << static_cast<int32_t>(type) << " of [Space: " << spaceId << ", Part: " << partId
              << "] has existed!";
    return;
  }
...
std::shared_ptr<Listener> NebulaStore::newListener(GraphSpaceID spaceId, PartitionID partId,
                                                   meta::cpp2::ListenerType type, const std::vector<HostAddr>& peers) {
  auto walPath = folly::stringPrintf("%s/%d/%d/wal", options_.listenerPath_.c_str(), spaceId, partId);
  // snapshot manager and client manager is set to nullptr, listener should
  // never use them
  auto listener = ListenerFactory::createListener(type, spaceId, partId, raftAddr_, walPath, ioPool_, bgWorkers_,
                                                  workers_, nullptr, nullptr, options_.schemaMan_);
...
void NebulaStore::removeListener(GraphSpaceID spaceId, PartitionID partId, meta::cpp2::ListenerType type) {
  folly::RWSpinLock::WriteHolder wh(&lock_);
  auto spaceIt = spaceListeners_.find(spaceId);
  if (spaceIt != spaceListeners_.end()) {
    auto partIt = spaceIt->second->listeners_.find(partId);
    if (partIt != spaceIt->second->listeners_.end()) {
      ...
      if (listener != partIt->second.end()) {
        ...
        LOG(INFO) << "Listener of type " << static_cast<int32_t>(type) << " of [Space: " << spaceId
                  << ", Part: " << partId << "] is removed";
        return;
      }
    }
  }
...
void NebulaStore::checkRemoteListeners(GraphSpaceID spaceId, PartitionID partId,
  ...
  if (spaceIt != spaces_.end()) {
    ...
    if (partIt != spaceIt->second->parts_.end()) {
      ...
      std::transform(remoteListeners.begin(), remoteListeners.end(), std::inserter(raftHosts, raftHosts.end()),
...
void NebulaStore::updateSpaceOption(GraphSpaceID spaceId, const std::unordered_map<std::string, std::string>& options,
                                    bool isDbOption) {
  if (isDbOption) {
...
ResultCode NebulaStore::get(GraphSpaceID spaceId, PartitionID partId, const std::string& key, std::string* value,
...
std::pair<ResultCode, std::vector<Status>> NebulaStore::multiGet(GraphSpaceID spaceId, PartitionID partId,
...
ResultCode NebulaStore::range(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                              const std::string& end, std::unique_ptr<KVIterator>* iter, bool canReadFromFollower) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    return error(ret);
  }
...
ResultCode NebulaStore::prefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix,
                               std::unique_ptr<KVIterator>* iter, bool canReadFromFollower) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    return error(ret);
  }
...
ResultCode NebulaStore::rangeWithPrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
...
void NebulaStore::asyncAppendBatch(GraphSpaceID spaceId, PartitionID partId, std::string&& batch, KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
    return;
...
void NebulaStore::asyncMultiPut(GraphSpaceID spaceId, PartitionID partId, std::vector<KV>&& keyValues, KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
    return;
...
void NebulaStore::asyncRemove(GraphSpaceID spaceId, PartitionID partId, const std::string& key, KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
    return;
...
void NebulaStore::asyncMultiRemove(GraphSpaceID spaceId, PartitionID partId, std::vector<std::string>&& keys,
                                   KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
...
void NebulaStore::asyncRemoveRange(GraphSpaceID spaceId, PartitionID partId, const std::string& start,
                                   const std::string& end, KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
    return;
...
void NebulaStore::asyncAtomicOp(GraphSpaceID spaceId, PartitionID partId, raftex::AtomicOp op, KVCallback cb) {
  auto ret = part(spaceId, partId);
  if (!ok(ret)) {
    cb(error(ret));
    return;
...
ErrorOr<ResultCode, std::shared_ptr<Part>> NebulaStore::part(GraphSpaceID spaceId, PartitionID partId) {
  folly::RWSpinLock::ReadHolder rh(&lock_);
  auto it = spaces_.find(spaceId);
  if (UNLIKELY(it == spaces_.end())) {
    return ResultCode::ERR_SPACE_NOT_FOUND;
...
ResultCode NebulaStore::setOption(GraphSpaceID spaceId, const std::string& configKey, const std::string& configValue) {
  auto spaceRet = space(spaceId);
  if (!ok(spaceRet)) {
    return error(spaceRet);
  }
...
ResultCode NebulaStore::setDBOption(GraphSpaceID spaceId, const std::string& configKey,
...
ErrorOr<ResultCode, std::string> NebulaStore::createCheckpoint(GraphSpaceID spaceId, const std::string& name) {
  auto spaceRet = space(spaceId);
  if (!ok(spaceRet)) {
    return error(spaceRet);
  }
  ...
  for (auto& engine : space->engines_) {
    ...
    for (auto& part : parts) {
      ...
      auto walPath = folly::stringPrintf("%s/checkpoints/%s/wal/%d", engine->getDataRoot(), name.c_str(), part);
...
ResultCode NebulaStore::dropCheckpoint(GraphSpaceID spaceId, const std::string& name) {
  ...
  for (auto& engine : space->engines_) {
     ...
     **/
    auto checkpointPath = folly::stringPrintf("%s/checkpoints/%s", engine->getDataRoot(), name.c_str());
...
int32_t NebulaStore::allLeader(std::unordered_map<GraphSpaceID, std::vector<PartitionID>>& leaderIds) {
  folly::RWSpinLock::ReadHolder rh(&lock_);
  int32_t count = 0;
...
void NebulaStore::cleanWAL() {
  SCOPE_EXIT { bgWorkers_->addDelayTask(FLAGS_clean_wal_interval_secs * 1000, &NebulaStore::cleanWAL, this); };
...
ErrorOr<ResultCode, std::vector<std::string>> NebulaStore::backupTable(
    GraphSpaceID spaceId, const std::string& name, const std::string& tablePrefix,
  ...
  for (auto& engine : space->engines_) {
    ...
    if (!ok(path)) {
      ...
      LOG(WARNING) << "Since the table(" << tablePrefix << ") is empty, the backup of the current table is skipped.";
      continue;
    }
...
ResultCode NebulaStore::restoreFromFiles(GraphSpaceID spaceId, const std::vector<std::string>& files) {
  auto spaceRet = space(spaceId);
  if (!ok(spaceRet)) {
    LOG(ERROR) << "Get Space " << spaceId << " Failed";
    return error(spaceRet);
