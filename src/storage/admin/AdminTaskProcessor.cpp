...
void AdminTaskProcessor::process(const cpp2::AddAdminTaskRequest& req) {
  ...
  auto toMetaErrCode = [](storage::cpp2::ErrorCode storageCode) {
    switch (storageCode) {
      ...
      default:
        LOG(ERROR) << "unsupported conversion of code " << apache::thrift::util::enumNameSafe(storageCode);
        return meta::cpp2::ErrorCode::E_UNKNOWN;
    }
  };

  auto cb = [env = env_, toMetaErrCode = std::move(toMetaErrCode), jobId = req.get_job_id(),
             taskId = req.get_task_id()](nebula::storage::cpp2::ErrorCode errCode,
                                         nebula::meta::cpp2::StatisItem& result) {
    meta::cpp2::StatisItem* pStatis = nullptr;
    if (errCode == cpp2::ErrorCode::SUCCEEDED && *result.status_ref() == nebula::meta::cpp2::JobStatus::FINISHED) {
      pStatis = &result;
    }

    auto metaCode = toMetaErrCode(errCode);
    LOG(INFO) << folly::sformat("reportTaskFinish(), job={}, task={}, rc={}", jobId, taskId,
    ...
    while (retry++ < maxRetry) {
      ...
      fut.wait();
      if (!fut.hasValue()) {
        LOG(INFO) << folly::sformat("reportTaskFinish() got rpc error:, job={}, task={}", jobId, taskId);
        continue;
      }
      if (!fut.value().ok()) {
        LOG(INFO) << folly::sformat("reportTaskFinish() has bad status:, job={}, task={}, rc={}", jobId, taskId,
      ...
      if (rc == meta::cpp2::ErrorCode::E_LEADER_CHANGED || rc == meta::cpp2::ErrorCode::E_STORE_FAILURE) {
        continue;
      } else {
        break;
      }
