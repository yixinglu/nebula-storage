...
class HBaseRangeIter : public KVIterator {
 public:
  HBaseRangeIter(KVArrayIterator begin, KVArrayIterator end) : current_(begin), begin_(begin), end_(end) {}
...
class HBaseStore : public KVStore {
 public:
  ...
  void asyncRemovePrefix(GraphSpaceID spaceId, PartitionID partId, const std::string& prefix, KVCallback cb);
  ...
  ResultCode createCheckpoint(GraphSpaceID, const std::string&) override { return ResultCode::ERR_UNSUPPORTED; }

  ResultCode dropCheckpoint(GraphSpaceID, const std::string&) override { return ResultCode::ERR_UNSUPPORTED; }
 ...
 private:
  ...
  std::shared_ptr<const meta::SchemaProviderIf> getSchema(GraphSpaceID spaceId, const std::string& key,
                                                          SchemaVer version = -1);
  ...
  ResultCode range(GraphSpaceID spaceId, const std::string& start, const std::string& end,
                   std::unique_ptr<KVIterator>* iter);

  ResultCode prefix(GraphSpaceID spaceId, const std::string& prefix, std::unique_ptr<KVIterator>* iter);
